...
public:
    inline static std::shared_ptr<EmbeddingInfo> Make(EmbeddingDataType embedding_data_type, size_t dimension) {
        ParserAssert(dimension <= EMBEDDING_LIMIT,
                   "Embedding dimension should less than " + std::to_string(EMBEDDING_LIMIT)) return std::make_shared<EmbeddingInfo>(embedding_data_type,
                                                                                                                               dimension);
    }
