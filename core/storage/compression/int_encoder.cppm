...
template <typename T, typename Compressor>
inline uint32_t IntEncoder<T, Compressor>::Encode(ByteSliceWriter &slice_writer, const T *src, uint32_t src_len) const {
    uint8_t buffer[ENCODER_BUFFER_BYTE_SIZE];
    uint32_t len;
...
template <typename T, typename Compressor>
inline uint32_t IntEncoder<T, Compressor>::Decode(T *dest, uint32_t dest_len, ByteSliceReader &slice_reader) const {
    uint8_t buffer[ENCODER_BUFFER_BYTE_SIZE];
