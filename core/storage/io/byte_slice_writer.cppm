...
public:
    ...
    void Write(const ByteSliceList &src, uint32_t start, uint32_t end);
    ...
    uint32_t GetAllocatedSize() const { return allocated_size_; }
...
private:
    ...
    ByteSlice *CreateSlice(uint32_t size);
...
inline uint32_t ByteSliceWriter::GetIncrementedSliceSize(uint32_t last_slice_size) {
    uint32_t prev = last_slice_size + ByteSlice::GetHeadSize();
    uint32_t slice_size = prev + (prev >> 2) - ByteSlice::GetHeadSize();
    uint32_t chunk_size = MemoryPool::DEFAULT_CHUNK_SIZE - ByteSlice::GetHeadSize() - sizeof(ChainedMemoryChunk);
