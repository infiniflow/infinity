...
This section provides instructions on building Infinity from source on Ubuntu.
...
:::tip NOTE
You must *also* install `simde` v0.7.4+ if using Ubuntu 22.04 on ARM architecture.
...
### 2. Build and run unit test with code coverage option on 
...
### 1. Build and start up Infinity server with code coverage option on
```shell
cmake -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_VERBOSE_MAKEFILE=ON -DCODE_COVERAGE=ON ..
cmake --build . -t infinity
find . -name "*.gcda"  | xargs rm -f
