...
This document provides instructions for loading Infinity's configuration file and descriptions of each entry in it.
...
1. Map the directory for the configuration file path to inside the Docker container.
2. When running Docker, specify the path to the configuration file using the `-f` option. For example: 
...
- It is in the local folder where Infinity embedded is saved (the path that you specify when calling `connect()`), for example `/var/infinity/`:
...
- If you do not specify a configuration file, Infinity will load all the default settings.
- If you specify a configuration file when starting Infinity, the settings in your configuration file override the corresponding default settings.
...
# The persistence manager maps block and index files to remote stored objects and manages the local cache
