...
class TestInsertDeleteUpdate:
    @pytest.mark.skip(reason="#issue 1087 Decrease row count exceed actual row count@src/storage/meta/entry/segment_entry.cppm:184")
...
def create_table(db_obj : Database):
    try:
        db_obj.create_table(table_name,  {"tag": "int", "c1": "vector,4,float"}, ConflictType.Error)
...
def insert(db_obj : Database):
    try:
        ...
        value = []
        for i in range(100):
            value.append({"tag" : random.randint(0,9), "c1": [random.random(), random.random(), random.random(), random.random()]})
