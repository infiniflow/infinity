...
class TestInfinity:
    ...
    def test_create_index_secondary_functional(self, suffix):
        ...
        pd.testing.assert_frame_equal(res, pd.DataFrame({'c1': [4], 'sqrt(c1)': [2.0]})
                                      .astype({'c1': dtype('int32'), 'sqrt(c1)': dtype('float64')}))
    ...
    def test_insert_data_fulltext_index_search(self, file_format, suffix):
        ...
        res, extra_result = table_obj.output(["doctitle", "docdate", "body", "_row_id", "_score"]).highlight(
            ["body"]).match_text(
            "body^5", "harmful chemical", 3).to_pl()
        assert not res.is_empty()
    ...
    def test_create_index_with_invalid_options(self, conflict_type, suffix):
        ...
        assert e.type == InfinityException
        assert e.value.args[0] == ErrorCode.INVALID_CONFLICT_TYPE or e.value.args[0] == ErrorCode.INVALID_CREATE_OPTION
    ...
    def test_drop_index_with_invalid_options(self, conflict_type, suffix):
        ...
        assert e.type == InfinityException
        assert e.value.args[0] == ErrorCode.INVALID_CONFLICT_TYPE or e.value.args[0] == ErrorCode.INVALID_DROP_OPTION
