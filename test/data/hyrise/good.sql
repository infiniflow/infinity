...
SELECT col1 AS myname, col2, 'test' FROM t1, foo AS t WHERE age > 12 AND zipcode = 12345 GROUP BY col1;
SELECT * from t1 JOIN table2 ON a = b WHERE (b OR NOT a) AND a = 12.5
(SELECT a FROM foo WHERE a > 12 OR b > 3 AND c NOT LIKE 's%' LIMIT 10);
SELECT * FROM t1 LIMIT 10 OFFSET 10; SELECT * FROM another;
...
# JOIN
SELECT t1.a, t1.b, t2.c FROM t1 AS t1 JOIN (SELECT * FROM foo JOIN bar ON foo.id = bar.id) t2 ON t1.a = t2.b WHERE (t1.b OR NOT t1.a) AND t2.c = 12.5
SELECT * FROM t1 JOIN t2 ON c1 = c2;
SELECT a, SUM(b) FROM t2 GROUP BY a HAVING SUM(b) > 100;
# CREATE statement
-- CREATE TABLE t1 FROM TBL FILE 'students.tbl'
...
CREATE TABLE teachers (name VARCHAR(30), student_number BIGINT, PRIMARY KEY (name, student_number), city CHAR(10), grade FLOAT)
