...
size_t MLASCALL MlasQ4GemmPackBSize(MLAS_BLK_QUANT_TYPE QType, size_t N, size_t K) {
...
template <typename T>
MLAS_FORCEINLINE void MlasQ4GemmPackBImpl(void *PackedBuf, const float *FpData, size_t N, size_t K, size_t ldb) {
...
template <>
MLAS_FORCEINLINE void MlasQ4GemmPackBImpl<MLAS_Q4TYPE_BLK1>(void *PackedBuf, const float *FpData, size_t N, size_t K, size_t ldb) {
    auto *dst_ptr = reinterpret_cast<uint8_t *>(PackedBuf);
...
void MLASCALL MlasQ4GemmPackB(MLAS_BLK_QUANT_TYPE QType, void *PackedBuf, const float *FpData, size_t N, size_t K, size_t ldb) {
    switch (QType) {
        case BlkQ4Sym:
...
template <typename T>
MLAS_FORCEINLINE void MlasQ4GemmUnPackBImpl(float *FpData, const void *PackedBuf, size_t N, size_t K, size_t ldb) {
...
template <>
MLAS_FORCEINLINE void MlasQ4GemmUnPackBImpl<MLAS_Q4TYPE_BLK1>(float *FpData, const void *PackedBuf, size_t N, size_t K, size_t ldb) {
    const auto *src = reinterpret_cast<const uint8_t *>(PackedBuf);
    for (size_t n = 0; n < N; n++) {
        for (size_t k = 0; k < K; k += MLAS_Q4TYPE_BLK1::BlkLen) {
...
void MLASCALL MlasQ4GemmUnPackB(MLAS_BLK_QUANT_TYPE QType, float *FpData, const void *PackedBuf, size_t N, size_t K, size_t ldb) {
    switch (QType) {
        case BlkQ4Sym:
...
MLAS_FORCEINLINE void range2scalezp(float min, float max, ScaleT &scale, uint8_t &zp) {
...
template <typename ScaleT, int qbits>
MLAS_FORCEINLINE void range2scale(float min, float max, ScaleT &scale) {
...
struct BlockwiseQuantizer {
    ...
    quantizedBufferSizes(int rows, int columns, size_t &data_bytes, size_t &scale_num_elements, size_t *zero_point_bytes) {
        ...
        MlasTryBatchParallel(thread_pool, total_thrd_blks, [&](ptrdiff_t block_idx) {
            ...
            for (int32_t j = c; j < c_end; ++j) {
                ...
                for (int32_t i = r; i < r_end; i += 2) {
                    ...
                    const uint8_t vi0 = (uint8_t)std::clamp(roundf(v0 * reciprocal_scale + zp), 0.0f, BitsTraits<qbits>::kMaxFp);
                    ...
                    if (i + 1 < r_end) {
                        ...
                        vi1 = (uint8_t)std::clamp(roundf(v1 * reciprocal_scale1 + zp1), 0.0f, BitsTraits<qbits>::kMaxFp);
                    }
        ...
        MlasTryBatchParallel(thread_pool, total_thrd_blks, [&](ptrdiff_t block_idx) {
            ...
            for (int32_t j = c; j < c_end; ++j) {
                ...
                for (int32_t i = r; i < r_end; i += 2) {
                    ...
                    const int zp_pair = (zero_points == nullptr) ? 0x88 : zero_points[meta_col * ((row_blks + 1) / 2) + meta_row / 2];
...
struct BlockwiseQDQQuantizer<Tin, 4> {
    static MLAS_FORCEINLINE uint8_t GetElem(uint8_t val, int32_t idx) { return (val >> (idx << 2)) & 0xF; }
    ...
    static MLAS_FORCEINLINE uint8_t Pack(uint8_t v0, uint8_t v1) { return (v0 & 0xF) | ((v1 & 0xF) << 4); }
    ...
    static MLAS_FORCEINLINE uint8_t QuantizeV(Tin src, float reciprocal_scale, uint8_t zero_point) {
        return static_cast<uint8_t>(
            std::clamp(static_cast<int32_t>(std::roundf(static_cast<float>(src) * reciprocal_scale)) + static_cast<int32_t>(zero_point),
                       0,
                       BitsTraits<4>::kMax));
    }
     ...
     */
    static void QuantizeRowWise(const Tin *src,
                                Tin *scales,
                                uint8_t *zero_points,
                                uint8_t *dst,
                                int32_t rows,
        ...
        throw std::runtime_error("BlockwiseQDQQuantizer::BlockwiseQDQQuantizer is not implemented");
    }
     ...
     * @param thread_pool       thread pool for parallel processing
     */
    static void QuantizeColumnWise(const Tin *src,
                                   Tin *scales,
                                   uint8_t *zero_points,
                                   uint8_t *dst,
                                   int32_t rows,
        ...
        if (columns & 1) {
            QuantizeColumnWisePackUnaligned(src, scales, zero_points, dst, rows, columns, quant_block_size, thread_pool);
        } else {
            QuantizeColumnWisePackAligned(src, scales, zero_points, dst, rows, columns, quant_block_size, thread_pool);
        }
    }
     ...
     * @param thread_pool       thread pool for parallel processing
     */
    static void TransposeColumnWiseQuantized(const uint8_t *src_weights,
        ...
        // block size based on the parity of columns.
        if (columns & 1) {
            TransposeColumnWiseQuantizedPackUnaligned(src_weights,
                                                      src_scales,
                                                      src_zero_points,
                                                      dst_weights,
                                                      dst_scales,
            ...
            TransposeColumnWiseQuantizedPackAligned(src_weights,
                                                    src_scales,
                                                    src_zero_points,
                                                    dst_weights,
                                                    dst_scales,
...
private:
    static void QuantizeColumnWisePackAligned(const Tin *src,
                                              Tin *scales,
                                              uint8_t *zero_points,
                                              uint8_t *dst,
                                              int32_t rows,
                                              ...
                                              MLAS_THREADPOOL *thread_pool) {
        if (columns % 2 != 0) {
            throw std::runtime_error("Columns must be multiple of 2.");
        }
        ...
        MlasTryBatchParallel(thread_pool, static_cast<ptrdiff_t>(num_thread_blk), [&](ptrdiff_t thread_blk_idx) {
            // !!warning!!: buffering the whole thread block
            constexpr int32_t buffer_size = 128;
            if (buffer_size != thread_blk_size) {
                throw std::runtime_error("buffer size must be equal to thread block size.");
            }
    ...
    static void QuantizeColumnWisePackUnaligned(const Tin *src,
                                                Tin *scales,
                                                uint8_t *zero_points,
                                                uint8_t *dst,
                                                int32_t rows,
        ...
        MlasTryBatchParallel(thread_pool, static_cast<ptrdiff_t>(num_row_thread_blk), [&](ptrdiff_t thread_blk_idx) {
            constexpr int32_t buffer_size = 128;
            float reciprocal_scale_t[buffer_size];
            uint8_t zp_t[buffer_size];
            float vmin_t[buffer_size];
                ...
                for (int32_t buffer_idx = 0; buffer_idx < columns; buffer_idx += buffer_size) {
                    ...
                    if (scale_buffer_idx & 1) {
                        ...
                        if (zero_points) {
                            ...
                            zero_points[scale_buffer_idx >> 1] = SetElem(v0_tt, 1, zero_points[scale_buffer_idx >> 1]);
                        } else {
                    ...
                    if (scale_buffer_idx < scale_buffer_idx_end) {
                        ...
                        if (zero_points) {
                            range2scalezp<Tin, 4>(vmin_t[col_idx], vmax_t[col_idx], scale0_tt, v0_tt);
                            zero_points[scale_buffer_idx >> 1] = SetElem(v0_tt, 0, zero_points[scale_buffer_idx >> 1]);
                        } else {
                            range2scale<Tin, 4>(vmin_t[col_idx], vmax_t[col_idx], scale0_tt);
                        }
                    ...
                    for (int32_t j = 0; j < quant_row_size; ++j, input_idx_t += columns) {
                        ...
                        for (; input_idx_t_start < input_idx_t_end - 1; col_idx += 2, input_idx_t_start += 2) {
                            ...
                            v1_tt = QuantizeV(src[input_idx_t_start + 1], reciprocal_scale_t[col_idx + 1], zp_t[col_idx + 1]);
    ...
    static void TransposeColumnWiseQuantizedPackAligned(const uint8_t *src_weights,     // [rows, columns / 2]
        ...
        MlasTryBatchParallel(thread_pool, static_cast<ptrdiff_t>(columns), [&](ptrdiff_t thread_blk_idx) {
        ...
        if (src_zero_points) {
            ...
            MlasTryBatchParallel(thread_pool, static_cast<ptrdiff_t>(packed_col_size), [&](ptrdiff_t thread_blk_idx) {
                uint8_t src0_t, src1_t;
                uint8_t dst0_t, dst1_t;
    ...
    static void
    TransposeColumnWiseQuantizedPackUnaligned(const uint8_t *src_weights,     // size of [ceil(rows * columns / 2)]
                                              const Tin *src_scales,          // [ceil(rows / quant_block_size), columns]
...
void MlasBlockwiseQuantMetaShape(int block_size, bool columnwise, int rows, int columns, int &meta_rows, int &meta_cols) {
    switch (block_size) {
        case 16: {
            if (columnwise) {
...
template <typename T, int qbits>
void MlasBlockwiseQuantizedShape(int block_size, bool columnwise, int rows, int columns, int &q_rows, int &q_cols) {
    switch (block_size) {
        case 16: {
            if (columnwise) {
...
template void MlasBlockwiseQuantMetaShape<float, 4>(int block_size, bool columnwise, int rows, int columns, int &meta_rows, int &meta_cols);

template void MlasBlockwiseQuantMetaShape<MLAS_FP16, 4>(int block_size, bool columnwise, int rows, int columns, int &meta_rows, int &meta_cols);

template void MlasBlockwiseQuantizedShape<float, 4>(int block_size, bool columnwise, int rows, int columns, int &q_rows, int &q_cols);

template void MlasBlockwiseQuantizedShape<MLAS_FP16, 4>(int block_size, bool columnwise, int rows, int columns, int &q_rows, int &q_cols);

void MLASCALL MlasBlockwiseQuantizedBufferSizes(int qbits,
                                                int block_size,
                                                bool columnwise,
                                                int rows,
                                                int columns,
    ...
    if (qbits == 4) {
        switch (block_size) {
            case 16:
                if (columnwise) {
                    BlockwiseQuantizer<float, 16, 4, true>::quantizedBufferSizes(rows,
                                                                                 columns,
                                                                                 q_data_size_in_bytes,
                                                                                 q_scale_num_elements,
                                                                                 q_zero_point_size_in_bytes);
                } else {
                    BlockwiseQuantizer<float, 16, 4, false>::quantizedBufferSizes(rows,
                                                                                  columns,
                                                                                  q_data_size_in_bytes,
                                                                                  q_scale_num_elements,
                                                                                  q_zero_point_size_in_bytes);
            ...
            case 32:
                if (columnwise) {
                    BlockwiseQuantizer<float, 32, 4, true>::quantizedBufferSizes(rows,
                                                                                 columns,
                                                                                 q_data_size_in_bytes,
                                                                                 q_scale_num_elements,
                                                                                 q_zero_point_size_in_bytes);
                } else {
                    BlockwiseQuantizer<float, 32, 4, false>::quantizedBufferSizes(rows,
                                                                                  columns,
                                                                                  q_data_size_in_bytes,
                                                                                  q_scale_num_elements,
                                                                                  q_zero_point_size_in_bytes);
            ...
            case 64:
                if (columnwise) {
                    BlockwiseQuantizer<float, 64, 4, true>::quantizedBufferSizes(rows,
                                                                                 columns,
                                                                                 q_data_size_in_bytes,
                                                                                 q_scale_num_elements,
                                                                                 q_zero_point_size_in_bytes);
                } else {
                    BlockwiseQuantizer<float, 64, 4, false>::quantizedBufferSizes(rows,
                                                                                  columns,
                                                                                  q_data_size_in_bytes,
                                                                                  q_scale_num_elements,
                                                                                  q_zero_point_size_in_bytes);
            ...
            case 128:
                if (columnwise) {
                    BlockwiseQuantizer<float, 128, 4, true>::quantizedBufferSizes(rows,
                                                                                  columns,
                                                                                  q_data_size_in_bytes,
                                                                                  q_scale_num_elements,
                                                                                  q_zero_point_size_in_bytes);
                } else {
                    BlockwiseQuantizer<float, 128, 4, false>::quantizedBufferSizes(rows,
                                                                                   columns,
                                                                                   q_data_size_in_bytes,
                                                                                   q_scale_num_elements,
                                                                                   q_zero_point_size_in_bytes);
            ...
            case 256:
                if (columnwise) {
                    BlockwiseQuantizer<float, 256, 4, true>::quantizedBufferSizes(rows,
                                                                                  columns,
                                                                                  q_data_size_in_bytes,
                                                                                  q_scale_num_elements,
                                                                                  q_zero_point_size_in_bytes);
                } else {
                    BlockwiseQuantizer<float, 256, 4, false>::quantizedBufferSizes(rows,
                                                                                   columns,
                                                                                   q_data_size_in_bytes,
                                                                                   q_scale_num_elements,
                                                                                   q_zero_point_size_in_bytes);
...
template <typename T, int qbits>
void MlasQuantizeBlockwise(uint8_t *dst,
                           T *scales,
                           uint8_t *zero_points,
                           const T *src,
                           int block_size,
    ...
    switch (block_size) {
        case 16:
            if (columnwise) {
                BlockwiseQuantizer<T, 16, qbits, true>::quantizeAndTranspose(dst,
                                                                             scales,
                                                                             zero_points,
                                                                             src,
                                                                             rows,
                ...
                BlockwiseQuantizer<T, 16, qbits, false>::quantizeAndTranspose(dst,
                                                                              scales,
                                                                              zero_points,
                                                                              src,
                                                                              rows,
        ...
        case 32:
            if (columnwise) {
                BlockwiseQuantizer<T, 32, qbits, true>::quantizeAndTranspose(dst,
                                                                             scales,
                                                                             zero_points,
                                                                             src,
                                                                             rows,
                ...
                BlockwiseQuantizer<T, 32, qbits, false>::quantizeAndTranspose(dst,
                                                                              scales,
                                                                              zero_points,
                                                                              src,
                                                                              rows,
        ...
        case 64:
            if (columnwise) {
                BlockwiseQuantizer<T, 64, qbits, true>::quantizeAndTranspose(dst,
                                                                             scales,
                                                                             zero_points,
                                                                             src,
                                                                             rows,
                ...
                BlockwiseQuantizer<T, 64, qbits, false>::quantizeAndTranspose(dst,
                                                                              scales,
                                                                              zero_points,
                                                                              src,
                                                                              rows,
        ...
        case 128:
            if (columnwise) {
                BlockwiseQuantizer<T, 128, qbits, true>::quantizeAndTranspose(dst,
                                                                              scales,
                                                                              zero_points,
                                                                              src,
                                                                              rows,
                ...
                BlockwiseQuantizer<T, 128, qbits, false>::quantizeAndTranspose(dst,
                                                                               scales,
                                                                               zero_points,
                                                                               src,
                                                                               rows,
        ...
        case 256:
            if (columnwise) {
                BlockwiseQuantizer<T, 256, qbits, true>::quantizeAndTranspose(dst,
                                                                              scales,
                                                                              zero_points,
                                                                              src,
                                                                              rows,
                ...
                BlockwiseQuantizer<T, 256, qbits, false>::quantizeAndTranspose(dst,
                                                                               scales,
                                                                               zero_points,
                                                                               src,
                                                                               rows,
...
template void MlasQuantizeBlockwise<float, 4>(uint8_t *dst,
                                              float *scales,
                                              uint8_t *zero_points,
                                              const float *src,
                                              int block_size,
...
template void MlasQuantizeBlockwise<MLAS_FP16, 4>(uint8_t *dst,
                                                  MLAS_FP16 *scales,
                                                  uint8_t *zero_points,
                                                  const MLAS_FP16 *src,
                                                  int block_size,
...
template <typename T, int qbits>
void MlasDequantizeBlockwise(T *dst,
                             const uint8_t *src,
                             const T *scales,
                             const uint8_t *zero_points,
                             int block_size,
...
template void MlasDequantizeBlockwise<float, 4>(float *dst,
                                                const uint8_t *src,
                                                const float *scales,
                                                const uint8_t *zero_points,
                                                int block_size,
...
template <typename Tin, int qbits>
void MlasQDQQuantizeBlockwise(const Tin *src,
                              Tin *scales,
                              uint8_t *zero_points,
                              uint8_t *dst,
                              bool columnwise,
                              ...
                              MLAS_THREADPOOL *thread_pool) {
    if (columnwise) {
        BlockwiseQDQQuantizer<Tin, qbits>::QuantizeColumnWise(src, scales, zero_points, dst, rows, columns, quant_block_size, thread_pool);
    } else {
        BlockwiseQDQQuantizer<Tin, qbits>::QuantizeRowWise(src, scales, zero_points, dst, rows, columns, quant_block_size, thread_pool);
    }
}

template void MlasQDQQuantizeBlockwise<float, 4>(const float *src,
                                                 float *scales,
                                                 uint8_t *zero_points,
                                                 uint8_t *dst,
                                                 bool columnwise,
...
template void MlasQDQQuantizeBlockwise<MLAS_FP16, 4>(const MLAS_FP16 *src,
                                                     MLAS_FP16 *scales,
                                                     uint8_t *zero_points,
                                                     uint8_t *dst,
                                                     bool columnwise,
...
template <typename Tin, int qbits>
void MlasQDQTransposeBlockwiseQuantized(const uint8_t *src_weights,
                                        const Tin *src_scales,
                                        const uint8_t *src_zero_points,
                                        uint8_t *dst_weights,
                                        Tin *dst_scales,
                                        ...
                                        MLAS_THREADPOOL *thread_pool) {
    if (columnwise) {
        BlockwiseQDQQuantizer<Tin, qbits>::TransposeColumnWiseQuantized(src_weights,
                                                                        src_scales,
                                                                        src_zero_points,
                                                                        dst_weights,
                                                                        dst_scales,
        ...
        throw std::runtime_error("Row-wise MlasQDQTransposeBlockwiseQuantized is not implemented");
    }
}

template void MlasQDQTransposeBlockwiseQuantized<float, 4>(const uint8_t *src_weights,
...
template void MlasQDQTransposeBlockwiseQuantized<MLAS_FP16, 4>(const uint8_t *src_weights,
