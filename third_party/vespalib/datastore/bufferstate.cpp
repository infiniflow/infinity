...
size_t roundUpToMatchAllocator(size_t sz) {
    if (sz == 0) {
        return 0;
    }
...
AllocResult calc_allocation(uint32_t bufferId, BufferTypeBase &typeHandler, size_t free_entries_needed, bool resizing) {
...
void BufferState::on_active(uint32_t bufferId,
                            uint32_t typeId,
    ...
    typeHandler->on_active(bufferId, &_stats.used_entries_ref(), &_stats.dead_entries_ref(), buffer.load(std::memory_order::relaxed));
}
...
void BufferState::free_entries(EntryRef ref, size_t num_entries, size_t ref_offset) {
    ...
    type_handler->clean_hold(get_buffer(buffer_underflow_size),
                             ref_offset,
                             num_entries,
                             BufferTypeBase::CleanContext(_stats.extra_used_bytes_ref(), _stats.extra_hold_bytes_ref()));
}

void BufferState::fallback_resize(uint32_t bufferId, size_t free_entries_needed, std::atomic<void *> &buffer, Alloc &holdBuffer) {
    assert(getState() == State::ACTIVE);
    auto type_handler = getTypeHandler();
    assert(type_handler != nullptr);
