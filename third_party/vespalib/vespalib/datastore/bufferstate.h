...
public:
     ...
     */
    void on_active(uint32_t bufferId, uint32_t typeId, BufferTypeBase *typeHandler, size_t free_entries_needed, std::atomic<void *> &buffer);
    ...
    bool isActive(uint32_t typeId) const { return (isActive() && (_typeId == typeId)); }
