...
public:
     ...
     */
    FreeList &getFreeList(uint32_t typeId) { return _free_lists[typeId]; }
    ...
    uint32_t getTypeId(uint32_t bufferId) const { return _buffers[bufferId].getTypeId(); }

    void finishCompact(const std::vector<uint32_t> &toHold);

    vespalib::GenerationHolder &getGenerationHolder() { return _genHolder; }

    // need object location before construction
    static vespalib::GenerationHolder &getGenerationHolderLocation(DataStoreBase &self) { return self._genHolder; }
...
protected:
    ...
    struct EntryRefHoldElem {
        ...
        EntryRefHoldElem(EntryRef ref_in, size_t num_entries_in) : ref(ref_in), num_entries(num_entries_in) {}
    };
...
private:
    ...
    public:
        ...
        FallbackHold(size_t bytesSize, BufferState::Alloc &&buffer, size_t used_entries, BufferTypeBase *typeHandler, uint32_t typeId);
