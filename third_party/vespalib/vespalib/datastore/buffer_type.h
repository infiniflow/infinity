...
public:
    ...
    BufferTypeBase(uint32_t entry_size_in,
...
protected:
    struct BufferCounts {
        ...
        BufferCounts(EntryCount used_entries_in, EntryCount dead_entries_in) : used_entries(used_entries_in), dead_entries(dead_entries_in) {}
    };
...
public:
    ...
    BufferType(uint32_t arraySize, uint32_t min_entries, uint32_t max_entries, uint32_t num_entries_for_new_buffer, float allocGrowFactor) noexcept;
    ~BufferType() override;
