...
void BufferTypeBase::CleanContext::extraBytesCleaned(size_t value) {
...
BufferTypeBase::BufferTypeBase(uint32_t entry_size_in,
    ...
    : _entry_size(entry_size_in), _arraySize(arraySize), _buffer_underflow_size(buffer_underflow_size_in),
      _min_entries(std::min(min_entries, max_entries)), _max_entries(max_entries),
      _num_entries_for_new_buffer(std::min(num_entries_for_new_buffer, max_entries)), _allocGrowFactor(allocGrowFactor), _holdBuffers(0),
      _hold_used_entries(0), _aggr_counts(), _active_buffers() {}
...
EntryCount BufferTypeBase::get_reserved_entries(uint32_t bufferId) const { return bufferId == 0 ? 1u : 0u; }

void BufferTypeBase::on_active(uint32_t bufferId, std::atomic<EntryCount> *used_entries, std::atomic<EntryCount> *dead_entries, void *buffer) {
...
void BufferTypeBase::on_hold(uint32_t buffer_id, const std::atomic<EntryCount> *used_entries, const std::atomic<EntryCount> *dead_entries) {
    ++_holdBuffers;
...
void BufferTypeBase::on_free(EntryCount used_entries) {
    --_holdBuffers;
    assert(_hold_used_entries >= used_entries);
    _hold_used_entries -= used_entries;
}

void BufferTypeBase::resume_primary_buffer(uint32_t buffer_id, std::atomic<EntryCount> *used_entries, std::atomic<EntryCount> *dead_entries) {
    auto itr = std::find(_active_buffers.begin(), _active_buffers.end(), buffer_id);
    assert(itr != _active_buffers.end());
    _active_buffers.erase(itr);
...
void BufferTypeBase::clamp_max_entries(uint32_t max_entries) {
...
size_t BufferTypeBase::calc_entries_to_alloc(uint32_t bufferId, EntryCount free_entries_needed, bool resizing) const {
    ...
    size_t wanted_entries = std::max((resizing ? used_entries : 0u) + grow_entries, static_cast<size_t>(_min_entries));
    ...
    if (result < needed_entries) {
        ...
        s << "BufferTypeBase::calcArraysToAlloc("
          << "bufferId=" << bufferId << ",free_entries_needed=" << free_entries_needed << ",resizing=" << (resizing ? "true" : "false") << ")"
          << " wanted_entries=" << wanted_entries << ", _arraySize=" << _arraySize << ", _max_entries=" << _max_entries
          << ", reserved_entries=" << reserved_entries << ", live_entries=" << live_entries << ", grow_entries=" << grow_entries
...
uint32_t BufferTypeBase::get_scaled_num_entries_for_new_buffer() const {
...
BufferTypeBase::AggregatedBufferCounts::AggregatedBufferCounts() : _counts() {}

void BufferTypeBase::AggregatedBufferCounts::add_buffer(const std::atomic<EntryCount> *used_entries, const std::atomic<EntryCount> *dead_entries) {
...
void BufferTypeBase::AggregatedBufferCounts::remove_buffer(const std::atomic<EntryCount> *used_entries, const std::atomic<EntryCount> *dead_entries) {
    auto itr = std::find_if(_counts.begin(), _counts.end(), [=](const auto &elem) { return elem.used_ptr == used_entries; });
    assert(itr != _counts.end());
    assert(itr->dead_ptr == dead_entries);
    _counts.erase(itr);
}

BufferTypeBase::BufferCounts BufferTypeBase::AggregatedBufferCounts::last_buffer() const {
    BufferCounts result;
...
BufferTypeBase::BufferCounts BufferTypeBase::AggregatedBufferCounts::all_buffers() const {
    BufferCounts result;
    for (const auto &elem : _counts) {
