...
DataStoreBase::FallbackHold::FallbackHold(size_t bytesSize,
                                          ...
                                          uint32_t typeId)
    : GenerationHeldBase(bytesSize), _buffer(std::move(buffer)), _used_entries(used_entries), _typeHandler(typeHandler), _typeId(typeId) {}
...
public:
    BufferHold(size_t bytesSize, DataStoreBase &dsb, uint32_t bufferId) : GenerationHeldBase(bytesSize), _dsb(dsb), _bufferId(bufferId) {
...
DataStoreBase::DataStoreBase(uint32_t numBuffers, uint32_t offset_bits, size_t max_entries)
    : _entry_ref_hold_list(), _buffers(numBuffers), _primary_buffer_ids(), _stash(), _typeHandlers(), _free_lists(), _compaction_count(0u),
      _genHolder(), _max_entries(max_entries), _bufferIdLimit(0u), _hold_buffer_count(0u), _offset_bits(offset_bits), _freeListsEnabled(false),
      _disable_entry_hold_list(false), _initializing(false) {}
...
void DataStoreBase::switch_primary_buffer(uint32_t typeId, size_t entries_needed) {
    ...
    if (buffer_id >= getMaxNumBuffers()) {
        // LOG_ABORT(make_string("switch_primary_buffer(%u, %zu): did not find a free buffer", typeId, entries_needed).c_str());
    }
...
uint32_t DataStoreBase::getFirstFreeBufferId() {
...
vespalib::MemoryUsage DataStoreBase::getDynamicMemoryUsage() const {
...
vespalib::MemoryUsage DataStoreBase::getMemoryUsage() const {
...
void DataStoreBase::disableFreeLists() {
...
MemoryStats DataStoreBase::getMemStats() const {
    MemoryStats stats;
...
vespalib::AddressSpace DataStoreBase::getAddressSpaceUsage() const {
    uint32_t buffer_id_limit = get_bufferid_limit_acquire();
...
void DataStoreBase::inc_hold_buffer_count() {
