...
public:
    static void reclaim(BTreeNode *node) { node->unFreeze(); }
};
...
public:
    BTreeNodeBufferType(uint32_t min_entries, uint32_t max_entries) : ParentType(1, min_entries, max_entries) {}
...
public:
    ...
    enum NodeTypes { NODETYPE_INTERNAL = 0, NODETYPE_LEAF = 1 };
...
public:
    ...
    LeafNodeTypeRefPair allocNewLeafNode() { return _store.allocator<LeafNodeType>(NODETYPE_LEAF).alloc(); }

    LeafNodeTypeRefPair allocLeafNode() { return _store.freeListAllocator<LeafNodeType, BTreeNodeReclaimer>(NODETYPE_LEAF).alloc(); }

    LeafNodeTypeRefPair allocNewLeafNodeCopy(const LeafNodeType &rhs) { return _store.allocator<LeafNodeType>(NODETYPE_LEAF).alloc(rhs); }
    ...
    InternalNodeTypeRefPair allocNewInternalNode() { return _store.allocator<InternalNodeType>(NODETYPE_INTERNAL).alloc(); }

    InternalNodeTypeRefPair allocInternalNode() { return _store.freeListAllocator<InternalNodeType, BTreeNodeReclaimer>(NODETYPE_INTERNAL).alloc(); }
    ...
    void hold_entry(EntryRef ref) { _store.hold_entry(ref); }

    void assign_generation(generation_t current_gen) { _store.assign_generation(current_gen); }

    // Inherit doc from DataStoreBase
    datastore::MemoryStats getMemStats() const { return _store.getMemStats(); }
    ...
    bool getCompacting(EntryRef ref) { return _store.getCompacting(ref); }

    bool has_held_buffers() const { return _store.has_held_buffers(); }
...
extern template class BTreeNodeStore<uint32_t, uint32_t, NoAggregated, BTreeDefaultTraits::INTERNAL_SLOTS, BTreeDefaultTraits::LEAF_SLOTS>;
extern template class BTreeNodeStore<uint32_t, BTreeNoLeafData, NoAggregated, BTreeDefaultTraits::INTERNAL_SLOTS, BTreeDefaultTraits::LEAF_SLOTS>;
extern template class BTreeNodeStore<uint32_t, int32_t, MinMaxAggregated, BTreeDefaultTraits::INTERNAL_SLOTS, BTreeDefaultTraits::LEAF_SLOTS>;
