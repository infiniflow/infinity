...
#include <type_traits>
#include <utility>
#include <vespalib/datastore/atomic_entry_ref.h>
...
protected:
    ...
    BTreeNode(const BTreeNode &rhs) noexcept : _level(rhs._level), _isFrozen(rhs._isFrozen), _validSlots(rhs._validSlots) {}
...
class BTreeNodeTT : public BTreeNodeT<KeyT, NumSlots>, public BTreeNodeDataWrap<DataT, NumSlots>, public BTreeNodeAggregatedWrap<AggrT> {
public:
...
protected:
    ...
    BTreeNodeTT(const BTreeNodeTT &rhs) noexcept : ParentType(rhs), DataWrapType(rhs), AggrWrapType(rhs) { copyData(rhs, _validSlots); }
...
public:
    ...
    void update(uint32_t idx, const KeyT &key, BTreeNode::Ref child) { update(idx, key, BTreeNode::ChildRef(child)); }
    void insert(uint32_t idx, const KeyT &key, BTreeNode::Ref child) { insert(idx, key, BTreeNode::ChildRef(child)); }
    ...
    template <typename NodeAllocatorType>
    void splitInsert(BTreeInternalNode *splitNode, uint32_t idx, const KeyT &key, const BTreeNode::Ref &data, NodeAllocatorType &allocator);
...
public:
    ...
    BTreeLeafNodeTemp(const KeyDataType *smallArray, uint32_t arraySize) noexcept : ParentType(smallArray, arraySize) {}
