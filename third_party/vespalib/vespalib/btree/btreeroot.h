...
template <typename KeyT, typename DataT, typename AggrT = NoAggregated, typename CompareT = std::less<KeyT>, typename TraitsT = BTreeDefaultTraits>
...
public:
    ...
    public:
        ...
        FrozenView(BTreeNode::Ref frozenRoot, const NodeAllocatorType &allocator) : _frozenRoot(frozenRoot), _allocator(&allocator) {}
...
private:
    static Iterator findHelper(BTreeNode::Ref root, const KeyType &key, const NodeAllocatorType &allocator, CompareT comp = CompareT());

    static Iterator lowerBoundHelper(BTreeNode::Ref root, const KeyType &key, const NodeAllocatorType &allocator, CompareT comp = CompareT());

    static Iterator upperBoundHelper(BTreeNode::Ref root, const KeyType &key, const NodeAllocatorType &allocator, CompareT comp = CompareT());
...
public:
    ...
    FrozenView getFrozenView(const NodeAllocatorType &allocator) const { return FrozenView(getFrozenRoot(), allocator); }
...
protected:
    bool
    isValid(BTreeNode::Ref node, bool ignoreMinSlots, uint32_t level, const NodeAllocatorType &allocator, CompareT comp, AggrCalcT aggrCalc) const;
...
public:
    ...
    bool remove(const KeyType &key, NodeAllocatorType &allocator, CompareT comp = CompareT(), const AggrCalcT &aggrCalc = AggrCalcT());
...
extern template class BTreeRoot<uint32_t, int32_t, MinMaxAggregated, std::less<uint32_t>, BTreeDefaultTraits, MinMaxAggrCalc>;
