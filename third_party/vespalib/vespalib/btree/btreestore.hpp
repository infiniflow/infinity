...
#include <vespalib/util/optimized.h>
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
typename BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::KeyDataTypeRefPair
BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::allocNewKeyDataCopy(const KeyDataType *rhs, uint32_t clusterSize) {
    assert(clusterSize >= 1 && clusterSize <= clusterLimit);
    uint32_t typeId = clusterSize - 1;
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
typename BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::KeyDataTypeRefPair
BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::allocKeyDataCopy(const KeyDataType *rhs, uint32_t clusterSize) {
    assert(clusterSize >= 1 && clusterSize <= clusterLimit);
    uint32_t typeId = clusterSize - 1;
    return _store.freeListAllocator<KeyDataType, datastore::DefaultReclaimer<KeyDataType>>(typeId).allocArray(
        vespalib::ConstArrayRef<KeyDataType>(rhs, clusterSize));
}

template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
const typename BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::KeyDataType *
BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::lower_bound(const KeyDataType *b,
                                                                          const KeyDataType *e,
                                                                          const KeyType &key,
                                                                          CompareT comp) {
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::makeTree(EntryRef &ref, const KeyDataType *array, uint32_t clusterSize) {
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::makeArray(EntryRef &ref, EntryRef root, LeafNodeType *leafNode) {
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
bool BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::insert(EntryRef &ref, const KeyType &key, const DataType &data, CompareT comp) {
#ifdef FORCE_APPLY
    bool retVal = true;
    if (ref.valid()) {
        RefType iRef(ref);
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
bool BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::remove(EntryRef &ref, const KeyType &key, CompareT comp) {
#ifdef FORCE_APPLY
    bool retVal = true;
    if (!ref.valid())
    ...
    apply(ref, &additions[0], &additions[additions.size()], &removals[0], &removals[removals.size()], comp);
    return retVal;
#else
    if (!ref.valid())
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
uint32_t BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::getNewClusterSize(const KeyDataType *o,
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyModifyTree(BTreeType *tree,
                                                                                   AddIter a,
                                                                                   AddIter ae,
                                                                                   RemoveIter r,
                                                                                   RemoveIter re,
    ...
    itr.lower_bound(tree->getRoot(), (a != ae && r != re) ? (comp(a->_key, *r) ? a->_key : *r) : ((a != ae) ? a->_key : *r), comp);
    while (a != ae || r != re) {
        if (r != re && (a == ae || comp(*r, a->_key))) {
            // remove
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyBuildTree(BTreeType *tree,
                                                                                  AddIter a,
                                                                                  AddIter ae,
                                                                                  RemoveIter r,
                                                                                  RemoveIter re,
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyNewArray(EntryRef &ref, AddIter aOrg, AddIter ae) {
    assert(!ref.valid());
    if (aOrg == ae) {
        // No new data
        return;
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyNewTree(EntryRef &ref, AddIter a, AddIter ae, CompareT comp) {
    assert(!ref.valid());
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyNew(EntryRef &ref, AddIter a, AddIter ae, CompareT comp) {
    // No old data
    assert(!ref.valid());
    size_t additionSize(ae - a);
    uint32_t clusterSize = additionSize;
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
bool BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyCluster(EntryRef &ref,
    ...
    uint32_t newSizeMin = std::max(clusterSize, static_cast<uint32_t>(additionSize)) - std::min(clusterSize, static_cast<uint32_t>(removeSize));
    RefType iRef(ref);
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::applyTree(BTreeType *tree,
                                                                             AddIter a,
                                                                             AddIter ae,
                                                                             RemoveIter r,
                                                                             RemoveIter re,
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::normalizeTree(EntryRef &ref, BTreeType *tree, bool wasArray) {
    EntryRef root = tree->getRoot();
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::apply(EntryRef &ref,
                                                                         AddIter a,
                                                                         AddIter ae,
                                                                         RemoveIter r,
                                                                         RemoveIter re,
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::beginFrozen(const EntryRef ref, std::vector<ConstIterator> &where) const {
    if (!ref.valid()) {
        where.emplace_back();
        return;
    }
...
template <typename KeyT, typename DataT, typename AggrT, typename CompareT, typename TraitsT, typename AggrCalcT>
void BTreeStore<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::move_btree_nodes(const std::vector<EntryRef> &refs) {
    for (auto &ref : refs) {
        RefType iRef(ref);
        assert(iRef.valid());
