...
bool field_data::invert(token_stream& stream, doc_id_t id) {
  ...
  if (!inc) {
    IRS_LOG_ERROR(rst::StrCat({"field '", meta_.name,
  ...
  while (stream.next()) {
    ...
    if (pos_ < last_pos_) {
      IRS_LOG_ERROR(rst::StrCat({"invalid position ", pos_, " < ", last_pos_,
                                 " in field '", meta_.name, "'"}));
      return false;
    }

    if (pos_ >= pos_limits::eof()) {
      IRS_LOG_ERROR(rst::StrCat({"invalid position ", pos_,
    ...
    if (offs) {
      ...
      if (start_offset < last_start_offs_ || end_offset < start_offset) {
        IRS_LOG_ERROR(rst::StrCat({"invalid offset start=", start_offset,
    ...
    if (p == nullptr) {
      IRS_LOG_WARN(rst::StrCat({"skipping too long term of size: ",
                                term->value.size(), " in field: ", meta_.name}));
      IRS_LOG_TRACE(
        rst::StrCat({"field: ", meta_.name,
    ...
    if (0 == ++stats_.len) {
      IRS_LOG_ERROR(rst::StrCat({"too many tokens in field: ", meta_.name,
                                 ", document: ", id}));
      return false;
    }
