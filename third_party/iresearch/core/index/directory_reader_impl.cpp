...
index_file_refs::ref_t LoadNewestIndexMeta(IndexMeta& meta,
  ...
  if (codec) {
    try {
      ...
      IRS_LOG_ERROR(
        rst::StrCat({"Caught exception while reading index meta with codec ''",
...
std::shared_ptr<const DirectoryReaderImpl> DirectoryReaderImpl::Open(
  ...
  phmap::flat_hash_map<std::string_view, size_t> reuse_candidates;
  ...
  for (const auto& [filename, meta] : segments) {
    ...
    if (!*reader) {
      throw index_error{rst::StrCat({"While opening reader for segment '",
                                     meta.name,
