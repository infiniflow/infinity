...
std::optional<Norm2Header> Norm2Header::Read(bytes_view payload) noexcept {
  if (IRS_UNLIKELY(payload.size() != ByteSize())) {
    IRS_LOG_ERROR(rst::StrCat({"Invalid 'norm2' header size ", payload.size()}));
    return std::nullopt;
  }
  ...
  if (const byte_type ver = *p++;
    ...
    IRS_LOG_ERROR(rst::StrCat({"'norm2' header version mismatch, expected: ",
  ...
  if (IRS_UNLIKELY(!CheckNumBytes(num_bytes))) {
    IRS_LOG_ERROR(
      rst::StrCat({"Malformed 'norm2' header, invalid number of bytes: ",
