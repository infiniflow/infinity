...
void read_compact(irs::index_input& in, irs::encryption::stream* cipher,
  ...
  if (IRS_UNLIKELY(!decompressor)) {
    throw irs::index_error{
      rst::StrCat({"While reading compact, error: can't decompress "
                   "block of size ",
...
bool meta_reader::prepare(const directory& dir, const SegmentMeta& meta,
  ...
  if (!dir.exists(exists, filename)) {
    throw io_error{
      rst::StrCat({"Failed to check existence of file, path: ", filename})};
  }
  ...
  if (length < kFooterLength) {
    throw index_error{rst::StrCat({"Invalid column id: ", max_id,
                                   " footer length, "
                                   "got ",
  ...
  if (max_id >= std::numeric_limits<size_t>::max()) {
    throw index_error{
      rst::StrCat({"Invalid max column id: ", max_id, ", path: ", filename})};
  }
...
bool writer::commit(const flush_state& state) {
  ...
  if (columns_.empty()) {
    ...
    if (!dir_->remove(filename_)) {  // Ignore error.
      IRS_LOG_ERROR(rst::StrCat({"Failed to remove file, path: ", filename_}));
    }
 ...
 public:
  ...
  void load(index_input& in, compression::decompressor* decomp,
    ...
    if (!encode::avg::read_block_rl32(in, base_, avg) || 1 != avg) {
      throw index_error{
        rst::StrCat({"Invalid RL encoding in 'dense_block', base_key=", base_,
                     " avg_delta=", avg})};
    }
 ...
 public:
  ...
  void load(index_input& in, compression::decompressor* /*decomp*/,
    ...
    if (!encode::avg::read_block_rl32(in, min_, avg) || 1 != avg) {
      throw index_error{
        rst::StrCat({"Invalid RL encoding in 'dense_mask_block', "
                     "base_key=",
                     min_, " avg_delta=", avg})};
    }
...
bool reader::read_meta(const directory& dir, const SegmentMeta& meta,
  ...
  for (column_meta col_meta; reader.read(col_meta);) {
    if (col_meta.id >= columns.size()) {
      throw index_error{rst::StrCat({"Invalid column '", col_meta.name,
...
bool reader::prepare(const directory& dir, const SegmentMeta& meta,
  ...
  if (count >= field_limits::invalid()) {
    throw index_error{
      rst::StrCat({"Too many columns in the columnstore (", count, ")"})};
  }
  ...
  for (field_id i = 0, size = static_cast<field_id>(count); i < size; ++i) {
    ...
    if (!factory) {
      ...
      throw index_error{
        rst::StrCat({"Failed to open column id=", static_cast<uint32_t>(props),
                     ", properties=", i})};
    }
    ...
    if (!column) {
      throw index_error{rst::StrCat({"Factory failed to create column id=", i})};
    }
    ...
    if (version > writer::FORMAT_MIN) {
      ...
      if (!decomp && !compression::exists(compression_id)) {
        throw index_error{rst::StrCat({"Failed to load compression '",
                                       compression_id, "' for column id=", i})};
      }

      if (decomp && !decomp->prepare(*stream)) {
        throw index_error{rst::StrCat({"Failed to prepare compression '",
                                       compression_id, "' for column id=", i})};
      }
    } else {
      // we don't support encryption and custom
    ...
    try {
      ...
      IRS_LOG_ERROR(rst::StrCat({"Failed to load column id=", i}));
