...
int32_t check_header(index_input& in, std::string_view req_format,
  ...
  if (static_cast<size_t>(left) < expected) {
    throw index_error{rst::StrCat({"While checking header, error: only '", left,
  ...
  if (req_format != format) {
    throw index_error{
      rst::StrCat({"While checking header, error: format mismatch '", format,
                   "' != '", req_format, "'"})};
  }
...
int64_t checksum(const index_input& in) {
  ...
  if (length < sizeof(uint64_t)) {
    throw index_error{
      rst::StrCat({"failed to read checksum from a file of size ", length})};
  }
