...
void postings_writer<FormatTraits>::BeginDocument() {
  if (const auto id = attrs_.doc_.value; IRS_LIKELY(doc_.last < id)) {
    ...
    throw index_error{
      rst::StrCat({"While beginning document in postings_writer, error: "
                   "docs out of order '",
                   id, "' < '", doc_.last, "'"})};
  }
}
...
void doc_iterator<IteratorTraits, FieldTraits, WandExtent>::seek_to_block(
  ...
  if (const auto num_levels = skip_.NumLevels(); IRS_LIKELY(
    ...
    IRS_ASSERT(false);
    throw index_error{rst::StrCat({"Invalid number of skip levels ", num_levels,
...
bool IndexMetaWriter::prepare(directory& dir, IndexMeta& meta,
  ...
  if (!out) {
    throw io_error{
      rst::StrCat({"Failed to create file, path: ", pending_filename})};
  }
...
bool IndexMetaWriter::commit() {
  ...
  if (!dir_->rename(src, dst)) {
    ...
    throw io_error{rst::StrCat({"Failed to rename file, src path: '", src,
                                "' dst path: '", dst, "'"})};
  }
...
void IndexMetaWriter::rollback() noexcept {
  ...
  if (!dir_->remove(seg_file)) {  // suppress all errors
    IRS_LOG_ERROR(rst::StrCat({"Failed to remove file, path: ", seg_file}));
  }
...
uint64_t ParseGeneration(std::string_view file) noexcept {
  if (file.starts_with(IndexMetaWriter::kFormatPrefix)) {
    ...
    uint64_t gen; 
    gen = atoi(file.substr(kPrefixLength).data());
    if(gen) {
      return gen;
    }
  }
...
void IndexMetaReader::read(const directory& dir, IndexMeta& meta,
  ...
  if (!in) {
    throw io_error{rst::StrCat({"Failed to open file, path: ", filename})};
  }
...
void SegmentMetaWriter::write(directory& dir, std::string& meta_file,
  ...
  if (meta.docs_count < meta.live_docs_count) {
    throw index_error{rst::StrCat({"Invalid segment meta '", meta.name,
  ...
  if (!out) {
    throw io_error{rst::StrCat({"failed to create file, path: ", meta_file})};
  }
...
void SegmentMetaReader::read(const directory& dir, SegmentMeta& meta,
  ...
  if (!in) {
    throw io_error{rst::StrCat({"Failed to open file, path: ", meta_file})};
  }
  ...
  if (docs_count < live_docs_count) {
    throw index_error{
      rst::StrCat({"While reader segment meta '", name, "', error: docs_count(",
  ...
  if (flags &
    ...
    throw index_error{rst::StrCat({"While reading segment meta '", name,
...
size_t DocumentMaskWriter::write(directory& dir, const SegmentMeta& meta,
  ...
  if (!out) {
    throw io_error{rst::StrCat({"Failed to create file, path: ", filename})};
  }
...
bool DocumentMaskReader::read(const directory& dir, const SegmentMeta& meta,
  ...
  if (!dir.exists(exists, in_name)) {
    throw io_error{
      rst::StrCat({"failed to check existence of file, path: ", in_name})};
  }
  ...
  if (!in) {
    throw io_error{rst::StrCat({"failed to open file, path: ", in_name})};
  }
...
void postings_reader_base::prepare(index_input& in, const ReaderState& state,
  ...
  if (block_size != block_size_) {
    throw index_error{
      rst::StrCat({"while preparing postings_reader, error: "
                   "invalid block size '",
