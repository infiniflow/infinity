...
class column_base : public column_reader, private util::noncopyable {
 ...
 protected:
  ...
  bool allocate_buffered_memory(size_t size, size_t mappings) noexcept try {
    ...
    IRS_LOG_WARN(
      rst::StrCat({"Failed to allocate memory for buffered column id ",
...
bool writer::commit(const flush_state& /*state*/) {
  ...
  if (columns_.empty()) {
    ...
    if (!dir_->remove(data_filename_)) {  // ignore error
      IRS_LOG_ERROR(
        rst::StrCat({"Failed to remove file, path: ", data_filename_}));
    }
  ...
  if (!index_out) {
    throw io_error{
      rst::StrCat({"Failed to create file, path: ", index_filename})};
  }
...
void reader::prepare_index(const directory& dir, const SegmentMeta& meta,
  ...
  for (field_id i = 0; i < count; ++i) {
    ...
    if (encrypted && !data_cipher_) {
      throw index_error{rst::StrCat({"Failed to load encrypted column id=", i,
                                     " without a cipher"})};
    }

    if (ColumnType::kMask != hdr.type && 0 == hdr.docs_count) {
      throw index_error{rst::StrCat({"Failed to load column id=", i,
    ...
    if (const size_t idx = static_cast<size_t>(hdr.type);
      ...
      if (!sorted_columns.empty() &&
        ...
        throw index_error{
          rst::StrCat({"Invalid column order in segment '", meta.name, "'"})};
      }
      ...
      throw index_error{
        rst::StrCat({"Failed to load column id=", i,
  ...
  if (opts.warmup_column) {
    ...
    for (size_t i = 0; i < sorted_columns.size(); ++i) {
      ...
      if (opts.warmup_column(*cb)) {
        ...
        IRS_LOG_TRACE(rst::StrCat({"Making buffered: ", cb->header().id}));
        cb->make_buffered(*direct_data_input,
                          std::span(sorted_columns.data() + i + 1,
                                    sorted_columns.size() - i - 1));
        IRS_LOG_TRACE(rst::StrCat({"Finished buffered: ", cb->header().id}));
      }
    }
  }
...
bool reader::prepare(const directory& dir, const SegmentMeta& meta,
  ...
  if (!dir.exists(exists, data_filename)) {
    throw io_error{
      rst::StrCat({"Failed to check existence of file, path: ", data_filename})};
  }
  ...
  if (!dir.exists(exists, index_filename)) {
    throw io_error{rst::StrCat({"Failed to check existence of file, path: ",
                                index_filename})};
  }
  ...
  if (!exists) {
    ...
    throw index_error{
      rst::StrCat({"Columnstore index file '", index_filename, "' is missing"})};
  }
