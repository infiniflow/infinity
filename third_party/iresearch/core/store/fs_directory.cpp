...
inline int get_posix_fadvice(IOAdvice advice) noexcept {
  ...
  IRS_LOG_ERROR(
    rst::StrCat({"fadvice '", static_cast<uint32_t>(advice),
 ...
 protected:
  ...
  void flush_buffer(const byte_type* b, size_t len) final {
    ...
    if (len && len_written != len) {
      throw io_error{rst::StrCat({"Failed to write buffer, written '",
...
class fs_index_input : public buffered_index_input {
 public:
  ...
  static index_input::ptr open(const path_char_t* name, size_t pool_size,
    ...
    if (nullptr == handle->handle) {
      IRS_LOG_ERROR(
        rst::StrCat({"Failed to open input file, error: ", GET_ERROR(),
                     ", path: ", file_utils::ToStr(name)}));
    ...
    if (!file_utils::byte_size(size, *handle)) {
      IRS_LOG_ERROR(
        rst::StrCat({"Failed to get stat for input file, error: ", GET_ERROR(),
                     ", path: ", file_utils::ToStr(name)}));
 ...
 protected:
  void seek_internal(size_t pos) final {
    if (pos > handle_->size) {
      throw io_error{rst::StrCat({"seek out of range for input file, length '",
  ...
  size_t read_internal(byte_type* b, size_t len) final {
    ...
    if (handle_->pos != pos_) {
      if (irs::file_utils::fseek(fd, static_cast<long>(pos_), SEEK_SET) != 0) {
        throw io_error{rst::StrCat({"failed to seek to '", pos_,
...
fs_index_input::file_handle::ptr pooled_fs_index_input::reopen(
  ...
  if (!handle->handle) {
    ...
    if (!handle->handle) {
      ...
      throw io_error{
        rst::StrCat({"Failed to reopen input file, error: ", GET_ERROR()})};
    }
    descriptors = 0;
...
index_output::ptr FSDirectory::create(std::string_view name) noexcept {
  try {
    ...
    if (!out) {
      IRS_LOG_ERROR(rst::StrCat({"Failed to open output file, path: ", name}));
    }
...
bool FSDirectory::sync(std::string_view name) noexcept {
  try {
    ...
    IRS_LOG_ERROR(rst::StrCat({"Failed to sync file, error: ", GET_ERROR(),
                               ", path: ", path.string()}));
  } catch (...) {
    IRS_LOG_ERROR(rst::StrCat({"Failed to sync file, name: ", name}));
  }
