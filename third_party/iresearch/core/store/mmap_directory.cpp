...
inline int GetPosixMadvice(IOAdvice advice) {
  ...
  IRS_LOG_ERROR(
    rst::StrCat({"madvice '", static_cast<uint32_t>(advice),
...
std::shared_ptr<mmap_handle> OpenHandle(const path_char_t* file,
  ...
  if (!handle->open(file)) {
    IRS_LOG_ERROR(rst::StrCat({"Failed to open mmapped input file, path: ",
  ...
  if (IR_MADVICE_NORMAL != padvice && !handle->advise(padvice)) {
    IRS_LOG_ERROR(rst::StrCat({"Failed to madvise input file, path: ",
