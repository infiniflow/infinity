...
bool parse_vpack_options(const VPackSlice slice, T& options) {
  ...
  if (auto pipeline_slice = slice.get(PIPELINE_PARAM_NAME);
    ...
    if (pipeline_slice.isArray()) {
        ...
        if (pipe.isObject()) {
          ...
          if (auto type_attr_slice = pipe.get(TYPE_PARAM_NAME);
            ...
            if (type_attr_slice.isString()) {
              ...
              IRS_LOG_ERROR(
                rst::StrCat({"Failed to read '", TYPE_PARAM_NAME,
            ...
            IRS_LOG_ERROR(
              rst::StrCat({"Failed to get '", TYPE_PARAM_NAME,
                           "' attribute of '", PIPELINE_PARAM_NAME,
          ...
          if (auto properties_attr_slice = pipe.get(PROPERTIES_PARAM_NAME);
            ...
            if constexpr (std::is_same_v<
              ...
              if (analyzer) {
                ...
                IRS_LOG_ERROR(
                  rst::StrCat({"Failed to create pipeline member of type '",
              ...
              if (irs::analysis::analyzers::normalize(
                ...
                IRS_LOG_ERROR(
                  rst::StrCat({"Failed to normalize pipeline member of type '",
                               type, "' with properties '",
                               irs::slice_to_string(properties_attr_slice),
                               "' while constructing pipeline_token_stream "
                               "from VPack arguments"}));
            ...
            IRS_LOG_ERROR(
              rst::StrCat({"Failed to get '", PROPERTIES_PARAM_NAME,
                           "' attribute of '", PIPELINE_PARAM_NAME,
                           "' member while constructing pipeline_token_stream "
                           "from VPack arguments"}));
            return false;
          ...
          IRS_LOG_ERROR(
            rst::StrCat({"Failed to read '", PIPELINE_PARAM_NAME,
      ...
      IRS_LOG_ERROR(rst::StrCat({"Failed to read '", PIPELINE_PARAM_NAME,
                                 "' attribute as array while constructing "
                                 "pipeline_token_stream from VPack arguments"}));
      return false;
    }
  } else {
    IRS_LOG_ERROR(rst::StrCat({"Not found parameter '", PIPELINE_PARAM_NAME,
                               "' while constructing pipeline_token_stream"}));
    return false;
  }
  if (options.empty()) {
