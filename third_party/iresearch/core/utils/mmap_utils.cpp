...
bool mmap_handle::open(const path_char_t* path) noexcept try {
  ...
  if (fd < 0) {
    IRS_LOG_ERROR(rst::StrCat({"Failed to open input file, error: ", errno,
  ...
  if (!file_utils::byte_size(size, fd)) {
    IRS_LOG_ERROR(rst::StrCat({"Failed to get stats for input file, error: ",
  ...
  if (size) {
    ...
    if (MAP_FAILED == addr) {
      IRS_LOG_ERROR(rst::StrCat({"Failed to mmap input file, error: ", errno,
                                 ", path: ", file_utils::ToStr(path)}));
      close();
      return false;
    }
