...
void* load_library(const char* soname, int mode /* = 2 */) {
  ...
  if (!handle) {
    IRS_LOG_ERROR(rst::StrCat({"load failed of shared object: ", name.string(),
                               " error: ", dlerror()}));
  }
...
void load_libraries(std::string_view path, std::string_view prefix,
  ...
  if (!file_utils::exists_directory(result, plugin_path.c_str()) || !result) {
    IRS_LOG_INFO(rst::StrCat({"library load failed, not a plugin path: ",
                              plugin_path.string()}));
  ...
  auto visitor = [&plugin_path, &prefix, &suffix](auto name) -> bool {
    ...
    if (!file_utils::exists_file(result, path.c_str())) {
      IRS_LOG_ERROR(
        rst::StrCat({"Failed to identify plugin file: ", path.string()}));
    ...
    if (!handle) {
      IRS_LOG_ERROR(
        rst::StrCat({"library load failed for path: ", path_stem.string()}));
    }
