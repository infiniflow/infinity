...
bool verify_lock_file(const path_char_t* file) {
  ...
  if (!is_same_hostname(buf, len)) {
    IRS_LOG_INFO(rst::StrCat({"Index locked by another host, hostname: '",
  ...
  if (is_valid_pid(pid)) {
    IRS_LOG_INFO(rst::StrCat({"Index locked by another process, PID: '", pid,
                              "', file: '", ToStr(file), "'"}));
    return true;  // locked
  }
...
lock_handle_t create_lock_file(const path_char_t* file) {
  ...
  if (INVALID_HANDLE_VALUE == fd) {
    IRS_LOG_ERROR(rst::StrCat({"Unable to create lock file: '", ToStr(file),
                               "', error: ", GET_ERROR()}));
    return nullptr;
  }
  ...
  if (const int err = get_host_name(buf, sizeof buf - 1)) {
    IRS_LOG_ERROR(rst::StrCat({"Unable to get hostname, error: ", err}));
    return nullptr;
  }

  if (!file_utils::write(fd, buf, strlen(buf) + 1)) {  // include terminate 0
    IRS_LOG_ERROR(rst::StrCat({"Unable to write lock file: '", ToStr(file),
                               "', error: ", GET_ERROR()}));
    return nullptr;
  }
...
#pragma warning(default : 4996)
  ...
  if (::FlushFileBuffers(fd) <= 0) {
    IRS_LOG_ERROR(rst::StrCat({"Unable to flush lock file: '", ToStr(file),
                               "', error: ", GET_ERROR()}));
    return nullptr;
  }
...
bool verify_lock_file(const path_char_t* file) {
  ...
  if (fd < 0) {
    IRS_LOG_ERROR(rst::StrCat({"Unable to open lock file '", file,
  ...
  // read file content
  {
    ...
    if (flock(fd, LOCK_EX | LOCK_NB)) {
      if (EWOULDBLOCK == errno) {
        IRS_LOG_ERROR(rst::StrCat({"Lock file '", file, "' is already locked"}));
        return true;  // locked
      } else {
        IRS_LOG_ERROR(rst::StrCat({"Unable to apply lock on lock file: '", file,
                                   "', error: ", errno}));
        return false;  // not locked
      }
    }
...
lock_handle_t create_lock_file(const path_char_t* file) {
  ...
  if (fd < 0) {
    IRS_LOG_ERROR(
      rst::StrCat({"Unable to create lock file: '", file, "', error: ", errno}));
    return nullptr;
  }
  ...
  if (!file_utils::write(reinterpret_cast<void*>(fd), buf,
    ...
    IRS_LOG_ERROR(
      rst::StrCat({"Unable to write lock file: '", file, "', error: ", errno}));
    return nullptr;
  }
...
bool exists(bool& result, const path_char_t* file) noexcept {
  ...
  if (!result && ENOENT != errno) {
    IRS_LOG_ERROR(rst::StrCat({"Failed to get stat, error ", errno,
                               " path: ", ToStr(file)}));
  }
...
handle_t open(const path_char_t* path, OpenMode mode, int advice) noexcept {
  ...
  switch (read_write) {
    ...
    case OpenMode::Write:
      ...
      break;
    default:
      IRS_LOG_ERROR(rst::StrCat({"Invalid OpenMode ", static_cast<int>(mode),
  ...
  if (fd < 0) {
    IRS_LOG_ERROR(
      rst::StrCat({"Failed to open file, error: ", errno, ", path: ", path}));
    return handle_t(nullptr);
  }
#ifdef __APPLE__
  // Implement Alignment and File Access Requirements
...
handle_t open(void* file, OpenMode mode, int advice) noexcept {
  ...
  IRS_LOG_WARN(rst::StrCat({"Required file path buffer size of ", length + 1,
  ...
  IRS_LOG_ERROR(
    rst::StrCat({"Failed to get filename from file handle, inconsistent length "
                 "detected, first ",
  ...
  if (0 > fd || 0 > fcntl(fd, F_GETPATH, path)) {
    IRS_LOG_ERROR(
      rst::StrCat({"Failed to get file path from file handle, error ", errno}));
    return nullptr;
  }
...
bool mkdir(const path_char_t* path, bool createNew) noexcept {
  ...
  if (!abs) {
    if (0 == ::CreateDirectoryW(path, nullptr)) {
      if (GET_ERROR() != ERROR_ALREADY_EXISTS || createNew) {
        ...
        IRS_LOG_ERROR(rst::StrCat({"Failed to create relative path: '",
  ...
  if (0 == ::CreateDirectoryW(dirname.c_str(), nullptr)) {
    if (GET_ERROR() != ERROR_ALREADY_EXISTS || createNew) {
      ...
      IRS_LOG_ERROR(rst::StrCat({"Failed to create absolute path: '",
                                 ToStr(path), "', error ", GET_ERROR()}));
  ...
  if (0 != ::mkdir(path, S_IRWXU | S_IRWXG | S_IRWXO)) {
    if (errno != EEXIST || createNew) {
      ...
      // perform creation
      IRS_LOG_ERROR(
        rst::StrCat({"Failed to create path: '", path, "', error ", errno}));
      return false;
    }
  }
#endif
...
bool remove(const path_char_t* path) noexcept {
  ...
  if (!abs) {
    ...
    if (!res) {  // 0 == error
      ...
      if (ERROR_FILE_NOT_FOUND == system_error) {
        ...
        IRS_LOG_DEBUG(rst::StrCat({"Failed to remove path: '", ToStr(path),
                                   "', error ", system_error}));
      } else {
        IRS_LOG_ERROR(rst::StrCat({"Failed to remove path: '", ToStr(path),
                                   "', error ", system_error}));
      }
      return false;
    }
  ...
  if (res) {                // non-0 == error
    if (ENOENT == errno) {  // file is just not here, so we are done actually
      IRS_LOG_DEBUG(
        rst::StrCat({"Failed to remove path: '", path, "', error ", errno}));
    } else {
      IRS_LOG_ERROR(
        rst::StrCat({"Failed to remove path: '", path, "', error ", errno}));
    }
