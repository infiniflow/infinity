...
class generic_register : public singleton<RegisterType> {
 ...
 protected:
  ...
  virtual entry_type load_entry_from_so(const key_type& key) const {
    ...
    if (!this_ptr->add_so_handle(handle)) {
      IRS_LOG_ERROR(rst::StrCat({"init failed in shared object : ", file}));
      return entry_type();
    }
    ...
    const entry_type* entry = lookup(key);
    if (nullptr == entry) {
      IRS_LOG_ERROR(rst::StrCat({"lookup failed in shared object : ", file}));
      return entry_type();
    }
 ...
 private:
  using register_map_t =
    phmap::flat_hash_map<key_type, entry_type, hash_type, pred_type>;
...
template<typename KeyType, typename EntryType, typename TagType,
         typename RegisterType, typename Hash = std::hash<KeyType>,
         typename Pred = std::equal_to<KeyType>>
class tagged_generic_register
 ...
 private:
  using tag_map_t =
    phmap::flat_hash_map<key_type, tag_type, hash_type, pred_type>;
