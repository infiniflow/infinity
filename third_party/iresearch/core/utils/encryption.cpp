...
bool decrypt(std::string_view filename, index_input& in, encryption* enc,
  ...
  if (header.size() != enc->header_length()) {
    throw index_error{
      rst::StrCat({"failed to open encrypted file, expect encryption header of "
                   "size ",
  ...
  if (!cipher) {
    throw index_error{
      rst::StrCat({"Failed to open encrypted file, path '", filename, "'"})};
  }
  ...
  if (!block_size) {
    throw index_error{
      rst::StrCat({"Invalid block size 0 specified for encrypted file, path '",
                   filename, "'"})};
  }
  ...
  if (crc.checksum() != in.read_vlong()) {
    throw index_error{
      rst::StrCat({"Invalid ecryption header, path '", filename, "'"})};
  }
...
void encrypted_output::flush() {
  ...
  if (!cipher_->encrypt(out_->file_pointer(), buf_.get(), size)) {
    throw io_error{rst::StrCat({"Buffer size ", size,
...
index_input::ptr encrypted_input::dup() const {
  ...
  if (!dup) {
    throw io_error{
      rst::StrCat({"Failed to duplicate input file, error: ", errno})};
  }
...
index_input::ptr encrypted_input::reopen() const {
  ...
  if (!reopened) {
    throw io_error(rst::StrCat({"Failed to reopen input file, error: ", errno}));
  }
...
size_t encrypted_input::read_internal(byte_type* b, size_t count) {
  ...
  if (!cipher_->decrypt(offset, b, read)) {
    throw io_error{rst::StrCat({"Buffer size ", read,
                                " is not multiple of cipher block size ",
                                cipher_->block_size()})};
  }
