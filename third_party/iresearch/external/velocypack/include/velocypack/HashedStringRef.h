...
class HashedStringRef {
 public:
  ...
  HashedStringRef() noexcept : _data(""), _length(0), _hash(hash("", 0)) {}
  ...
  HashedStringRef(char const* data, uint32_t length) noexcept
      : _data(data), _length(length), _hash(hash(_data, _length)) {}

  /// @brief create a HashedStringRef from a VPack slice (must be of type
  /// String)
  ...
  constexpr HashedStringRef(HashedStringRef const& other) noexcept
      : _data(other._data), _length(other._length), _hash(other._hash) {}
  ...
  HashedStringRef substr(std::size_t pos = 0,
  ...
  std::size_t rfind(char c,
                    std::size_t offset = std::string::npos) const noexcept;
  ...
  constexpr inline bool empty() const noexcept { return (_length == 0); }

  inline char const* begin() const noexcept { return _data; }

  inline char const* cbegin() const noexcept { return _data; }

  inline char const* end() const noexcept { return _data + _length; }

  inline char const* cend() const noexcept { return _data + _length; }
  ...
  constexpr inline char const* data() const noexcept { return _data; }

  constexpr inline std::size_t size() const noexcept { return _length; }

  constexpr inline std::size_t length() const noexcept { return _length; }

  constexpr inline uint32_t hash() const noexcept { return _hash; }

  // return a combined hash/length value for comparisons with less branching
  constexpr inline uint64_t tag() const noexcept {
    return (static_cast<uint64_t>(_hash) << 32) |
           static_cast<uint64_t>(_length);
  }

 private:
  inline uint32_t hash(char const* data, uint32_t length) const noexcept {
    return static_cast<uint32_t>(
        VELOCYPACK_HASH_WYHASH(data, length, 0xdeadbeef));
  }
...
inline bool operator==(
    arangodb::velocypack::HashedStringRef const& lhs,
    arangodb::velocypack::HashedStringRef const& rhs) noexcept {
...
inline bool operator==(arangodb::velocypack::HashedStringRef const& lhs,
...
struct hash<arangodb::velocypack::HashedStringRef> {
  std::size_t operator()(
      arangodb::velocypack::HashedStringRef const& value) const noexcept {
