...
void benchmark_annivfflatl2() {
...

    {
        ...
        printf("[%.3f s] Training and Indexing on %ld vectors\n, with %ld centroids", elapsed() - t0, nt, partition_num);

        ann_index_data = AnnIVFFlatL2<float>::CreateIndex(d, nt, xt, partition_num, 0, 0);
        delete[] xt;
    }
    ...
    { // Perform a search
        ...
        AnnIVFFlatL2 test_ivf(xq, nq, k, d, EmbeddingDataType::kEmbeddingF32);
        test_ivf.Search(ann_index_data, n_probes);
