...
void Console::Register(const String &name, const std::function<void(const String &)> &func) { commands_[name] = func; }

String Console::HandleCommand(const char *command) {
...
void Console::Exit(const String &command) {
...
void Console::Explain(const String &arguments) {
    ...
    if (parameter == "AST") {
        ...
        NotImplementError("Not implement explain AST") return;
    }
...
void Console::Visualize(const String &arguments) {
    SharedPtr<SQLParser> parser = MakeShared<SQLParser>();
    SharedPtr<ParserResult> parsed_result = MakeShared<ParserResult>();
...
void Console::VerifyScript(const String &arguments) { GeneralError("Verify script not supported now."); }

void Console::RunScript(const String &arguments) { GeneralError("Run script not supported now."); }

void Console::ExecuteSQL(const String &sql_text) {
    SharedPtr<SQLParser> parser = MakeShared<SQLParser>();
    SharedPtr<ParserResult> parsed_result = MakeShared<ParserResult>();
