...
public:
    SeqScanFunctionData(TableCollectionEntry *table_entry_ptr, SharedPtr<Vector<SegmentEntry *>> segment_entries, Vector<SizeT> column_ids)
        : table_entry_ptr_(table_entry_ptr), segment_entries_(std::move(segment_entries)), column_ids_(std::move(column_ids)) {}
...
public:
    static inline SharedPtr<SeqScanFunction> Make(NewCatalog *catalog, const String &func_name) {
...
public:
    explicit SeqScanFunction(String name, TableFunctionType function) : TableFunction(std::move(name), {}, std::move(function)) {}
