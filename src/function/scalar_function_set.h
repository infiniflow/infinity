...
public:
    explicit
    ScalarFunctionSet(std::string name): FunctionSet(std::move(name), FunctionType::kScalar) {}

    void
    AddFunction(const ScalarFunction& func);

    ScalarFunction
    GetMostMatchFunction(const std::vector<std::shared_ptr<BaseExpression>>& input_arguments);
