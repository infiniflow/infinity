...
void PersistenceManager::CheckValid() {
    ...
    while (iter->Valid() && iter->Key().starts_with(pm_object_prefix)) {
...
PersistWriteResult PersistenceManager::PutObjCache(const String &file_path) {
    ...
    if (obj_stat == nullptr) {
        ...
        if (current_object_ref_count_ <= 1) {
...
void PersistenceManager::Deserialize(KVInstance *kv_instance) { objects_->Deserialize(kv_instance); }
