...
template <typename T>
    requires NotString<T>
inline static size_t AppendToBuf(char *buf, const T &t, size_t start) {
...
template <typename T>
    requires NotString<T>
inline static size_t LoadFromBuf(const char *buf, T &t, size_t start) {
...
template <class T>
    requires NotString<T>
inline static T LoadFromBuf(const void *p) {
    T x;
    memcpy(&x, p, sizeof(T));
    return x;
}
...
static uint64_t ExtractHead(const Slice &key) {
    switch (key.size()) {
        ...
        case 2:
            return static_cast<uint64_t>(__builtin_bswap16(LoadFromBuf<uint16_t>(key.data()))) << 48;
        case 3:
            return (static_cast<uint64_t>(__builtin_bswap16(LoadFromBuf<uint16_t>(key.data()))) << 48) | (static_cast<uint64_t>(key.data()[2]) << 40);
        case 4:
            return static_cast<uint64_t>(__builtin_bswap32(LoadFromBuf<uint32_t>(key.data()))) << 32;

        case 5:
            return (static_cast<uint64_t>(__builtin_bswap32(LoadFromBuf<uint32_t>(key.data()))) << 32) | (static_cast<uint64_t>(key.data()[4]) << 24);

        case 6:
            return (static_cast<uint64_t>(__builtin_bswap32(LoadFromBuf<uint32_t>(key.data()))) << 32) |
                   (static_cast<uint64_t>(__builtin_bswap16(LoadFromBuf<uint16_t>(key.data() + 4))) << 16);
        case 7:
            return (static_cast<uint64_t>(__builtin_bswap32(LoadFromBuf<uint32_t>(key.data()))) << 32) |
                   (static_cast<uint64_t>(__builtin_bswap16(LoadFromBuf<uint16_t>(key.data() + 4))) << 16) |
...
public:
    ...
    u32 Size() const { return size_func_(); }

    void Encode(const void *value, std::string &buf) const { encode_func_(value, buf); }
...
public:
    static u32 Size() { return sizeof(bool); }
...
public:
    static u32 Size() { return sizeof(i8); }
...
public:
    static u32 Size() { return sizeof(i16); }
...
public:
    static u32 Size() { return sizeof(i32); }
...
public:
    static u32 Size() { return sizeof(i64); }
...
public:
    static u32 Size() { return sizeof(HugeInt); }
...
public:
    static u32 Size() { return sizeof(DecimalType); }
...
public:
    static u32 Size() { return sizeof(FloatT); }
...
public:
    static u32 Size() { return sizeof(DoubleT); }
...
public:
    static u32 Size() { return sizeof(DateType); }
...
public:
    static u32 Size() { return sizeof(TimeType); }
...
public:
    static u32 Size() { return sizeof(DateTimeType); }
...
public:
    static u32 Size() { return sizeof(TimestampType); }
...
public:
    static u32 Size() { return sizeof(IntervalT); }
