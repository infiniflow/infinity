...
bool PostingIterator::Init(const std::shared_ptr<std::vector<SegmentPosting>> &seg_postings, const uint32_t state_pool_size) {
...
void PostingIterator::MoveToCurrentDoc() {
    ...
    if (posting_option_.HasTermFrequency()) {
        ...
        uint32_t seeked_doc_count = GetCurrentSeekedDocCount();
