...
InMemPositionListDecoder::InMemPositionListDecoder(const PositionListFormatOption& option,
    ...
    : PositionListDecoder(option, session_pool),
...
void
InMemPositionListDecoder::Init(ttf_t total_tf, PairValueSkipListReader* skipList_reader, BufferedByteSlice* pos_list_buffer) {
...
bool
InMemPositionListDecoder::SkipTo(ttf_t current_ttf, InDocPositionState* state) {
...
bool
InMemPositionListDecoder::LocateRecord(const InDocPositionState* state, uint32_t& tf) {
...
uint32_t
InMemPositionListDecoder::DecodeRecord(pos_t* pos_buffer, uint32_t pos_buffer_len) {
