...
BufferedSkipListWriter::BufferedSkipListWriter(MemoryPool* byte_slice_pool,
                                               MemoryPool* buffer_pool)
    : BufferedByteSlice(byte_slice_pool, buffer_pool), last_key_(0), last_value1_(0) {
}

void
BufferedSkipListWriter::AddItem(uint32_t key, uint32_t value1, uint32_t value2) {
...
void
BufferedSkipListWriter::AddItem(uint32_t key, uint32_t value1) {
    PushBack(0, key - last_key_);
    PushBack(1, value1);
    EndPushBack();
...
void
BufferedSkipListWriter::AddItem(uint32_t value_delta) {
...
size_t
BufferedSkipListWriter::DoFlush() {
...
void
BufferedSkipListWriter::Dump(const std::shared_ptr<FileWriter>& file) {
...
size_t
BufferedSkipListWriter::EstimateDumpSize() const {
    uint32_t skiplist_size = GetTotalCount();
    if(skiplist_size == 0) {
        return 0;
    }
