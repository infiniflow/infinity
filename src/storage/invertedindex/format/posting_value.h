...
struct PostingValue {
    ...
    virtual uint32_t Encode(ByteSliceWriter &slice_writer, const uint8_t *src, uint32_t len) const = 0;

    virtual uint32_t Decode(uint8_t *dest, uint32_t dest_len, ByteSliceReader &slice_reader) const = 0;
...
extern const Int32Encoder *GetPosListEncoder();
...
struct TypedPostingValue : public PostingValue {
    ...
    uint32_t Encode(ByteSliceWriter &slice_writer, const uint8_t *src, uint32_t len) const override {
        return encoder_->Encode(slice_writer, (const T *)src, len / sizeof(T));
    }

    uint32_t Decode(uint8_t *dest, uint32_t dest_len, ByteSliceReader &slice_reader) const override {
...
struct PostingValues {
    ...
    PostingValue *GetValue(size_t index) const { return values_[index]; }

    size_t GetSize() const { return values_.size(); }

    size_t GetTotalSize() const { return values_.size() * values_[0]->GetSize(); }
