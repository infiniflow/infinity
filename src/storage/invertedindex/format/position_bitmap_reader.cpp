...
uint32_t PositionBitmapReader::Init(ByteSlice *sliceList, uint32_t offset) {
    slice_reader_.Open(sliceList);
    slice_reader_.Seek(offset);
    return DoInit();
}

uint32_t PositionBitmapReader::Init(const ByteSliceList *sliceList, uint32_t offset) {
