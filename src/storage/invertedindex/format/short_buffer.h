...
public:
    ...
    uint8_t GetRowCount() const { return posting_values_->GetSize(); }
...
private:
    void *Allocate(size_t size) { return pool_->Allocate(size); }
    void Deallocate(void *buf, size_t size) { pool_->Deallocate(buf, size); }
    ...
    static void
    BufferMemoryCopy(uint8_t *dst, uint8_t dst_col_count, uint8_t *src, uint8_t src_col_count, const PostingValues *posting_values, uint8_t srcSize);

    bool IsFull() const { return size_ >= capacity_; }

    static uint8_t *GetRow(uint8_t *buffer, uint8_t capacity, const PostingValue *value);
...
inline uint8_t *ShortBuffer::GetRow(uint8_t *buffer, uint8_t capacity, const PostingValue *value) {
