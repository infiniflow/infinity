...
public:
    ...
    void
    Init(const PositionListFormatOption& option) {
        uint8_t row_count = 0;
        uint32_t offset = 0;
...
public:
    PositionListFormat(const PositionListFormatOption& option) : skiplist_format_(nullptr) {
        Init(option);
    }
    ...
    const PositionSkipListFormat*
    GetPositionSkipListFormat() const { return skiplist_format_; }
