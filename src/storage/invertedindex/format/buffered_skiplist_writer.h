...
public:
    ...
    void
    AddItem(uint32_t deltaValue1);

    void
    AddItem(uint32_t key, uint32_t value1);

    void
    AddItem(uint32_t key, uint32_t value1, uint32_t value2);

    void
    Dump(const std::shared_ptr<FileWriter>& file) override;

    size_t
    EstimateDumpSize() const override;
