...
void PositionBitmapWriter::Set(uint32_t index) { bitmap_.Set(index); }

void PositionBitmapWriter::Resize(uint32_t size) { bitmap_.ReSize(size); }

void PositionBitmapWriter::EndDocument(uint32_t df, uint32_t total_pos_count) {
    if (df % MAX_DOC_PER_BITMAP_BLOCK == 0) {
        block_offsets_.push_back(total_pos_count - 1);
    }
}

uint32_t PositionBitmapWriter::GetDumpLength(uint32_t bit_count) const {
...
void PositionBitmapWriter::Dump(const std::shared_ptr<FileWriter> &file, uint32_t bit_count) {
