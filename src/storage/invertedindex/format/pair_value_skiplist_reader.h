...
public:
    virtual void Load(const ByteSliceList *byte_slice_list, uint32_t start, uint32_t end, const uint32_t &item_count);

    virtual void Load(ByteSlice *byte_slice, uint32_t start, uint32_t end, const uint32_t &item_count);

    bool SkipTo(uint32_t query_key, uint32_t &key, uint32_t &prev_key, uint32_t &value, uint32_t &delta) override;

    bool SkipTo(uint32_t query_key, uint32_t &key, uint32_t &value, uint32_t &delta) { return SkipTo(query_key, key, prev_key_, value, delta); }

    uint32_t GetPrevKey() const { return prev_key_; }

    uint32_t GetCurrentKey() const { return current_key_; }
