...
public:
    ...
    void Init(ByteSlice *pos_list, tf_t total_tf, uint32_t pos_list_begin, InDocPositionState *state);

    void Init(const ByteSliceList *pos_list, tf_t total_tf, uint32_t pos_list_begin, InDocPositionState *state);
    ...
    virtual bool LocateRecord(const InDocPositionState *state, uint32_t &tf);

    virtual uint32_t DecodeRecord(pos_t *pos_buffer, uint32_t pos_buffer_len);

    uint32_t GetRecordOffset() const { return record_offset_; }
...
private:
    void InitPositionBitmapReader(ByteSlice *pos_list, uint32_t &pos_list_cursor, InDocPositionState *state);

    void InitPositionBitmapReader(const ByteSliceList *pos_list, uint32_t &pos_list_cursor, InDocPositionState *state);
    ...
    void InitPositionSkipList(const ByteSliceList *pos_list,
                              tf_t total_tf,
                              uint32_t pos_skiplist_start,
                              uint32_t pos_skiplist_len,
                              InDocPositionState *state);

    void InitPositionSkipList(ByteSlice *pos_list, tf_t total_tf, uint32_t pos_skiplist_start, uint32_t pos_skiplist_len, InDocPositionState *state);

    void CalculateRecordOffsetByPosBitmap(const InDocPositionState *state, uint32_t &tf, uint32_t &record_offset, int32_t &offset_in_record);
