...
void PositionListDecoder::InitPositionBitmapReader(const ByteSliceList *pos_list, uint32_t &pos_list_cursor, InDocPositionState *state) {
...
void PositionListDecoder::InitPositionBitmapReader(ByteSlice *pos_list, uint32_t &pos_list_cursor, InDocPositionState *state) {
    uint32_t bitmapListBegin = pos_list_cursor;
    pos_bitmap_reader_ =
        session_pool_ ? new ((session_pool_)->Allocate(sizeof(PositionBitmapReader))) PositionBitmapReader() : new PositionBitmapReader;
    pos_bitmap_reader_->Init(pos_list, bitmapListBegin);
...
void PositionListDecoder::InitPositionBitmapBlockBuffer(ByteSliceReader &pos_list_reader,
    ...
    pos_bitmap_block_buffer_ =
        session_pool_ ? (uint32_t *)session_pool_->Allocate(pos_bitmap_block_count_ * sizeof(uint32_t)) : new uint32_t[pos_bitmap_block_count_];
...
void PositionListDecoder::Init(ByteSlice *pos_list, tf_t total_tf, uint32_t pos_list_begin, InDocPositionState *state) {
    assert(pos_list);
...
void PositionListDecoder::Init(const ByteSliceList *pos_list, tf_t total_tf, uint32_t pos_list_begin, InDocPositionState *state) {
    total_tf_ = total_tf;
    pos_encoder_ = GetPosListEncoder();
    if (option_.HasTfBitmap()) {
        InitPositionBitmapReader(pos_list, pos_list_begin, state);
...
bool PositionListDecoder::LocateRecord(const InDocPositionState *state, uint32_t &tf) {
...
void PositionListDecoder::CalculateRecordOffsetByPosBitmap(const InDocPositionState *state,
                                                           uint32_t &tf,
                                                           uint32_t &record_offset,
                                                           int32_t &offset_in_record) {
...
uint32_t PositionListDecoder::DecodeRecord(pos_t *pos_buffer, uint32_t pos_buffer_len) {
