...
void
ShortBuffer::ReleaseBuffer(uint8_t* buffer, uint8_t capacity) {
...
bool
ShortBuffer::SnapShot(ShortBuffer& short_buffer) const {
    short_buffer.Clear();
...
void
ShortBuffer::Reserve(uint8_t capacity) {
    if(buffer_ != nullptr) {
...
uint8_t
ShortBuffer::AllocatePlan(uint8_t cur_capacity) {
    if(cur_capacity < 2) {
        return 2;
