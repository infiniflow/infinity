...
public:
    ...
    uint32_t Init(const ByteSliceList *sliceList, uint32_t offset);

    uint32_t Init(ByteSlice *sliceList, uint32_t offset);

    inline PositionCountInfo GetPosCountInfo(uint32_t seeked_doc_count);

    uint32_t GetBitmapListSize() const { return bitmap_list_size_; }
...
private:
    ...
    inline int32_t KthHighBitIdx(uint32_t value, uint32_t &signed_bit_count, uint32_t &k);

    inline uint32_t PopCount(uint32_t value) {
        return PopCount((uint8_t)value) + PopCount((uint8_t)(value >> 8)) + PopCount((uint8_t)(value >> 16)) + PopCount((uint8_t)(value >> 24));
    }

    inline uint8_t PopCount(uint8_t value) {
...
inline PositionCountInfo PositionBitmapReader::GetPosCountInfo(uint32_t seeked_doc_count) {
...
inline int32_t PositionBitmapReader::KthHighBitIdx(uint32_t value, uint32_t &signed_bit_count, uint32_t &k) {
...
inline uint32_t PositionBitmapReader::GetNextSignedBitDistance(uint32_t begin) {
    ...
    uint32_t block_end_pos = last_block_index_ == (int32_t)block_count_ ? total_bit_count_ - 1 : block_offsets_[last_block_index_];
