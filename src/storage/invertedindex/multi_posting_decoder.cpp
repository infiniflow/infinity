...
void MultiPostingDecoder::Init(const std::shared_ptr<std::vector<SegmentPosting>> &seg_postings) {
    seg_postings_ = seg_postings;
    segment_count_ = (uint32_t)seg_postings_->size();
...
IndexDecoder *MultiPostingDecoder::CreateIndexDecoder(uint32_t doc_list_begin_pos) {
...
bool MultiPostingDecoder::MoveToSegment(docid_t start_doc_id) {
    uint32_t locate_seg_cursor = LocateSegment(segment_cursor_, start_doc_id);
    ...
    uint32_t doc_skiplist_size = doc_list_reader.ReadVUInt32();
    uint32_t doc_list_size = doc_list_reader.ReadVUInt32();

    uint32_t doc_list_begin_pos = doc_list_reader.Tell() + doc_skiplist_size;
    if (index_decoder_) {
        if (session_pool_) {
            index_decoder_->~IndexDecoder();
            session_pool_->Deallocate((void *)index_decoder_, sizeof(index_decoder_));
    ...
    uint32_t doc_skipList_start = doc_list_reader.Tell();
    uint32_t doc_skip_list_end = doc_skipList_start + doc_skiplist_size;
    ...
    if (cur_segment_format_option_.HasPositionList() || cur_segment_format_option_.HasTfBitmap()) {
        uint32_t pos_list_begin = doc_list_reader.Tell() + doc_skiplist_size + doc_list_size;
