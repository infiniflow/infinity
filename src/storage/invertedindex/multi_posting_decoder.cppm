...
public:
    ...
    bool DecodeDocBuffer(RowID start_doc_id, docid_t *doc_buffer, RowID &first_doc_id, RowID &last_doc_id, ttf_t &current_ttf);
...
private:
    bool DecodeDocBufferInOneSegment(RowID start_doc_id, docid_t *doc_buffer, RowID &first_doc_id, RowID &last_doc_id, ttf_t &current_ttf);

    IndexDecoder *CreateIndexDecoder(u32 doc_list_begin_pos);

    inline RowID GetSegmentBaseDocId(u32 seg_cursor) {
    ...
    inline u32 LocateSegment(u32 start_seg_cursor, RowID start_doc_id) {
        RowID cur_seg_base_doc_id = GetSegmentBaseDocId(start_seg_cursor);
        if (cur_seg_base_doc_id == INVALID_ROWID) {
            return start_seg_cursor;
        }

        u32 cur_seg_cursor = start_seg_cursor;
        RowID next_seg_base_doc_id = GetSegmentBaseDocId(cur_seg_cursor + 1);
        while (next_seg_base_doc_id != INVALID_ROWID && start_doc_id >= next_seg_base_doc_id) {
            ++cur_seg_cursor;
    ...
    bool MoveToSegment(RowID start_doc_id);
