...
Bitmap*
Bitmap::Clone() const { return new Bitmap(*this); }

bool
Bitmap::Alloc(uint32_t item_count, bool bSet) {
    Clear();
...
uint32_t
Bitmap::Size() const { return slot_count_ * sizeof(uint32_t); }

void
Bitmap::RefreshSetCountByScanning() const {
    if(data_ == nullptr) {
        set_count_ = 0;
        return;
    }
...
uint32_t
Bitmap::GetSetCount() const { return set_count_; }

void
Bitmap::SetSetCount(uint32_t nCount) { set_count_ = nCount; }

uint32_t
Bitmap::GetUnsetCount() const { return item_count_ - GetSetCount(); }
...
uint32_t
Bitmap::Next(uint32_t index) const {
...
void
Bitmap::ResetAllAfter(uint32_t index) {
    assert(index < item_count_);
...
void
Bitmap::Mount(uint32_t item_count, uint32_t* pData, bool donot_delete) {
    MountWithoutRefreshSetCount(item_count, pData, donot_delete);
    RefreshSetCountByScanning();
}

void
Bitmap::Copy(uint32_t start_index, uint32_t* data, uint32_t item_count) {
...
void
Bitmap::Copy(uint32_t* dst_slot_addr, uint32_t start_index, uint32_t* data, uint32_t item_count) {
...
Bitmap&
Bitmap::operator&=(Bitmap& bitmap) { return *this & bitmap; }
...
Bitmap&
Bitmap::operator|=(const Bitmap& bitmap) { return *this | bitmap; }
...
uint32_t
Bitmap::GetSetCount(uint32_t begin, uint32_t end) const {
    uint32_t data;
...
bool
Bitmap::HasSetData(uint32_t begin_index, uint32_t end_index) const {
...
void
Bitmap::ResetBetween(uint32_t begin_index, uint32_t end_index) const {
    assert(begin_index <= end_index);
    assert(end_index < item_count_);
