...
DynamicBitmap::DynamicBitmap(uint32_t item_count, bool set, MemoryPool *pool) : Bitmap(item_count, set, pool) { valid_item_count_ = item_count_; }

DynamicBitmap::DynamicBitmap(const DynamicBitmap &rhs) : Bitmap(rhs) { valid_item_count_ = rhs.valid_item_count_; }

bool DynamicBitmap::Set(uint32_t index) {
...
bool DynamicBitmap::Reset(uint32_t index) {
    if (index >= item_count_) {
        Expand(index);
    }
...
void DynamicBitmap::ReSize(uint32_t item_count) {
...
void DynamicBitmap::Expand(uint32_t index) {
...
void DynamicBitmap::Mount(uint32_t item_count, uint32_t *pData, bool donot_delete) {
    valid_item_count_ = item_count;
    Bitmap::Mount(item_count, pData, donot_delete);
}

void DynamicBitmap::MountWithoutRefreshSetCount(uint32_t item_count, uint32_t *pData, bool donot_delete) {
    valid_item_count_ = item_count;
