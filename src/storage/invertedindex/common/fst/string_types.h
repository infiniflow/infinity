...
struct char_traits_byte {
    ...
    static constexpr int_type to_int_type(const char_type &c) noexcept { return static_cast<int_type>(static_cast<unsigned char>(c)); }
...
using char_traits = std::conditional_t<std::is_same_v<Char, byte_type>, char_traits_byte, std::char_traits<char>>;
...
template <typename Char>
inline size_t CommonPrefixLength(const Char *lhs, size_t lhs_size, const Char *rhs, size_t rhs_size) noexcept {
...
template <typename Char>
inline size_t CommonPrefixLength(basic_string_view<Char> lhs, basic_string_view<Char> rhs) noexcept {
...
constexpr inline basic_string_view<ElemDst> ViewCast(basic_string_view<ElemSrc> src) noexcept {
...
template <>
struct hash<::infinity::bstring> {
    size_t operator()(const ::infinity::bstring &value) const noexcept {
        std::cout << "hash1 " << std::endl;
        return ::infinity::hash_utils::Hash(value);
    }
};

template <>
struct hash<::infinity::bytes_view> {
    size_t operator()(::infinity::bytes_view value) const noexcept { return ::infinity::hash_utils::Hash(value); }
};
