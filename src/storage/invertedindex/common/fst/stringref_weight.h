...
public:
    ...
    friend bool operator==(StringRefWeight lhs, StringRefWeight rhs) noexcept { return lhs.str_ == rhs.str_; }
    ...
    explicit StringRefWeight(std::basic_string_view<Label> rhs) noexcept : str_{rhs} {}
    ...
    bool Member() const noexcept { return StringRefWeightTraits<Label>::Member(*this); }

    size_t Hash() const noexcept { return std::hash<str_t>()(str_); }

    StringRefWeight Quantize(float delta = kDelta) const noexcept { return *this; }
...
template <typename Label>
inline std::ostream &operator<<(std::ostream &strm, StringRefWeight<Label> weight) {
    if (weight.Empty()) {
        return strm << "Epsilon";
    }
...
template <>
struct StringRefWeightTraits<infinity::byte_type> {
    static constexpr StringRefWeight<infinity::byte_type> Zero() noexcept { return {}; }

    static constexpr StringRefWeight<infinity::byte_type> One() noexcept { return Zero(); }

    static constexpr StringRefWeight<infinity::byte_type> NoWeight() noexcept { return Zero(); }

    static constexpr bool Member(StringRefWeight<infinity::byte_type>) noexcept {
        // always a member
        return true;
    }
};

inline std::ostream &operator<<(std::ostream &strm, StringRefWeight<infinity::byte_type> weight) {
    if (weight.Empty()) {
        return strm << "Epsilon";
    }
