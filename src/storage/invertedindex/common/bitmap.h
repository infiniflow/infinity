...
public:
    bool Alloc(uint32_t item_count, bool set = false);
    void Clear();
    ...
    void ResetAllAfter(uint32_t index);

    uint32_t GetItemCount() const { return item_count_; }

    virtual uint32_t GetValidItemCount() const { return item_count_; }
    ...
    void Copy(uint32_t start_index, uint32_t *data, uint32_t item_count);
    ...
    inline bool Test(uint32_t index) const;

    inline bool IsEmpty() const;

    inline uint32_t CountBits(uint64_t value) const;

    inline uint32_t CountBits(uint32_t value, uint32_t valid_count) const;
...
private:
    void Copy(uint32_t *dst_slot_addr, uint32_t start_index, uint32_t *data, uint32_t item_count);
...
public:
    ...
    static int32_t GetSlotCount(int32_t doc_count) { return (doc_count + SLOT_SIZE - 1) / SLOT_SIZE; }

    static uint32_t UpperPack(uint32_t number, uint32_t pack) { return ((number + pack - 1) / pack) * pack; }

    static uint32_t GetDumpSize(uint32_t bit_count) { return UpperPack(bit_count, SLOT_SIZE) / BYTE_SLOT_NUM; }
...
inline bool Bitmap::Test(uint32_t index) const {
...
inline uint32_t Bitmap::CountBits(uint32_t value, uint32_t valid_count) const {
    uint32_t count = 0;
...
inline void Bitmap::MountWithoutRefreshSetCount(uint32_t item_count, uint32_t *pData, bool donot_delete) {
    if (!mount_)
        assert(data_ == NULL);
    data_ = pData;
    mount_ = donot_delete;
