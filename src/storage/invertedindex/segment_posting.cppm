...
public:
    ...
    void Init(std::shared_ptr<ByteSliceList> slice_list, RowID base_row_id, u64 doc_count, TermMeta &term_meta);
    void Init(std::shared_ptr<ByteSliceList> doc_slice_list,
    ...
    void Init(RowID base_row_id, const std::shared_ptr<PostingWriter> &posting_writer);
    ...
    const std::shared_ptr<PostingWriter> &GetInMemPostingWriter() const { return posting_writer_; }
    ...
    const std::shared_ptr<ByteSliceList> &GetDocSliceListPtr() const { return doc_slice_list_; }

    const std::shared_ptr<ByteSliceList> &GetPosSliceListPtr();

private:
    std::shared_ptr<ByteSliceList> slice_list_{nullptr};
    std::shared_ptr<ByteSliceList> doc_slice_list_{nullptr};
    std::shared_ptr<ByteSliceList> pos_slice_list_{nullptr};
    RowID base_row_id_ = INVALID_ROWID;
    u32 doc_count_ = 0;
    TermMeta term_meta_;
    std::shared_ptr<PostingWriter> posting_writer_{nullptr};
    std::shared_ptr<FileReader> posting_reader_{nullptr};
