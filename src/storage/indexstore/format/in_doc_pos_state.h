...
public:
    InDocPositionState(const PositionListFormatOption& option = PositionListFormatOption())
        : pos_decoder_(nullptr),
          record_offset_(0),
          offset_in_record_(0),
          total_position_count_(0) {}
    ...
    void
    SetPositionListDecoder(PositionListDecoder* pos_decoder) {
        pos_decoder_ = pos_decoder;
    }

    PositionListDecoder*
    GetPositionListDecoder() const {
        return pos_decoder_;
    }

    void
    SetSeekedDocCount(uint32_t seeked_doc_count) {
        seeked_doc_count_ = seeked_doc_count;
    }

    uint32_t
    GetSeekedDocCount() const {
        return seeked_doc_count_;
    }

    void
    SetRecordOffset(int32_t record_offset) {
        record_offset_ = record_offset;
    }

    int32_t
    GetRecordOffset() const {
        return record_offset_;
    }

    void
    SetOffsetInRecord(int32_t offset_in_record) {
        offset_in_record_ = offset_in_record;
    }

    int32_t
    GetOffsetInRecord() const {
        return offset_in_record_;
    }

    void
    SetTotalPosCount(uint32_t total_position_count) {
        total_position_count_ = total_position_count;
    }

    uint32_t
    GetTotalPosCount() const {
        return total_position_count_;
    }
