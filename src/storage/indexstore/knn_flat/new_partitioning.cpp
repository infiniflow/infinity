...
partition_fuzzy_median3(
    ...
    IFV
        printf("shrink: thresh=%g n_eq_1=%ld\n", float(thresh), long(n_eq_1));
...
template<class C>
simd16uint16
max_func(simd16uint16 v, simd16uint16 thr16) {
...
template<class C>
uint16_t
simd_partition_fuzzy_with_bounds(
        uint16_t* vals,
        typename C::TI* ids,
        size_t n,
        size_t q_min,
    ...
    IFV
        printf("shrink: thresh=%d q=%ld n_eq_1=%ld\n", thresh, q, n_eq_1);
    if(n_eq_1 < 0) { // happens when > q elements are at lower bound
        assert(s0 + 1 == s1);
        q = q_min;
        if(C::is_max) {
        ...
        n_eq_1 = q;
        IFV
            printf("  override: thresh=%d n_eq_1=%ld\n", thresh, n_eq_1);
    } else {
        assert(n_eq_1 <= n_eq);
    }
...
template<class C>
uint16_t
simd_partition_fuzzy_with_bounds_histogram(
        uint16_t* vals,
        typename C::TI* ids,
        size_t n,
        size_t q_min,
    ...
    IFV
        printf("end bissection: thresh=%d q=%ld n_eq=%ld\n", thresh, q, n_eq);
    ...
    if(!C::is_max) {
        ...
        q = n - q;
        IFV
            printf("revert due to CMin, q->%ld n_eq->%ld\n", q, n_eq);
    }
...
template float
partition_fuzzy<CMin<float, int64_t>>(
        float* vals,
        int64_t* ids,
        size_t n,
        size_t q_min,
...
template float
partition_fuzzy<CMax<float, int64_t>>(
        float* vals,
        int64_t* ids,
        size_t n,
        size_t q_min,
...
template uint16_t
partition_fuzzy<CMin<uint16_t, int64_t>>(
        uint16_t* vals,
        int64_t* ids,
        size_t n,
        size_t q_min,
...
template uint16_t
partition_fuzzy<CMax<uint16_t, int64_t>>(
        uint16_t* vals,
        int64_t* ids,
        size_t n,
        size_t q_min,
...
template uint16_t
partition_fuzzy<CMin<uint16_t, int>>(
        uint16_t* vals,
        int* ids,
        size_t n,
        size_t q_min,
...
template uint16_t
partition_fuzzy<CMax<uint16_t, int>>(
        uint16_t* vals,
        int* ids,
        size_t n,
        size_t q_min,
...
simd16uint16
histogram_8(const uint16_t* data, Preproc pp, size_t n_in) {
    assert(n_in % 16 == 0);
    int n = n_in / 16;
...
simd32uint8
accu4to8_2(simd32uint8 a4_0, simd32uint8 a4_1) {
...
template<class Preproc>
simd16uint16
histogram_16(const uint16_t* data, Preproc pp, size_t n_in) {
    assert(n_in % 16 == 0);
    int n = n_in / 16;
...
void
simd_histogram_8_unbounded(const uint16_t* data, int n, int* hist) {
    PreprocNOP pp;
...
void
simd_histogram_16_unbounded(const uint16_t* data, int n, int* hist) {
