...
public:
    ...
    // KnnHnswBase(segment_manager *sm) : M_(0), ef_construction_(0), mult_(0), data_store_(sm), prefetch_step_(DEFAULT_PREFETCH_SIZE), sm_(sm) {}
    KnnHnswBase(const void_allocator &alloc_inst)
        : M_(0), ef_construction_(0), mult_(0), data_store_(alloc_inst), prefetch_step_(DEFAULT_PREFETCH_SIZE), alloc_inst_(alloc_inst) {}
    KnnHnswBase(KnnHnswBase &&other) noexcept
        : M_(std::exchange(other.M_, 0)), ef_construction_(std::exchange(other.ef_construction_, 0)), mult_(std::exchange(other.mult_, 0.0)),
          ...
          prefetch_step_(L1_DATA_CACHE_SIZE / data_store_.vec_store_meta().GetVecSizeInBytes()), alloc_inst_(std::move(other.alloc_inst_)) {}
...
protected:
    ...
    SearchLayer(VertexType enter_point, const QueryType &query, VertexType query_i, i32 layer_idx, size_t result_n, const Filter &filter) const {
        ...
        while (!candidate.empty()) {
            ...
            std::shared_lock<IpShMut> lock;
            if constexpr (WithLock) { // yee todo
    ...
    void ConnectNeighbors(VertexType vertex_i, const VertexType *q_neighbors_p, VertexListSize q_neighbor_size, i32 layer_idx) {
        for (int i = 0; i < q_neighbor_size; ++i) {
            ...
            auto lock = data_store_.UniqueLock(n_idx); // yee todo
...
public:
    ...
    void Build(VertexType vertex_i) {
        auto lock = data_store_.UniqueLock(vertex_i); // yee todo
...
export template <typename VecStoreType, typename LabelType>
class KnnHnsw : public KnnHnswBase<VecStoreType, LabelType> {
public:
    using DataStore = DataStore<VecStoreType, LabelType>;
    using Distance = VecStoreType::Distance;
    ...
    using Base = KnnHnswBase<VecStoreType, LabelType>;
    // constexpr static bool kOwnMem = OwnMem;

    using segment_manager_t = boost::interprocess::managed_mapped_file::segment_manager;
    using void_allocator = boost::interprocess::allocator<void, segment_manager_t>;

    KnnHnsw(size_t M, size_t ef_construction, DataStore data_store, Distance distance, const void_allocator &alloc_inst) : Base(alloc_inst) {
        this->M_ = M;
    ...
    static std::unique_ptr<KnnHnsw>
    Make(size_t chunk_size, size_t max_chunk_n, size_t dim, size_t M, size_t ef_construction, const void_allocator &alloc_inst) {
        auto [Mmax0, Mmax] = KnnHnsw::GetMmax(M);
        // static_assert(std::is_standard_layout_v<DataStore>);
        auto data_store = DataStore::Make(chunk_size, max_chunk_n, dim, Mmax0, Mmax, alloc_inst);
        static_assert(std::is_standard_layout_v<Distance>);
        Distance distance(data_store.dim());
        return std::make_unique<KnnHnsw>(M, ef_construction, std::move(data_store), std::move(distance), alloc_inst);
    }
...
//     //         UnrecoverableError(fmt::format("LoadFromPtr failed, ptr {:p}, ptr_end {:p}, diff {}", (const void *)ptr, (const void *)ptr_end,
//     diff));
//     //     }
//     //     return std::make_unique<KnnHnsw>(M, ef_construction, std::move(data_store), std::move(distance));
//     // }
