...
HnswLSGBuilder::HnswLSGBuilder(const IndexHnsw *index_hnsw, SharedPtr<ColumnDef> column_def, LSGConfig lsg_config, BufferManager *buffer_mgr)
    : index_hnsw_(index_hnsw), column_def_(column_def), lsg_config_(std::move(lsg_config)), buffer_mgr_(buffer_mgr) {
...
UniquePtr<HnswIndexInMem> HnswLSGBuilder::Make(SegmentEntry *segment_entry, SizeT column_id, TxnTimeStamp begin_ts, bool check_ts) {
    const DataType *column_type = column_def_->type().get();
    auto *embedding_info = static_cast<EmbeddingInfo *>(column_type->type_info().get());
    switch (embedding_info->Type()) {
        case EmbeddingDataType::kElemFloat:
            return MakeImpl<float, float>(segment_entry, column_id, begin_ts, check_ts);
        case EmbeddingDataType::kElemDouble:
            return MakeImpl<double, float>(segment_entry, column_id, begin_ts, check_ts);
        default:
...
UniquePtr<HnswIndexInMem> HnswLSGBuilder::MakeImpl(SegmentEntry *segment_entry, SizeT column_id, TxnTimeStamp begin_ts, bool check_ts) {
    [[maybe_unused]] auto avg = GetLSAvg<DataType, DistanceDataType>(segment_entry, column_id, begin_ts, check_ts);
    return nullptr;
}

template <typename DataType, typename DistanceDataType>
UniquePtr<DataType[]> HnswLSGBuilder::GetLSAvg(SegmentEntry *segment_entry, SizeT column_id, TxnTimeStamp begin_ts, bool check_ts) {
    ...
    ivf_index->BuildIVFIndex(base_row_id, row_count, segment_entry, column_def_, buffer_mgr_);

    OneColumnIterator<DataType> iter(segment_entry, buffer_mgr_, column_id, begin_ts);
    UniquePtr<DataType[]> avg;
    switch (index_hnsw_->metric_type_) {
        case MetricType::kMetricL2:
            avg = GetAvgByIVF<decltype(iter), DataType, CompareMax, DistanceDataType>(std::move(iter), row_count, ivf_index.get());
            break;
        case MetricType::kMetricInnerProduct:
            avg = GetAvgByIVF<decltype(iter), DataType, CompareMin, DistanceDataType>(std::move(iter), row_count, ivf_index.get());
            break;
        default:
            UnrecoverableError(fmt::format("Invalid metric type: {}", MetricTypeToString(index_hnsw_->metric_type_)));
    }
