...
public:
    RabitqVecStoreMetaBase(segment_manager *sm)
        : origin_dim_(0), rom_(sm), rot_centroid_(sm), dim_(0), compress_data_size_(0), compress_query_size_(0) {}
...
protected:
    ...
    boost::interprocess::vector<DataType, boost::interprocess::allocator<DataType, segment_manager>> rom_;

    // ArrayPtr<DataType> rot_centroid_; // Rotation centroid of all vector in DataStore
    boost::interprocess::vector<DataType, boost::interprocess::allocator<DataType, segment_manager>> rot_centroid_;
...
private:
    RabitqVecStoreMeta(size_t origin_dim, segment_manager *sm) : Base(sm) {
...
public:
    ...
    using segment_manager = boost::interprocess::managed_mapped_file::segment_manager;
    using string_allocator = boost::interprocess::allocator<char, segment_manager>;
    using string = boost::container::basic_string<char, std::char_traits<char>, string_allocator>;

private:
    RabitqVecStoreInner(size_t max_vec_num, const Meta &meta, segment_manager *sm) {
