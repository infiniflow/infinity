...
public:
    ...
    LVQVecStoreMetaBase(segment_manager *sm) : dim_(0), compress_data_size_(0), mean_(sm), normalize_(false), sm_(sm) {}
...
protected:
    ...
    // <MeanType> mean_;
    boost::interprocess::vector<MeanType, boost::interprocess::allocator<MeanType, segment_manager>> mean_;
...
private:
    LVQVecStoreMeta(size_t dim, segment_manager *sm) : Base(sm) {
        this->dim_ = dim;
...
public:
    ...
    LVQVecStoreMeta(segment_manager *sm) : Base(sm) {}
...
private:
    LVQVecStoreInner(size_t max_vec_num, const Meta &meta, segment_manager *sm) {
