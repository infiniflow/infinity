...
public:
    ...
    template <typename DistanceT, typename LabelT, bool WithLock = true, typename Query = NoneType>
    Tuple<SizeT, UniquePtr<DistanceT[]>, UniquePtr<LabelT[]>>
    KnnSearch(const Query &q, SizeT k, const KnnSearchOption &option = {}) const {
        Tuple<SizeT, UniquePtr<DistanceT[]>, UniquePtr<LabelT[]>> res{};
        std::visit(
            [&](auto &&arg) {
                using T = std::decay_t<decltype(arg)>;
                if constexpr (!std::is_same_v<T, std::nullptr_t>) {
                    res = arg->template KnnSearch<WithLock>(q, k, option);
                }
            },
            hnsw_);
        return res;
...
public:
   ...
   return mem_usage;
    ...
    Pair<SizeT, SizeT> GetInfo() const {
        return std::visit(
            [&](auto &&index) {
                using T = std::decay_t<decltype(index)>;
                if constexpr (std::is_same_v<T, std::nullptr_t>) {
                    return Pair<SizeT, SizeT>{};
                } else {
                    return Pair<SizeT, SizeT>{index->mem_usage(), index->GetVecNum()};
                }
            },
            hnsw_);
    }
    ...
    void Dump(SizeT& row_count, SizeT& index_size, SizeT& dump_size) {
        std::visit(
            [&](auto &&index) {
                using T = std::decay_t<decltype(index)>;
                if constexpr (std::is_same_v<T, std::nullptr_t>) {
                    ...
                    if constexpr (IndexT::kOwnMem) {
                        ...
                        UnrecoverableError("HnswHandler::Dump: index does not own memory");
                    }
                }
            },
            hnsw_);
    }
    void Dump(SizeT& dump_size) {
        std::visit(
            [&](auto &&index) {
                using T = std::decay_t<decltype(index)>;
                if constexpr (std::is_same_v<T, std::nullptr_t>) {
...
public:
    ...
    void InsertVecs(SizeT block_offset,
                    ...
                    SizeT row_count,
                    const HnswInsertConfig &config = kDefaultHnswInsertConfig) {
        SizeT mem_usage = hnsw_handler_->InsertVecs(block_offset, block_column_entry, buffer_manager, row_offset, row_count, kBuildBucketSize, config);
