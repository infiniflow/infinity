...
public:
    explicit HeapResultHandler(size_t n_queries, size_t top_k, DistType *distance, ID *id)
    ...
    [[nodiscard]] u32 GetSize(size_t q_id) const { return sizes[q_id]; }

    void ReInitialize() { std::fill(sizes.begin(), sizes.end(), 0); }

    void AddResult(size_t q_id, DistType d, ID i) {
        u32 &size = sizes[q_id];
    ...
    void AddResults(size_t i_start, size_t i_end, size_t j0, size_t j1, const DistType *dis_tab, u32 segment_id, u32 segment_offset_start) {
    ...
    void EndWithoutSort(size_t q_id) {
...
public:
    explicit SingleBestResultHandler(size_t n_queries, DistType *distance, ID *id)
    ...
    void Begin(const size_t i) { distance_ptr[i] = Compare::InitialValue(); }
    ...
    void AddResult(const size_t i, const DistType dis, const ID idx) {
    ...
    void AddResults(size_t i_start, size_t i_end, size_t j0, size_t j1, const DistType *dis_tab_input, u32 segment_id, u32 segment_offset_start) {
        for (size_t i = i_start; i < i_end; ++i) {
            ...
            auto tmp_min_j = std::numeric_limits<size_t>::max();
            for (u32 j = j0; j < j1; ++j) {
            ...
            if (tmp_min_j != std::numeric_limits<size_t>::max()) {
...
class ReservoirResultHandler : public ResultHandlerBase {
    ...
    std::vector<DistType> thresholds;
    std::unique_ptr<DistType[]> reservoir_distance_ptr;
    std::unique_ptr<ID[]> reservoir_id_ptr;
    ...
    static void count_lt_and_eq(const DistType *vals, size_t n, DistType thresh, size_t &n_lt, size_t &n_eq) {
        n_lt = n_eq = 0;
    ...
    static DistType sample_threshold_median3(const DistType *vals, size_t n, DistType thresh_inf, DistType thresh_sup) {
    ...
    static size_t compress_array(DistType *vals, ID *ids, size_t n, DistType thresh, size_t n_eq) {
        size_t wp = 0;
        for (size_t i = 0; i < n; ++i) {
    ...
    static DistType partition_median3(DistType *vals, ID *ids, size_t n, size_t q_min, size_t q_max, size_t &q_out) {
        if (n < 3) {
...
public:
    explicit ReservoirResultHandler(size_t n_queries, size_t top_k, DistType *distance, ID *id)
        ...
        reservoir_distance_ptr = std::make_unique_for_overwrite<DistType[]>(n_queries * capacity);
        reservoir_id_ptr = std::make_unique_for_overwrite<ID[]>(n_queries * capacity);
    }
    ...
    [[nodiscard]] DistType GetThreshold(size_t q_id) const { return thresholds[q_id]; }

    [[nodiscard]] size_t GetSize(size_t q_id) const { return sizes[q_id]; }

    void AddResult(size_t q_id, DistType distance, ID id) {
