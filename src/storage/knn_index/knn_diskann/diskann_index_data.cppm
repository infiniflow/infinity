...
private:
    ...
    void CreateDiskLayout(FileHandler &data_file_handler, FileHandler &mem_index_file_handler, FileHandler &index_file_handler) {
        SizeT npts = data_num_;
        SizeT ndims = dimension_;
        SizeT actual_file_size = 0;
        u32 width;
        SizeT medoid, vamana_frozen_num, vamana_frozen_loc = 0;
        mem_index_file_handler.Read(&actual_file_size, sizeof(SizeT));
        mem_index_file_handler.Read(&width, sizeof(u32));
        mem_index_file_handler.Read(&medoid, sizeof(SizeT));
        mem_index_file_handler.Read(&vamana_frozen_num, sizeof(SizeT));
        ...
        index_file_handler.Write(output_file_meta.data(), DISKANN_SECTOR_LEN);
        ...
        if (nnodes_per_sector > 0) { // Write multiple nodes per sector 
            for (u64 sector = 0; sector < n_sectors; sector++) {
                ...
                for (u64 sector_node_id = 0; sector_node_id < nnodes_per_sector && cur_node_id < npts; sector_node_id++) {
                    ...
                    mem_index_file_handler.Read(&nnbrs, sizeof(u32)); // read neighbor num
                    assert(nnbrs <= width && nnbrs > 0);
                    mem_index_file_handler.Read(nhood_buf, nnbrs * sizeof(SizeT)); // read nnbrs * neighbor_id
                    data_file_handler.Read(cur_node_coord.get(), ndims * sizeof(VectorDataType)); // read vector
                    ...
                    memcpy(node_buf.get() + ndims * sizeof(VectorDataType) + sizeof(u32), nhood_buf, nnbrs * sizeof(SizeT)); // copy neighbor_id to node_buf
                ...
                index_file_handler.Write(sector_buf.get(), DISKANN_SECTOR_LEN);
            }
            ...
            for (u64 i = 0; i < npts; i++) {
                ...
                memcpy(multisector_buf.get() + ndims * sizeof(VectorDataType) + sizeof(u32), nhood_buf, nnbrs * sizeof(SizeT)); // copy neighbor_id to multisector_buf

                index_file_handler.Write(multisector_buf.get(), DISKANN_SECTOR_LEN * nsectors_per_node);
            }
        }
    }
...
public:
    ...
    static UniquePtr<This> Make(u32 dimension, SizeT data_num, u32 R, u32 L, u32 num_pq_chunks, u32 num_parts, u32 num_centers=DISKANN_NUM_CENTERS) {
    ...
    void BuildIndex(const u32 dimension,
        ...
        {
            fs.Seek(*data_file_handler, 0);
            auto [pqCompressed_data_file_handler, pq_data_file_status] = 
                fs.OpenFile(pqCompressed_data_file_path_, FileFlags::CREATE_FLAG | FileFlags::WRITE_FLAG, FileLockType::kWriteLock);
        ...
        {
            fs.Seek(*data_file_handler, 0);
            auto [mem_index_file_handler, mem_index_write_status] = fs.OpenFile(mem_index_file_path_, 
                                                                FileFlags::CREATE_FLAG | FileFlags::WRITE_FLAG, FileLockType::kWriteLock);                 
            if (!mem_index_write_status.ok()) {
                UnrecoverableError(mem_index_write_status.message());
            }
            BuildMergedVamanaIndex(*data_file_handler, *mem_index_file_handler, labels_);
            mem_index_file_handler->Close();
        ...
        {
            fs.Seek(*data_file_handler, 0);
            auto [mem_index_file_handler, mem_index_read_status] = fs.OpenFile(mem_index_file_path_, 
                                                                FileFlags::READ_FLAG, FileLockType::kReadLock);    
            if (!mem_index_read_status.ok()) {
                UnrecoverableError(mem_index_read_status.message());
            }
            auto [index_file_handler, index_file_status] = fs.OpenFile(index_file_path_, 
                                                                FileFlags::CREATE_FLAG | FileFlags::WRITE_FLAG, FileLockType::kWriteLock);
            if (!index_file_status.ok()) {
                UnrecoverableError(index_file_status.message());
            }
            CreateDiskLayout(*data_file_handler, *mem_index_file_handler, *index_file_handler);
            mem_index_file_handler->Close();
