...
public:
    ...
    Neighbor &operator[](SizeT i) {
        return data_[i];
    }

    const Neighbor operator[](SizeT i) const {
        return data_[i];
    }
...
public:
    ...
    PQScratch(SizeT graph_degree, SizeT aligned_dim){
        AllocAligned((void**)&aligned_pq_coord_scratch,
                      graph_degree * DISKANN_MAX_PQ_CHUNKS * sizeof(u8), 256);
...
public:
    ...
    DataType *AlignedQueryT()
    {
        return aligned_query_T_;
    }
    PQScratch<DataType> *PqScratch()
    {
        return pq_scratch_;
    }
...
public:
    InMemQueryScratch(u32 search_l, u32 indexing_l, u32 r, u32 maxc, SizeT dim, SizeT aligned_dim,
                      SizeT alignment_factor, bool init_pq_scratch = false): L_(0), R_(r), maxc_(maxc){
    ...
    inline DataType *AlignedQuery(){
        return this->aligned_query_T_;
    }
    inline PQScratch<DataType> *PqScratch(){
        return this->pq_scratch_;
    }
    inline Vector<Neighbor> &Pool(){
        return this->pool_;
    }
    inline NeighborPriorityQueue &BestLNodes(){
        return this->best_l_nodes_;
    }
    inline Vector<f32> &OccludeFactor(){
        return this->occlude_factor_;
    }
    inline std::unordered_set<u32> &InsertedIntoPoolRs(){
        return this->inserted_into_pool_rs_;
    }
    inline boost::dynamic_bitset<> &InsertedIntoPoolBs(){
        return *this->inserted_into_pool_bs_;
    }
    inline Vector<SizeT> &IdScratch(){
        return this->id_scratch_;
    }
    inline Vector<f32> &DistScratch(){
        return this->dist_scratch_;
    }
    inline std::unordered_set<u32> &ExpandedNodesSet(){
        return this->expanded_nodes_set_;
    }
    inline Vector<Neighbor> &ExpandedNghrsVec(){
        return this->expanded_nghrs_vec_;
    }
    inline Vector<u32> &OccludeListOutput(){
        return this->occlude_list_output_;
    }
