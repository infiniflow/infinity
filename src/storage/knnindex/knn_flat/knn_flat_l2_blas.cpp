...
void KnnFlatL2Blas<DistType>::Begin() {
...
template <typename DistType>
void KnnFlatL2Blas<DistType>::Search(const DistType *base, i16 base_count, i32 segment_id, i16 block_id) {
    if (!begin_) {
...
template <typename DistType>
void KnnFlatL2Blas<DistType>::End() {
    if (!begin_)
        return;
