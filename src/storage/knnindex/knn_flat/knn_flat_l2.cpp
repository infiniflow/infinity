...
template <typename DistType>
void KnnFlatL2<DistType>::Search(const DistType *base, i16 base_count, i32 segment_id, i16 block_id) {
    if (!begin_) {
    ...
    for (int64_t i = 0; i < this->query_count_; i++) {
        ...
        for (i16 j = 0; j < base_count; j++, y_j += this->dimension_) {
            DistType l2 = faiss::fvec_L2sqr(x_i, y_j, this->dimension_);
