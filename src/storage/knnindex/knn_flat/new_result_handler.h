...
struct NewHeapResultHandler : public ResultHandler {
    ...
    void add_results(size_t i_start, size_t i_end, size_t j0, size_t j1, const T *dis_tab, i32 segment_id, i16 block_id) {
...
struct ReservoirTopN {
    ...
    ReservoirTopN(size_t n, size_t capacity, T *vals, TI *ids) : vals(vals), ids(ids), i(0), n(n), capacity(capacity) {
        assert(n < capacity);
        threshold = C::neutral();
    }
...
struct NewReservoirResultHandler : public ResultHandler {
    ...
    explicit NewReservoirResultHandler(size_t nq, T *heap_dis_tab, TI *heap_ids_tab, size_t k)
