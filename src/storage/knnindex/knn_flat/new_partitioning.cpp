...
template <class C>
typename C::T sample_threshold_median3(const typename C::T *vals, int n, typename C::T thresh_inf, typename C::T thresh_sup) {
...
template <class C>
void count_lt_and_eq(const typename C::T *vals, size_t n, typename C::T thresh, size_t &n_lt, size_t &n_eq) {
    n_lt = n_eq = 0;
...
template <class C>
size_t compress_array(typename C::T *vals, typename C::TI *ids, size_t n, typename C::T thresh, size_t n_eq) {
    size_t wp = 0;
    for (size_t i = 0; i < n; i++) {
...
void find_minimax(const uint16_t *vals, size_t n, uint16_t &smin, uint16_t &smax) {
...
template <class C>
void count_lt_and_eq(const uint16_t *vals, int n, uint16_t thresh, size_t &n_lt, size_t &n_eq) {
    n_lt = n_eq = 0;
...
template <class C>
int simd_compress_array(uint16_t *vals, typename C::TI *ids, size_t n, uint16_t thresh, int n_eq) {
    simd16uint16 thr16(thresh);
    ...
    for (i0 = 0; i0 + 15 < n && n_eq > 0; i0 += 16) {
        ...
        simd16uint16 gemask = (v == max2);
        simd16uint16 eqmask = (v == thr16);
        uint32_t bits = get_MSBs(blendv(simd32uint8(eqmask), simd32uint8(gemask), simd32uint8(mixmask)));
        bits ^= 0xAAAAAAAA;
        // bit 2*i     : eq
        // bit 2*i + 1 : lt
...
template <class C>
uint16_t simd_partition_fuzzy_with_bounds(uint16_t *vals,
                                          typename C::TI *ids,
                                          size_t n,
                                          size_t q_min,
                                          size_t q_max,
...
template <class C>
uint16_t simd_partition_fuzzy_with_bounds_histogram(uint16_t *vals,
                                                    typename C::TI *ids,
                                                    size_t n,
                                                    size_t q_min,
                                                    size_t q_max,
...
template <class C>
uint16_t simd_partition_fuzzy(uint16_t *vals, typename C::TI *ids, size_t n, size_t q_min, size_t q_max, size_t *q_out) {
    ...
    return simd_partition_fuzzy_with_bounds<C>(vals, ids, n, q_min, q_max, q_out, s0i, s1i);
}
...
uint16_t simd_partition(uint16_t *vals, typename C::TI *ids, size_t n, size_t q) {
    ...
    return simd_partition_fuzzy_with_bounds<C>(vals, ids, n, q, q, nullptr, s0i, s1i);
}

template <class C>
uint16_t simd_partition_with_bounds(uint16_t *vals, typename C::TI *ids, size_t n, size_t q, uint16_t s0i, uint16_t s1i) {
    return simd_partition_fuzzy_with_bounds<C>(vals, ids, n, q, q, nullptr, s0i, s1i);
}
...
template <class C>
typename C::T partition_fuzzy(typename C::T *vals, typename C::TI *ids, size_t n, size_t q_min, size_t q_max, size_t *q_out) {
    ...
    if (is_uint16 && is_aligned_pointer(vals)) {
        return simd_partitioning::simd_partition_fuzzy<C>((uint16_t *)vals, ids, n, q_min, q_max, q_out);
    }
#endif
    return partitioning::partition_fuzzy_median3<C>(vals, ids, n, q_min, q_max, q_out);
}
...
static const simd32uint8 shifts =
    simd32uint8::create<1, 16, 0, 0, 4, 64, 0, 0, 0, 0, 1, 16, 0, 0, 4, 64, 1, 16, 0, 0, 4, 64, 0, 0, 0, 0, 1, 16, 0, 0, 4, 64>();
...
void compute_accu2(const uint16_t *&data, Preproc &pp, simd16uint16 &a4lo, simd16uint16 &a4hi) {
...
static const simd32uint8 shifts2 =
    simd32uint8::create<1, 2, 4, 8, 16, 32, 64, 128, 1, 2, 4, 8, 16, 32, 64, 128, 1, 2, 4, 8, 16, 32, 64, 128, 1, 2, 4, 8, 16, 32, 64, 128>();

simd32uint8 shiftr_16(simd32uint8 x, int n) { return simd32uint8(simd16uint16(x) >> n); }

// 2-bit accumulator: we can add only up to 3 elements
// on output we return 2*4-bit results
template <int N, class Preproc>
void compute_accu2_16(const uint16_t *&data, Preproc pp, simd32uint8 &a4_0, simd32uint8 &a4_1, simd32uint8 &a4_2, simd32uint8 &a4_3) {
...
simd32uint8 accu4to8_2(simd32uint8 a4_0, simd32uint8 a4_1) {
    ...
    simd16uint16 a8_0 = combine2x2((simd16uint16)(a4_0 & mask4), (simd16uint16)(shiftr_16(a4_0, 4) & mask4));

    simd16uint16 a8_1 = combine2x2((simd16uint16)(a4_1 & mask4), (simd16uint16)(shiftr_16(a4_1, 4) & mask4));
...
struct PreprocNOP {
    simd16uint16 operator()(simd16uint16 x) { return x; }
};
...
void PartitionStats::reset() { memset(this, 0, sizeof(*this)); }
