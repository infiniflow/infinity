...
Status TableIndexMeeta::RemoveSegmentIndexIDs(KVInstance *kv_instance, const Vector<SegmentID> &segment_ids) {
    ...
    for (SegmentID segment_id : segment_ids) {
        String segment_id_key = KeyEncode::CatalogIdxSegmentKey(table_meta_.db_id_str(), table_meta_.table_id_str(), index_id_str_, segment_id);
        Status status = kv_instance->Delete(segment_id_key);
        if (!status.ok()) {
            return status;
        }
    }
...
Status TableIndexMeeta::UninitSet1(KVInstance *kv_instance, TxnTimeStamp begin_ts, UsageFlag usage_flag) {
    ...
    std::tie(index_base, status) = GetIndexBase(kv_instance);
    if (!status.ok()) {
        return status;
    }
    ...
    String index_def_key = GetTableIndexTag("index_base");
    status = kv_instance->Delete(index_def_key);
    if (!status.ok()) {
        return status;
    }
...
Tuple<SharedPtr<TableIndexSnapshotInfo>, Status>
TableIndexMeeta::MapMetaToSnapShotInfo(KVInstance *kv_instance, TxnTimeStamp begin_ts, TxnTimeStamp commit_ts) {
    ...
    if (!segment_ids_) {
        segment_ids_ =
            infinity::GetTableIndexSegments(kv_instance, table_meta_.db_id_str(), table_meta_.table_id_str(), index_id_str_, begin_ts, commit_ts);
    }
    for (const auto &segment_id : *segment_ids_) {
        SegmentIndexMeta segment_index_meta(segment_id, *this);
        auto [segment_index_snapshot, segment_index_status] = segment_index_meta.MapMetaToSnapShotInfo(kv_instance, begin_ts, commit_ts);
