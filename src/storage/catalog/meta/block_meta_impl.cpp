...
//     auto rel_file_path = std::make_shared<std::string>(fmt::format("{}/{}", *block_dir, BlockVersion::PATH));
//     version_file_worker_ = fileworker_mgr->GetFileWorker(*rel_file_path);
//     if (!version_file_worker_) {
//         return Status::BufferManagerError(fmt::format("Get version buffer failed: {}", version_file_worker_->GetFilePath()));
//     }
//     return Status::OK();
// }
