...
Status ColumnMeta::GetFileWorker(FileWorker *&column_buffer, FileWorker *&outline_buffer) {
    return GetFileWorker(column_buffer, outline_buffer, nullptr);
}
...
Status ColumnMeta::LoadColumnBuffer(std::shared_ptr<ColumnDef> column_def) {
    ...
    auto *fileworker_mgr = InfinityContext::instance().storage()->fileworker_manager();
    {
        ...
        if (data_file_worker_ == nullptr) {
            return Status::BufferManagerError(fmt::format("Get buffer object failed: {}", data_file_path));
        }
    }
    ...
    if (buffer_type == VectorBufferType::kVarBuffer) {
        ...
        if (var_file_worker_ == nullptr) {
            return Status::BufferManagerError(fmt::format("Get outline buffer object failed: {}", var_file_path));
        }
    }
    return Status::OK();
}
