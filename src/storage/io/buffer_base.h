...
public:
    ...
    struct Buffer {
        ...
        Position
        Begin() const { return begin_pos_; }
        Position
        End() const { return end_pos_; }
        size_t
        Size() const { return size_t(end_pos_ - begin_pos_); }
        void
        Resize(size_t size) { end_pos_ = begin_pos_ + size; }
        bool
    ...
    BufferBase(Position ptr, size_t size, size_t offset)
            : pos_(ptr + offset), working_buffer_(ptr, ptr + size), internal_buffer_(ptr, ptr + size) {}

    void
    Set(Position ptr, size_t size, size_t offset) {
    ...
    Buffer&
    InternalBuffer() { return internal_buffer_; }

    Buffer&
    GetBuffer() { return working_buffer_; }

    Position&
    Pos() { return pos_; }

    size_t
    Offset() const { return size_t(pos_ - working_buffer_.Begin()); }

    /// How many bytes are available for read/write
    size_t
    Available() const { return size_t(working_buffer_.End() - pos_); }

    /// How many bytes have been read/written
    size_t
    Count() const { return bytes_ + Offset(); }

    bool
    HasPendingData() const { return Available() > 0; }
