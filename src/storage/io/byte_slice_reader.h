...
public:
    ...
    size_t
    Read(void* value, size_t len);

    size_t
    ReadMayCopy(void*& value, size_t len);
    ...
    size_t
    Tell() const { return global_offset_; }

    bool
    CurrentSliceEnough(size_t len) {
...
public:
    void
    Open(ByteSliceList* sliceList);

    void
    Open(ByteSlice* slice);

    void
    Close();

    size_t
    GetSize() const { return slice_list_->GetTotalSize(); }

    ByteSliceList*
    GetByteSliceList() const { return slice_list_; }
    bool
    End() const { return Tell() + 1 >= size_; }

private:
    void
    SetSlice(ByteSlice* slice);
...
private:
    ...
    ByteSlice*
    NextSlice(ByteSlice* byte_slice) {
        return byte_slice->next_;
    }

    size_t
    GetSliceDataSize(ByteSlice* byte_slice) const {
...
inline int16_t
ByteSliceReader::ReadInt16() {
    return ReadInt<int16_t>();
}

inline int32_t
ByteSliceReader::ReadInt32() {
    return ReadInt<int32_t>();
}

inline uint32_t
ByteSliceReader::ReadUInt32() {
    return ReadInt<uint32_t>();
}

inline int32_t
ByteSliceReader::ReadVInt32() {
    return (int32_t)ReadVUInt32();
}

inline uint32_t
ByteSliceReader::ReadVUInt32() {
...
inline int64_t
ByteSliceReader::ReadInt64() {
    return ReadInt<int64_t>();
}

inline uint64_t
ByteSliceReader::ReadUInt64() {
    return ReadInt<uint64_t>();
}

inline int32_t
ByteSliceReader::PeekInt32() {
    ...
    for(size_t i = 0; i < sizeof(int32_t); ++i) {
        if(cur_slice_offset >= GetSliceDataSize(slice)) {
            ...
            if(next_slice == NULL || next_slice->data_ == NULL) {
                StorageError(fmt::format("Read past EOF, State: list length = {}, offset = {}",
                                         GetSize(),
                                         global_offset_));
            } else {
                slice = next_slice;
