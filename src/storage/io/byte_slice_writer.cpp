...
ByteSliceWriter::ByteSliceWriter(MemoryPool* pool, uint32_t min_slice_size)
        : pool_(pool), last_slice_size_(min_slice_size - ByteSlice::GetHeadSize()), is_own_slice_list_(true),
          allocated_size_(0) {
...
ByteSlice*
ByteSliceWriter::CreateSlice(uint32_t size) {
...
size_t
ByteSliceWriter::GetSize() const { return size_t(slice_list_->GetTotalSize()); }

void
ByteSliceWriter::Dump(const std::shared_ptr<FileWriter>& file) {
...
void
ByteSliceWriter::Write(const void* value, size_t len) {
...
void
ByteSliceWriter::Write(ByteSliceList& src) { slice_list_->MergeWith(src); }

void
ByteSliceWriter::Write(const ByteSliceList& src, uint32_t start, uint32_t end) {
...
void
ByteSliceWriter::SnapShot(ByteSliceWriter& byte_slice_writer) const {
