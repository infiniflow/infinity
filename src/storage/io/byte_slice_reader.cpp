...
ByteSliceReader::ByteSliceReader()
        : current_slice_(nullptr), current_slice_offset_(0), global_offset_(0), slice_list_(nullptr), size_(0) {}

ByteSliceReader::ByteSliceReader(ByteSliceList* slice_list) : ByteSliceReader() { Open(slice_list); }

void
ByteSliceReader::Open(ByteSliceList* slice_list) {
    Close();
...
void
ByteSliceReader::Open(ByteSlice* slice) {
    Close();
...
size_t
ByteSliceReader::Read(void* value, size_t len) {
...
size_t
ByteSliceReader::ReadMayCopy(void*& value, size_t len) {
    if(current_slice_ == nullptr || len == 0)
        return 0;
...
size_t
ByteSliceReader::Seek(size_t offset) {
    if(offset < global_offset_) {
        StorageError(fmt::format("invalid offset value: seek offset = {}, State: list length = {}, offset = {}",
                                 offset,
                                 GetSize(),
                                 global_offset_));
    }
