...
FileReader::FileReader(FileSystem &fs, const String &path, SizeT buffer_size)
    : fs_(fs), path_(path), buffer_size_(buffer_size), data_(MakeUnique<char_t[]>(buffer_size)), buffer_offset_(0), buffer_start_(0) {
...
FileReader::FileReader(const FileReader &other)
    : fs_(other.fs_), path_(other.path_), buffer_size_(other.buffer_size_), data_(MakeUnique<char_t[]>(buffer_size_)) {}
...
i64 FileReader::GetFilePointer() const { return buffer_start_ + buffer_offset_; }

void FileReader::Seek(const i64 pos) {
