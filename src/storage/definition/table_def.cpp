...
bool TableDef::operator==(const TableDef &other) const {
    if (this->schema_name_ == nullptr || other.schema_name_ == nullptr || this->table_name_ == nullptr || other.table_name_ == nullptr ||
        *(this->schema_name_) != *(other.schema_name_) || *(this->table_name_) != *(other.table_name_) ||
    ...
    for (int32_t i = 0; i < this->columns_.size(); i++) {
        ...
        if (cd1.type_ != cd2.type_ || cd1.id_ != cd2.id_ || cd1.column_type_ == nullptr || cd2.column_type_ == nullptr ||
            *(cd1.column_type_) != *(cd2.column_type_) || cd1.name_ != cd2.name_ || cd1.constraints_.size() != cd2.constraints_.size())
            return false;
...
int32_t TableDef::GetSizeInBytes() const {
    int32_t size = 0;
