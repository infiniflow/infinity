...
bool DataFileWorker::WriteSnapshotFileImpl(std::unique_ptr<LocalFileHandle> file_handle, const FileWorkerSaveCtx &ctx) {
    u64 magic_number = 0x00dd3344;
    ...
    size_t data_size = data_size_.load();
    status = file_handle->Append(data_, data_size);
    if (!status.ok()) {
        RecoverableError(status);
    }
