...
void DataFileWorker::WriteToFileImpl(bool &prepare_success) {
    ...
    if (nbytes != sizeof(magic_number)) {
        Error<StorageException>(fmt::format("Write magic number which length is {}.", nbytes));
    }

    nbytes = fs.Write(*file_handler_, const_cast<SizeT *>(&buffer_size_), sizeof(buffer_size_));
    if (nbytes != sizeof(buffer_size_)) {
        Error<StorageException>(fmt::format("Write buffer length field which length is {}.", nbytes));
    }

    nbytes = fs.Write(*file_handler_, data_, buffer_size_);
    if (nbytes != buffer_size_) {
        Error<StorageException>(fmt::format("Expect to write buffer with size: {}, but {} bytes is written", buffer_size_, nbytes));
    }
...
void DataFileWorker::ReadFromFileImpl() {
    ...
    if (file_size < sizeof(u64) * 3) {
        Error<StorageException>(fmt::format("Incorrect file length {}.", file_size));
    }
    ...
    if (nbytes != sizeof(magic_number)) {
        Error<StorageException>(fmt::format("Read magic number which length isn't {}.", nbytes));
    }
    if (magic_number != 0x00dd3344) {
        Error<StorageException>(fmt::format("Incorrect file header magic number: {}.", magic_number));
    }

    u64 buffer_size_{};
    nbytes = fs.Read(*file_handler_, &buffer_size_, sizeof(buffer_size_));
    if (nbytes != sizeof(buffer_size_)) {
        Error<StorageException>(fmt::format("Unmatched buffer length: {} / {}", nbytes, buffer_size_));
    }
    if (file_size != buffer_size_ + 3 * sizeof(u64)) {
        Error<StorageException>(fmt::format("File size: {} isn't matched with {}.", file_size, buffer_size_ + 3 * sizeof(u64)));
    }
    ...
    if (nbytes != buffer_size_) {
        Error<StorageException>(fmt::format("Expect to read buffer with size: {}, but {} bytes is read", buffer_size_, nbytes));
    }
    ...
    if (nbytes != sizeof(checksum)) {
        Error<StorageException>(fmt::format("Incorrect file checksum length: {}.", nbytes));
    }
}
