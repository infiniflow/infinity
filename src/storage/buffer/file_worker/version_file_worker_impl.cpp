...
void VersionFileWorker::Read(BlockVersion *&data, std::unique_ptr<LocalFileHandle> &file_handle, size_t file_size) {
    ...
    if (!inited_) {
        ...
        //     */);
        segment_ = boost::interprocess::managed_mapped_file(boost::interprocess::open_or_create_infinity, tmp_path.c_str(), 2 * 1024 * 1024);
        boost::interprocess::allocator<void, segment_manager_t> alloc_inst(segment_.get_segment_manager());
        data = segment_.find_or_construct<BlockVersion>(path.c_str())(8192, alloc_inst);
        inited_ = true;
        return;
    }
...
void VersionFileWorker::GrowNolock() {
    ...
    auto size = segment_.get_size();
    segment_.~basic_managed_mapped_file();

    {
        segment_ = boost::interprocess::managed_mapped_file(boost::interprocess::open_only_infinity, tmp_path.c_str());
        boost::interprocess::managed_mapped_file::grow(tmp_path.c_str(), size);
        segment_.~basic_managed_mapped_file();
    }
