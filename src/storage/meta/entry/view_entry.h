...
public:
    explicit ViewEntry(const SharedPtr<String> &base_dir,
        ...
        : BaseEntry(EntryType::kView), base_dir_(base_dir), create_view_info_(create_view_info), view_name_(std::move(view_name)),
          column_names_(std::move(column_names)), column_types_(std::move(column_types)), table_collection_meta_(table_collection_meta) {
        begin_ts_ = begin_ts;
        txn_id_ = txn_id;
    }
