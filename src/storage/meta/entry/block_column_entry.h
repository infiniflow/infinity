...
public:
    explicit
    BlockColumnEntry(const BlockEntry* block_entry,
                     u64 column_id,
                     const SharedPtr<String>& base_dir_ref)
            : BaseEntry(EntryType::kSegmentColumn),
              block_entry_(block_entry),
              column_id_(column_id),
              base_dir_(base_dir_ref)
              {}
...
public:
    ...
    MakeNewBlockColumnEntry(const BlockEntry* block_entry,
                            u64 column_id,
                            BufferManager* buffer_manager);

    static ColumnBuffer
    GetColumnData(BlockColumnEntry* column_data_entry,
                  BufferManager* buffer_manager);

    static void
    Append(BlockColumnEntry* column_entry,
    ...
    static void
    AppendRaw(BlockColumnEntry* block_column_entry,
              SizeT dst_offset,
              ptr_t src_ptr,
              SizeT data_size);

    static void
    Flush(BlockColumnEntry* block_column_entry,
          SizeT row_count);

    static nlohmann::json
    Serialize(const BlockColumnEntry* block_column_entry);

    static UniquePtr<BlockColumnEntry>
    Deserialize(const nlohmann::json& column_data_json,
