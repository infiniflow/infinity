...
Status NewTxn::AddColumns(const String &db_name, const String &table_name, const Vector<SharedPtr<ColumnDef>> &column_defs) {
    ...
    std::tie(old_column_defs, status) = table_meta->GetColumnDefs(kv_instance, begin_ts, commit_ts);
    if (!status.ok()) {
        return status;
    }
...
Status NewTxn::DropColumns(const String &db_name, const String &table_name, const Vector<String> &column_names) {
    ...
    for (const auto &column_name : column_names) {
        String column_key;
        std::tie(column_key, status) = table_meta->GetColumnKeyByColumnName(kv_instance, begin_ts, commit_ts, column_name);
        if (!status.ok()) {
            return status;
        }
...
Status NewTxn::CreateIndex(const String &db_name, const String &table_name, const SharedPtr<IndexBase> &index_base, ConflictType conflict_type) {
    ...
    status = table_meta->GetIndexID(kv_instance, begin_ts, commit_ts, *index_base->index_name_, index_key, index_id, create_index_ts);
    if (status.ok()) {
        if (conflict_type == ConflictType::kIgnore) {
            return Status::OK();
        }
    ...
    String index_id_str;
    std::tie(index_id_str, status) = table_meta->GetNextIndexID(kv_instance);
    if (!status.ok()) {
        return status;
    }
...
Status NewTxn::DropIndexByName(const String &db_name, const String &table_name, const String &index_name, ConflictType conflict_type) {
    ...
    String index_id;
    TxnTimeStamp create_index_ts;
    status = table_meta->GetIndexID(kv_instance, begin_ts, commit_ts, index_name, index_key, index_id, create_index_ts);
    if (!status.ok()) {
        if (conflict_type == ConflictType::kIgnore) {
            return Status::OK();
        }
    ...
    std::tie(column_defs, status) = table_meta->GetColumnDefs(kv_instance, begin_ts, commit_ts);
    if (!status.ok()) {
        return status;
    }
...
Tuple<SharedPtr<TableInfo>, Status> NewTxn::GetTableInfo(const String &db_name, const String &table_name) {
    ...
    status = table_meta->GetComment(kv_instance, *table_info);
    if (!status.ok()) {
        return {nullptr, status};
    }
...
Tuple<SharedPtr<TableIndexInfo>, Status> NewTxn::GetTableIndexInfo(const String &db_name, const String &table_name, const String &index_name) {
    ...
    status = table_index_meta->GetTableIndexInfo(kv_instance, begin_ts, commit_ts, *table_index_info);
...
Tuple<SharedPtr<SegmentInfo>, Status> NewTxn::GetSegmentInfo(const String &db_name, const String &table_name, SegmentID segment_id) {
    ...
    status = table_meta->CheckSegments(kv_instance, begin_ts, {segment_id});
    if (!status.ok()) {
        return {nullptr, status};
    }
...
Tuple<Vector<SharedPtr<SegmentInfo>>, Status> NewTxn::GetSegmentsInfo(const String &db_name, const String &table_name) {
    ...
    for (SegmentID segment_id : *segment_ids_ptr) {
        ...
        std::tie(segment_info, status) = segment_meta.GetSegmentInfo(kv_instance, begin_ts, commit_ts);
        if (!status.ok()) {
            return {segment_info_list, status};
        }
...
NewTxn::GetBlockColumnInfo(const String &db_name, const String &table_name, SegmentID segment_id, BlockID block_id, ColumnID column_id) {
    ...
    SegmentMeta segment_meta(segment_id, table_meta.value());
    BlockMeta block_meta(block_id, segment_meta);
    return block_meta.GetBlockColumnInfo(kv_instance, begin_ts, commit_ts, column_id);
}
...
Tuple<SharedPtr<DatabaseSnapshotInfo>, Status> NewTxn::GetDatabaseSnapshotInfo(const String &db_name) {
    ...
    Vector<String> *table_names_ptr = nullptr;
    status = db_meta->GetTableIDs(kv_instance, begin_ts, table_ids_ptr, &table_names_ptr);
    if (!status.ok()) {
        return {nullptr, status};
    }
    ...
    for (size_t i = 0; i < table_ids_ptr->size(); i++) {
        ...
        SharedPtr<TableSnapshotInfo> table_snapshot_info;
        std::tie(table_snapshot_info, status) = table_meta->MapMetaToSnapShotInfo(db_name, table_names_ptr->at(i), kv_instance, begin_ts, commit_ts);
        if (!status.ok()) {
            return {nullptr, status};
        }
...
Status NewTxn::RestoreTableSnapshot(const String &db_name, const SharedPtr<TableSnapshotInfo> &table_snapshot_info) {
    ...
    String table_key;
    TxnTimeStamp table_create_ts;
    status = db_meta->GetTableID(kv_instance, begin_ts, table_name, table_key, table_id_str, table_create_ts);
    ...
    // Get the latest table id
    std::tie(table_id_str, status) = db_meta->GetNextTableID(kv_instance);
    if (!status.ok()) {
        return status;
    }
...
NewTxn::GetTableIndexMeta(const String &index_name, TableMeeta &table_meta, Optional<TableIndexMeeta> &table_index_meta, String *index_key_ptr) {
    ...
    String index_id_str;
    TxnTimeStamp create_index_ts;
    Status status = table_meta.GetIndexID(kv_instance, begin_ts, commit_ts, index_name, index_key, index_id_str, create_index_ts);
    if (!status.ok()) {
        return status;
    }
...
Status NewTxn::PrepareCommitAddColumns(const WalCmdAddColumnsV2 *add_columns_cmd) {
    ...
    status = table_meta->GetNextColumnID(kv_instance, next_column_id);
    if (!status.ok()) {
        return status;
    }
    for (const auto &column : add_columns_cmd->column_defs_) {
        column->id_ = next_column_id++;
        status = table_meta->AddColumn(kv_instance, commit_ts, *column);
        if (!status.ok()) {
            return status;
        }
    }
    status = table_meta->SetNextColumnID(kv_instance, next_column_id);
    if (!status.ok()) {
        return status;
    }
...
Status NewTxn::RestoreTableFromSnapshot(const WalCmdRestoreTableSnapshot *restore_table_snapshot_cmd, DBMeeta &db_meta, bool is_link_files) {
    ...
    status = table_meta->RestoreFromSnapshot(kv_instance,
                                             begin_ts,
                                             commit_ts,
                                             const_cast<WalCmdRestoreTableSnapshot *>(restore_table_snapshot_cmd),
                                             is_link_files);
    if (!status.ok()) {
        return status;
    }
    ...
    status = table_meta->LoadSet(kv_instance, begin_ts);
    if (!status.ok()) {
        return status;
    }
    ...
    for (auto &meta : metas) {
        switch (meta->type_) {
            case MetaType::kDB: {
                ...
                status = NewCatalog::CleanDB(kv_instance, begin_ts, commit_ts, db_meta, UsageFlag::kOther);
                if (!status.ok()) {
                    return status;
                }
                break;
            ...
            case MetaType::kSegment: {
                ...
                status = NewCatalog::CleanSegment(kv_instance, begin_ts, segment_meta, UsageFlag::kOther);
                if (!status.ok()) {
                    return status;
                }
                break;
            ...
            case MetaType::kChunkIndex: {
                ...
                Status status = NewCatalog::CleanChunkIndex(chunk_index_meta, kv_instance, begin_ts, commit_ts, UsageFlag::kOther);
                if (!status.ok()) {
                    return status;
                }
                break;
...
NewTxn::GetBlockFilePaths(const String &db_name, const String &table_name, SegmentID segment_id, BlockID block_id, Vector<String> &file_paths) {
    ...
    TxnTimeStamp begin_ts = txn_context_ptr_->begin_ts_;
    TxnTimeStamp commit_ts = txn_context_ptr_->commit_ts_;
    return NewCatalog::GetBlockFilePaths(kv_instance, begin_ts, commit_ts, block_meta, file_paths);
}
...
Status NewTxn::GetBlockColumnFilePaths(const String &db_name,
    ...
    return NewCatalog::GetBlockColumnFilePaths(kv_instance, begin_ts, commit_ts, column_meta, file_paths);
}
...
Status NewTxn::GetColumnFilePaths(const String &db_name, const String &table_name, const String &column_name, Vector<String> &file_paths) {
    ...
    {
        auto [column_defs, col_status] = table_meta->GetColumnDefs(kv_instance, begin_ts, commit_ts);
        if (!col_status.ok()) {
            return col_status;
        }
    ...
    return NewCatalog::GetColumnFilePaths(kv_instance, begin_ts, commit_ts, *table_meta, column_def, file_paths);
}
...
Status NewTxn::CommitBottomCreateTableSnapshot(WalCmdCreateTableSnapshot *create_table_snapshot_cmd) {
    ...
    SharedPtr<TableSnapshotInfo> table_snapshot_info;
    std::tie(table_snapshot_info, status) = table_meta->MapMetaToSnapShotInfo(create_table_snapshot_cmd->db_name_,
                                                                              create_table_snapshot_cmd->table_name_,
                                                                              kv_instance,
                                                                              begin_ts,
                                                                              commit_ts);
    if (!status.ok()) {
