...
module infinity_core:txn_allocator.impl;
...
void TxnAllocator::Submit(std::shared_ptr<TxnAllocatorTask> task) {
    ++task_count_;
...
void TxnAllocator::Process() {
    bool running{true};
    std::deque<std::shared_ptr<TxnAllocatorTask>> tasks;
    while (running) {
            ...
            if (txn_allocator_task->IsStopTask()) {
                ...
                switch (txn_type) {
                    case TransactionType::kAppend: {
                        ...
                        size_t row_count = txn_store->input_block_->row_count();
