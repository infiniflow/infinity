...
Status NewTxn::DumpMemIndex(const String &db_name, const String &table_name, const String &index_name) {
    ...
    for (SegmentID segment_id : *segment_ids_ptr) {
        ...
        chunk_infos.emplace_back(chunk_index_meta, kv_instance_.get());
...
Status NewTxn::OptimizeIndexInner(SegmentIndexMeta &segment_index_meta,
    ...
    auto [old_chunk_ids_ptr, status] = segment_index_meta.GetChunkIDs1(kv_instance, begin_ts, commit_ts);
    if (!status.ok()) {
        return status;
    }
    ...
    if (index_base->index_type_ == IndexType::kFullText) {
        ...
        for (ChunkID old_chunk_id : *old_chunk_ids_ptr) {
            ...
            {
                status = old_chunk_meta.GetChunkInfo(kv_instance_.get(), chunk_info_ptr);
                if (!status.ok()) {
                    return status;
                }
            }
    ...
    BufferObj *buffer_obj = nullptr;
    {
        ...
        status = chunk_index_meta->GetIndexBuffer(kv_instance_.get(), begin_ts, buffer_obj);
        if (!status.ok()) {
            return status;
        }
    }
    ...
    switch (index_base->index_type_) {
        case IndexType::kSecondary: {
            ...
            for (SizeT i = 0; i < deprecate_ids.size(); ++i) {
                ...
                BufferObj *buffer_obj = nullptr;
                {
                    ...
                    status = old_chunk_meta.GetIndexBuffer(kv_instance_.get(), begin_ts, buffer_obj);
                    if (!status.ok()) {
                        return status;
                    }
                }
        ...
        case IndexType::kEMVB: {
            ...
            data_ptr->BuildEMVBIndex(base_rowid, row_cnt, segment_meta, kv_instance, begin_ts, commit_ts, column_def);
            break;
        }
        default: {
...
Status NewTxn::AppendIndex(TableIndexMeeta &table_index_meta, const Pair<RowID, u64> &append_range) {
    ...
    SizeT column_idx = 0;
    {
        auto [column_def, status] =
            table_index_meta.table_meta().GetColumnDefByColumnName(kv_instance, begin_ts, commit_ts, index_base->column_name(), &column_idx);
        if (!status.ok()) {
            return status;
        }
    }
    ...
    if (!segment_meta || segment_meta->segment_id() != segment_id) {
        ...
        if (!table_index_meta.HasSegmentIndexID(kv_instance, begin_ts, commit_ts, segment_id) && append_range.first.segment_offset_ == 0) {
...
NewTxn::AppendMemIndex(SegmentIndexMeta &segment_index_meta, BlockID block_id, const ColumnVector &col, BlockOffset offset, BlockOffset row_cnt) {
    ...
    switch (index_base->index_type_) {
        case IndexType::kSecondary: {
            ...
            if (is_null) {
                auto [column_def, status] = segment_index_meta.table_index_meta().GetColumnDef(kv_instance, begin_ts, commit_ts);
                if (!status.ok()) {
                    return status;
                }
        ...
        case IndexType::kEMVB: {
            ...
            memory_emvb_index->Insert(col, offset, row_cnt, kv_instance, begin_ts, commit_ts);
            break;
        }
        default: {
            UnrecoverableError("Not implemented yet");
...
Status NewTxn::PopulateIndex(const String &db_name,
    ...
    {
        auto [column_def_, status] =
            table_index_meta.table_meta().GetColumnDefByColumnName(kv_instance, begin_ts, commit_ts, index_base->column_name());
        if (!status.ok()) {
            return status;
        }
...
Status NewTxn::ReplayDumpIndex(WalCmdDumpIndexV2 *dump_index_cmd) {
    ...
    {
        auto [segment_ids_ptr, index_status] = table_index_meta->GetSegmentIndexIDs1(kv_instance, begin_ts, commit_ts);
        if (!index_status.ok()) {
            return index_status;
        }
        auto iter = std::find(segment_ids_ptr->begin(), segment_ids_ptr->end(), segment_id);
        if (iter == segment_ids_ptr->end()) {
            index_status = NewCatalog::AddNewSegmentIndex1(*table_index_meta, this, segment_id, segment_index_meta_opt);
            if (!index_status.ok()) {
                return index_status;
            }
        } else {
...
Status NewTxn::PopulateIndexToMem(SegmentIndexMeta &segment_index_meta, SegmentMeta &segment_meta, ColumnID column_id, SizeT segment_row_cnt) {
    ...
    for (BlockID block_id : *block_ids) {
        ...
        ColumnVector col;
        status = NewCatalog::GetColumnVector(column_meta, kv_instance, begin_ts, commit_ts, row_cnt, ColumnVectorMode::kReadOnly, col);
        if (!status.ok()) {
            return status;
        }
        u32 offset = 0;
...
Status NewTxn::PopulateEmvbIndexInner(SharedPtr<IndexBase> index_base,
    ...
    }
    {
        BufferHandle buffer_handle = buffer_obj->Load();
        auto *data_ptr = static_cast<EMVBIndex *>(buffer_handle.GetDataMut());
        data_ptr->BuildEMVBIndex(base_row_id, row_count, segment_meta, kv_instance, begin_ts, commit_ts, column_def);
    }
    buffer_obj->Save();
    return Status::OK();
}
...
Status NewTxn::OptimizeFtIndex(SharedPtr<IndexBase> index_base,
    ...
    for (auto iter = chunk_ids.begin(); iter != chunk_ids.end(); ++iter) {
        ...
        ChunkIndexMetaInfo *chunk_info_ptr = nullptr;
        {
            Status status = chunk_index_meta.GetChunkInfo(kv_instance_.get(), chunk_info_ptr);
            if (!status.ok()) {
                return status;
            }
        }
    ...
    switch (index_base->index_type_) {
        case IndexType::kBMP: {
            ...
            for (ChunkID chunk_id : *chunk_ids_ptr) {
                ...
                status = chunk_index_meta.GetIndexBuffer(kv_instance_.get(), begin_ts, index_buffer);
                if (!status.ok()) {
                    return status;
                }
...
Status NewTxn::CountMemIndexGapInSegment(SegmentIndexMeta &segment_index_meta, SegmentMeta &segment_meta, Vector<Pair<RowID, u64>> &append_ranges) {
    ...
    for (ChunkID chunk_id : *chunk_ids_ptr) {
        ...
        status = chunk_index_meta.GetChunkInfo(kv_instance_.get(), chunk_index_meta_info_ptr);
        if (!status.ok()) {
            return status;
        }
...
Status NewTxn::RecoverMemIndex(TableIndexMeeta &table_index_meta) {
    ...
    std::tie(index_segment_ids_ptr, status) = table_index_meta.GetSegmentIndexIDs1(kv_instance, begin_ts, commit_ts);
    if (!status.ok()) {
        return status;
    }
...
Status NewTxn::RestoreTableIndexesFromSnapshot(TableMeeta &table_meta, const Vector<WalCmdCreateIndexV2> &index_cmds, bool is_link_files) {
    ...
    if (!is_link_files) {
        KVInstance *kv_instance = kv_instance_.get();
        status = table_meta.SetNextIndexID(kv_instance, std::to_string(max_index_id + 1));
        if (!status.ok()) {
            return status;
        }
    }
...
Status NewTxn::ManualDumpIndex(const String &db_name, const String &table_name) {
    ...
    Vector<String> *index_names_ptr = nullptr;
    status = table_meta->GetIndexIDs(kv_instance, begin_ts, index_ids_ptr, &index_names_ptr);
    if (!status.ok()) {
        return status;
    }
    for (const auto &index_id : *index_ids_ptr) {
        ...
        for (SegmentID segment_id : *segment_ids_ptr) {
            ...
            Vector<ChunkID> old_chunk_ids;
            auto [existing_chunk_ids_ptr, chunk_status] = segment_index_meta.GetChunkIDs1(kv_instance, begin_ts, commit_ts);
            if (chunk_status.ok()) {
