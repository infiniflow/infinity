    ...
    Status FinalizeBlock() {
        if (block_meta_) {
            ...
            for (ColumnID i = 0; i < column_cnt_; ++i) {
                ...
                // auto data = std::make_shared<char[]>(data_size);
                // auto var = std::make_shared<char[]>(data_size);
                // data_file_worker->Write(data);
                // if (var_file_worker) {
                //     var_file_worker->Write(var);
                // }
            }
        }
        block_meta_.reset();
...
Status NewTxn::AddSegmentVersion(WalSegmentInfo &segment_info, SegmentMeta &segment_meta) {
    ...
    for (WalBlockInfo &block_info : segment_info.block_infos_) {
        ...
        // version_file_worker->Write(block_version);
    }
    return Status::OK();
}
...
Status NewTxn::WriteDataBlockToFile(const std::string &db_name,
    ...
    for (size_t i = 0; i < input_block->column_count(); ++i) {
        ...
        // auto var = std::make_shared<char[]>(data_size);
        //
        // [[maybe_unused]] auto foo = data_file_worker->Write(data);
        // if (var_file_worker) {
        //     [[maybe_unused]] auto foo = var_file_worker->Write(var);
        // }
    }
