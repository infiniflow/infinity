...
template<FastPForCodec Codec>
u32
FastPForWrapper<Codec>::Compress(
        const u32* src,
        u32 count,
        u32* dest,
...
template<FastPForCodec Codec>
const u32*
FastPForWrapper<Codec>::Decompress(
        const u32* src,
        u32 count,
        u32* dest,
        size_t& outsize) const {
...
template<FastPForCodec Codec>
void
FastPForWrapper<Codec>::ApplyDelta(u32* src, size_t count) {
...
template<FastPForCodec Codec>
void
FastPForWrapper<Codec>::RevertDelta(u32* src, size_t count) {
    using namespace FastPForLib;
    FastPForLib::Delta::inverseDeltaSIMD(src, count);
}

template
struct FastPForWrapper<FastPForCodec::FastPFor>;
template
struct FastPForWrapper<FastPForCodec::SIMDBitPacking>;
