...
WalSegmentInfoV2::WalSegmentInfoV2(SegmentID segment_id, const std::vector<BlockID> &block_ids) : segment_id_(segment_id) {
...
std::shared_ptr<WalCmd> WalCmd::ReadAdv(const char *&ptr, i32 max_bytes) {
    ...
    switch (cmd_type) {
        ...
        case WalCommandType::SET_SEGMENT_STATUS_SEALED: {
            ...
            cmd = std::make_shared<WalCmdSetSegmentStatusSealed>(db_name, table_name, segment_id, segment_filter_binary_data, block_filter_binary_data);
            break;
        }
        ...
        case WalCommandType::UPDATE_SEGMENT_BLOOM_FILTER_DATA: {
            ...
            cmd =
                std::make_shared<WalCmdUpdateSegmentBloomFilterData>(db_name, table_name, segment_id, segment_filter_binary_data, block_filter_binary_data);
            break;
        }
        ...
        case WalCommandType::RESTORE_TABLE_SNAPSHOT: {
            ...
            cmd = std::make_shared<WalCmdRestoreTableSnapshot>(db_name, db_id, table_name, table_id, snapshot_name, table_def, segment_infos, index_cmds, files, addr_serializer);
            break;
        }
...
WalCmdRestoreTableSnapshot::WalCmdRestoreTableSnapshot(const std::string &db_name, const std::string &db_id, const std::string &table_name, const std::string &table_id, const std::string &snapshot_name, std::shared_ptr<TableDef> table_def_, const std::vector<WalSegmentInfoV2> &segment_infos, const std::vector<WalCmdCreateIndexV2> &index_cmds, const std::vector<std::string> &files)
...
std::string WalCmdCleanup::ToString() const {
