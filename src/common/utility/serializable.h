...
template<>
inline int32_t
GetSizeInBytes(const String& value) {
    return sizeof(int32_t) + value.length();
}

template<typename T>
inline T
ReadBuf(char* const buf) {
    static_assert(std::is_standard_layout_v<T>, "T must be POD");
    T* ptr = (T*)buf;
    T value = *ptr;
    return value;
...
template<typename T>
inline void
WriteBuf(char* const buf, const T& value) {
    static_assert(std::is_standard_layout_v<T>, "T must be POD");
    T* ptr = (T*)buf;
    *ptr = value;
}
...
template<typename T>
inline void
WriteBufAdv(char*& buf, const T& value) {
    static_assert(std::is_standard_layout_v<T>, "T must be POD");
    T* ptr = (T*)buf;
    *ptr = value;
    buf += sizeof(T);
...
template<>
inline void
WriteBuf<String>(char* const buf, const String& value) {
...
template<>
inline void
WriteBufAdv<String>(char*& buf, const String& value) {
    int32_t len = value.length();
    WriteBufAdv(buf, len);
