...
public:
    ...
    void* data() const {
        return data_;    // The data buffer.
    }
    size_t size() const {
        return size_;    // In bytes.
    }
...
class HeapBufferAllocator : public BufferAllocator,
    Singleton<HeapBufferAllocator> {
public:
    explicit HeapBufferAllocator()
        : aligned_mode_(true) {}
    ...
    static HeapBufferAllocator* Get() {
        return std::addressof(Singleton<HeapBufferAllocator>::instance());
    }
...
private:
    ...
    NONCOPYABLE(HeapBufferAllocator);
};
...
private:
    ...
    NONCOPYABLE(ClearingBufferAllocator);
};
...
public:
    ...
    virtual size_t Available() const {
        lock_guard_maybe<std::mutex> lock(Quota<thread_safe>::mutex());
...
private:
    NONCOPYABLE(MemoryStatisticsCollectorInterface);
};
...
public:
    ...
    virtual size_t Available() const {
        lock_guard_maybe<std::mutex> lock(mutex());
...
private:
    ...
    mutable std::mutex mutex_;
    NONCOPYABLE(ThreadSafeBufferAllocator);
};
