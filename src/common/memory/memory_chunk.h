...
public:
    ...
    void*
    Allocate(size_t num_bytes, size_t align) {
    ...
    bool
    IsInChunk(const void* ptr) const { return ptr >= (void*)this && ptr < (void*)((char*)this + pos_); }

    bool
    IsEmpty() const { return (pos_ >= size_); }

    size_t
    GetTotalBytes() const { return size_; }

    size_t
    GetPos() const { return pos_; }

    size_t
    GetFreeSize() const { return size_ - pos_; }

    size_t
    GetUsedSize() const { return pos_ - holder_; }

    void
    Reset() { pos_ = holder_; }

    size_t
    GetHolderSize() const { return holder_; }
...
public:
    ...
    size_t
    GetUsedBytes() const { return used_bytes_; }

    size_t
    GetTotalBytes() const { return total_bytes_; }

    size_t
    GetChunkSize() const { return chunk_size_; }

    size_t
    GetAvailableChunkSize() const { return chunk_size_ - sizeof(ChainedMemoryChunk); }

    bool
    IsInChunk(const void* ptr) const {
...
inline MemoryChunk*
ChunkAllocator::Allocate(size_t num_bytes) {
