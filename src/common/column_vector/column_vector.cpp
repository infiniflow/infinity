...
void ColumnVector::Initialize(const ColumnVector &other, const Selection &input_select) {
    ...
    if (buffer_ == nullptr) {
        ...
        if (vector_buffer_type == VectorBufferType::kHeap) {
            StorageAssert(buffer_->heap_mgr_ == nullptr, "Vector heap should be null.") buffer_->heap_mgr_ = MakeUnique<StringHeapMgr>();
        }
    }
...
String ColumnVector::ToString() const {
    StorageAssert(initialized, "Column vector isn't initialized.") std::stringstream ss;
    switch (data_type_->type()) {
        case kBoolean: {
...
Value ColumnVector::GetValue(SizeT index) const {
    StorageAssert(initialized, "Column vector isn't initialized.") if (index >= tail_index_) {
...
void ColumnVector::Finalize(SizeT index) {
    StorageAssert(index <= capacity_, fmt::format("Attempt to set column vector tail index to {}, capacity: {}", index, capacity_));
    tail_index_ = index;
}
...
void ColumnVector::AppendWith(const ColumnVector &other, SizeT from, SizeT count) {
    ...
    StorageAssert(*this->data_type_ == *other.data_type_,
                  fmt::format("Attempt to append column vector{} to column vector{}", other.data_type()->ToString(), this->data_type()->ToString()));
    ...
    switch (data_type_->type()) {
        ...
        case kArray: {
            NotImplementError("Array copy") break;
        }
        case kTuple: {
            TypeError("Shouldn't store tuple directly, a tuple is flatten as many columns");
        }
...
void ColumnVector::Reserve(SizeT new_capacity) {
    StorageAssert(vector_type_ != ColumnVectorType::kConstant, "Constant column vector can only have one value")
        StorageAssert(initialized, "Column vector isn't initialized.") if (new_capacity <= capacity_) return;
...
bool ColumnVector::operator==(const ColumnVector &other) const {
    ...
    if (!this->initialized && !other.initialized)
        return true;
    if (!this->initialized || !other.initialized || this->data_type_ == nullptr || other.data_type_ == nullptr ||
    ...
    switch (data_type_->type()) {
        ...
        default:
            NotImplementError(fmt::format("Not supported data_type {}", int(data_type_->type())));
    }
    return true;
}
...
int32_t ColumnVector::GetSizeInBytes() const {
    StorageAssert(initialized, "Column vector isn't initialized.")
        NotImplementAssert(vector_type_ == ColumnVectorType::kFlat || vector_type_ == ColumnVectorType::kConstant,
                           fmt::format("Not supported vector_type {}", int(vector_type_)));
