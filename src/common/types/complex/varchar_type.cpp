...
VarcharType::VarcharType(const String &str) { Initialize(str); }
...
void VarcharType::Initialize(const String &input) {
...
void VarcharType::Initialize(const char *input_ptr, size_t input_len) {
    TypeAssert(input_len <= std::numeric_limits<u16>::max(), "Attempt to write string with length exceed 65535 into value");
...
String VarcharType::ToString() const {
    if (IsInlined()) {
