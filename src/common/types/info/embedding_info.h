...
public:
    inline static SharedPtr<EmbeddingInfo> Make(EmbeddingDataType embedding_data_type, SizeT dimension) {
        TypeAssert(dimension <= EMBEDDING_LIMIT,
                   "Embedding dimension should less than " + std::to_string(EMBEDDING_LIMIT)) return MakeShared<EmbeddingInfo>(embedding_data_type,
                                                                                                                               dimension);
    }
    ...
    [[nodiscard]] inline SizeT Size() const override { return EmbeddingType::EmbeddingSize(embedding_data_type_, dimension_); }

    [[nodiscard]] inline EmbeddingDataType Type() const noexcept { return embedding_data_type_; }

    [[nodiscard]] inline SizeT Dimension() const noexcept { return dimension_; }
    ...
    static String EmbeddingDataTypeToString(EmbeddingDataType type);
