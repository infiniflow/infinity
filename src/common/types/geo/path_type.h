...
public:
    ...
    explicit inline PathType(u32 count, i32 closed = 0) : point_count(count), closed(closed) {
        if (count == 0)
            return;
    ...
    inline void SetPoint(u32 index, PointType point) {
        if (ptr == nullptr)
    ...
    inline PointType GetPoint(u32 index) {
        if (ptr == nullptr)
            TypeError("Not initialized.");
        return ((PointType *)(ptr))[index];
    }

    [[nodiscard]] inline u32 PointCount() const { return point_count; }

    inline void Initialize(u32 count, i32 is_closed = 0) {
        if (point_count != 0) {
