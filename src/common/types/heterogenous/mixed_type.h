...
public:
    static MixedType MakeInteger(i64 value);

    static MixedType MakeFloat(f64 value);

    static MixedType MakeString(const String &str);

    static MixedType MakeTuple(u16 count);

    static MixedType MakeArray(u16 count);
...
public:
    void CopyIntoTuple(const String &key, const MixedType &value);

    void MoveIntoTuple(const String &key, MixedType &&value);

    void InsertIntegerIntoTuple(const String &key, i64 value);

    void InsertFloatIntoTuple(const String &key, f64 value);

    void InsertStringIntoTuple(const String &key, const String &value);

    void InsertNullIntoTuple(const String &key);

    MixedType *GetFromTuple(const String &key) const;

private:
    static void CheckKeyConflict(MixedType &key_ref, const String &key_str);

    // Array related method
public:
    void CopyIntoArray(const MixedType &value, u16 index);

    void MoveIntoArray(MixedType &&value, u16 index);

    void InsertIntegerIntoArray(i64 value, u16 index);

    void InsertFloatIntoArray(f64 value, u16 index);

    void InsertStringIntoArray(const String &value, u16 index);

    void InsertNullIntoArray(u16 index);

    MixedType *GetByIndex(u16) const;
