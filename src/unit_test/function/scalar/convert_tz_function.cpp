...
import convert_timezone;
import third_party;

using namespace infinity;

class ConvertTimeZoneFunctionsTest : public BaseTestParamStr {};

INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, ConvertTimeZoneFunctionsTest, ::testing::Values(BaseTestParamStr::NULL_CONFIG_PATH));

TEST_P(ConvertTimeZoneFunctionsTest, convert_tz_func) {
    using namespace infinity;

    UniquePtr<Catalog> catalog_ptr = MakeUnique<Catalog>();

    RegisterConvertTimeZoneFunction(catalog_ptr);
...

    {
        ...
        EXPECT_STREQ("converttz(Varchar)->Varchar", func.ToString().c_str());
