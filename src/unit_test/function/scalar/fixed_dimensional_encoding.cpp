...
TEST_F(FixedDimensionalEncodingTest, sql_function_test) {
    // Test the SQL function interface with multi-dimensional array input
    // This simulates the case: FDE([[1.0, 2.0], [3.0, 4.0]], 64)

    // Create a mock DataBlock with array input and target dimension
    Vector<SharedPtr<DataType>> column_types;

    // First column: Array type (for [[1.0, 2.0], [3.0, 4.0]])
    auto float_type = MakeShared<DataType>(LogicalType::kFloat);
    auto array_type_info = ArrayInfo::Make(*float_type);
    auto array_type = DataType(LogicalType::kArray, array_type_info);
    column_types.push_back(MakeShared<DataType>(array_type));

    // Second column: BigInt type (for target dimension)
    column_types.push_back(MakeShared<DataType>(LogicalType::kBigInt));

    // Create DataBlock
    DataBlock input_block;
    input_block.Init(column_types);

    // Create nested array value: [[1.0, 2.0], [3.0, 4.0]]
    Vector<Value> sub_array1_values = {Value::MakeFloat(1.0f), Value::MakeFloat(2.0f)};
    Vector<Value> sub_array2_values = {Value::MakeFloat(3.0f), Value::MakeFloat(4.0f)};

    Value sub_array1 = Value::MakeArray(std::move(sub_array1_values), array_type_info);
    Value sub_array2 = Value::MakeArray(std::move(sub_array2_values), array_type_info);

    Vector<Value> nested_array_values = {sub_array1, sub_array2};
    Value nested_array = Value::MakeArray(std::move(nested_array_values), array_type_info);

    // Create target dimension value
    Value target_dim = Value::MakeBigInt(64);

    // Add values to DataBlock
    input_block.column_vectors[0]->AppendValue(nested_array);
    input_block.column_vectors[1]->AppendValue(target_dim);
    input_block.Finalize();

    // Create output column vector
    auto result_embedding_info = EmbeddingInfo::Make(EmbeddingDataType::kElemFloat, 64);
    auto result_embedding_type = DataType(LogicalType::kEmbedding, result_embedding_info);
    SharedPtr<ColumnVector> output = ColumnVector::Make(result_embedding_type);
    output->Initialize();

    // Call FDE function
    FDEFunction(input_block, output);

    // Verify result
    EXPECT_EQ(output->Size(), 1u);

    Value result_value = output->GetValue(0);
    EXPECT_EQ(result_value.type().type(), LogicalType::kEmbedding);

    const auto *result_embedding_info_ptr = static_cast<const EmbeddingInfo *>(result_value.type().type_info().get());
    EXPECT_EQ(result_embedding_info_ptr->Dimension(), 64u);
    EXPECT_EQ(result_embedding_info_ptr->Type(), EmbeddingDataType::kElemFloat);
}
