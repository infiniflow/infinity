...
module infinity_core:ut.modulo_functions;
...
TEST_F(ModuloFunctionsTest, mod_func) {
    ...
    RegisterModuloFunction(catalog_ptr.get());
...

    {
        ...
        std::shared_ptr<ColumnExpression> col1_expr_ptr = std::make_shared<ColumnExpression>(data_type, "t1", 1, "c1", 0, 0);
        std::shared_ptr<ColumnExpression> col2_expr_ptr = std::make_shared<ColumnExpression>(data_type, "t1", 1, "c1", 0, 0);
