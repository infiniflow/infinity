...
TEST_F(IntervalCastTest, date_cast0) {
    ...
    // Try to cast Interval type to wrong type.
    {
        IntervalT source;
        TinyIntT target;
        EXPECT_THROW_WITHOUT_STACKTRACE(IntervalTryCastToVarlen::Run(source, target, nullptr), UnrecoverableException);
    }
    {
        IntervalT source;
        VarcharT target;
        ...
        EXPECT_THROW_WITHOUT_STACKTRACE(IntervalTryCastToVarlen::Run(source, target, col_varchar_ptr.get()), UnrecoverableException);
    }
}
...
TEST_F(IntervalCastTest, date_cast1) {
    ...
    std::shared_ptr<DataType> source_type = std::make_shared<DataType>(LogicalType::kInterval);
