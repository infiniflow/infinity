...
module infinity_core:ut.line_segment_cast;
...
TEST_F(LineSegCastTest, line_seg_cast1) {
    ...
    // Call BindGeographyCast with wrong type of parameters
    {
        ...
        EXPECT_THROW_WITHOUT_STACKTRACE(BindGeographyCast<LineT>(source_type, target_type), UnrecoverableException);
    }

    std::shared_ptr<DataType> source_type = std::make_shared<DataType>(LogicalType::kLineSeg);
