...
TEST_P(DumpMemIndexTaskTest, DISABLED_SLOW_row_cnt_exceed_memindex_capacity) {
    ...
    // Default mem index capacity is 8 * 8192 = 65536 rows (DEFAULT_MEMINDEX_CAPACITY)
    ...
    // Append 8192 rows for 8 times to exceed the mem index capacity
    ...
    // Second time: append 8192 rows for 8 times to exceed the mem index capacity
    for (size_t i = 0; i < 8; ++i) {
        append();
    }
