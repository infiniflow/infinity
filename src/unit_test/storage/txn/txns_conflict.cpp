...
TEST_P(TestTxnsConflictTest, import) {
    LOG_INFO("--import--");

    {
        String create_table_sql = "CREATE TABLE t1 (c1 integer, c2 varchar)";
        UniquePtr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(create_table_sql);
        bool ok = HandleQueryResult(query_result);
        EXPECT_TRUE(ok);
    ...
    {
        String import_sql = "COPY t1 FROM 'test/data/csv/pysdk_test_big_varchar_rows.csv' WITH ( DELIMITER "
                            "',', FORMAT CSV );";
         import_sql = "COPY t1 FROM 'test/data/csv/pysdk_test_import_default.csv' WITH ( DELIMITER "
                    "',', FORMAT CSV );";
        import_sql = "COPY t1 FROM 'b.csv' WITH ( DELIMITER "
                "',', FORMAT CSV );";
    ...
    {
        String select_sql = "select count(*) from t1";
        UniquePtr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(select_sql);
        bool ok = HandleQueryResult(query_result);
        EXPECT_TRUE(ok);
    ...
    {
        String select_sql = "show table t1";
        UniquePtr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(select_sql);
        bool ok = HandleQueryResult(query_result);
        EXPECT_TRUE(ok);
