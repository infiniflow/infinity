    ...
    {
        {
            ...
            Status status = txn->CreateDatabase(*db_name, ConflictType::kIgnore, std::make_shared<String>());
            EXPECT_TRUE(status.ok());
            status = new_txn_mgr_->CommitTxn(txn);
            EXPECT_TRUE(status.ok());
        }
...
TEST_P(TestTxnCleanupInternal, test_cleanup_table) {
    ...
    };
    {
        ...
        {
            auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>("append"), TransactionType::kNormal);
        ...
        {
            auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>("drop table"), TransactionType::kNormal);
...
TEST_P(TestTxnCleanupInternal, test_cleanup_index) {
    ...
    auto create_index = [&](const std::shared_ptr<IndexBase> &index_base) {
        auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>(fmt::format("create index {}", *index_base->index_name_)), TransactionType::kNormal);
    ...
    };
    {
        ...
        {
            auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>("drop index"), TransactionType::kNormal);
...
TEST_P(TestTxnCleanupInternal, test_cleanup_optimize) {
    ...
    auto dump_index = [&](const String &index_name) {
        auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>("dump index"), TransactionType::kNormal);
    ...
    auto merge_index = [&](SegmentID segment_id, const String &index_name) {
        auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>(fmt::format("merge index {}", index_name)), TransactionType::kNormal);
...
TEST_P(TestTxnCleanupInternal, test_cleanup_drop_column) {
    ...
    {
        auto *txn = new_txn_mgr_->BeginTxn(std::make_unique<String>("drop column"), TransactionType::kNormal);
...
TEST_P(TestTxnCleanupInternal, test_cleanup_drop_index_and_checkpoint_and_restart) {
    ...
    auto index_name1 = std::make_shared<std::string>("index1");
    auto index_def1 = IndexSecondary::Make(index_name1, std::make_shared<String>(), "my_file_name", {column_def1->name()});
