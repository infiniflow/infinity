...
TEST_P(TestTxnNewCatalog, test_table_mem_index) {
    ...
    NewCatalog *new_catalog = infinity::InfinityContext::instance().storage()->new_catalog();
    String table_key = KeyEncode::CatalogTableKey(db_id_str, "table_name", 1);

    Status status = new_catalog->IncreaseTableReferenceCountForMemIndex(table_key);
    EXPECT_TRUE(status.ok());
    status = new_catalog->IncreaseTableReferenceCountForMemIndex(table_key);
    EXPECT_TRUE(status.ok());

    SizeT ref_count = new_catalog->GetTableReferenceCountForMemIndex(table_key);
    ...
    status = new_catalog->UnsetMemIndexDump(table_key);
    EXPECT_TRUE(status.ok());
}
