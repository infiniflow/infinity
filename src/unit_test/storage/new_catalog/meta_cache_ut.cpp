...
TEST_F(MetaCacheTest, test_db) {
    MetaCache cache(2);
    {
        SharedPtr<MetaDbCache> db_cache = MakeShared<MetaDbCache>("db1", 0, 4, "catalog|db|db1|4", false, 0);
    ...
    {
        SharedPtr<MetaDbCache> db_cache = MakeShared<MetaDbCache>("db1", 0, 6, "catalog|db|db1|6", true, 0);
        cache.PutOrErase({db_cache}, nullptr);
        db_cache = cache.GetDb("db1", 3);
        EXPECT_EQ(db_cache.get(), nullptr);
        db_cache = cache.GetDb("db1", 5);
    ...
    {
        SharedPtr<MetaDbCache> db_cache = MakeShared<MetaDbCache>("db1", 0, 8, "catalog|db|db1|8", false, 0);
        cache.PutOrErase({db_cache}, nullptr);
        db_cache = cache.GetDb("db1", 3);
        EXPECT_EQ(db_cache.get(), nullptr);
        db_cache = cache.GetDb("db1", 5);
    ...
    }
    {
        ...
        db_cache = MakeShared<MetaDbCache>("db2", 1, 10, "catalog|db|db2|10", false, 0);
        cache.PutOrErase({db_cache}, nullptr);
...
TEST_F(MetaCacheTest, test_table) {
    MetaCache cache(2);
    {
        SharedPtr<MetaTableCache> table_cache = MakeShared<MetaTableCache>(0, "tbl1", 0, 4, "catalog|tbl|0|tbl1|4", false, 0);
    ...
    {
        SharedPtr<MetaTableCache> table_cache = MakeShared<MetaTableCache>(0, "tbl1", 0, 6, "catalog|tbl|0|tbl1|6", true, 0);
        cache.PutOrErase({table_cache}, nullptr);
        table_cache = cache.GetTable(0, "tbl1", 3);
        EXPECT_EQ(table_cache.get(), nullptr);
        table_cache = cache.GetTable(0, "tbl1", 5);
    ...
    {
        SharedPtr<MetaTableCache> table_cache = MakeShared<MetaTableCache>(0, "tbl1", 0, 8, "catalog|tbl|0|tbl1|8", false, 0);
        cache.PutOrErase({table_cache}, nullptr);
        table_cache = cache.GetTable(0, "tbl1", 3);
        EXPECT_EQ(table_cache.get(), nullptr);
        table_cache = cache.GetTable(0, "tbl1", 5);
    ...
    }
    {
        ...
        table_cache = MakeShared<MetaTableCache>(0, "tbl2", 1, 10, "catalog|tbl|0|tbl2|8", false, 0);
        cache.PutOrErase({table_cache}, nullptr);
...
TEST_F(MetaCacheTest, test_index) {
    MetaCache cache(2);
    {
        SharedPtr<MetaIndexCache> index_cache = MakeShared<MetaIndexCache>(0, 0, "idx1", 0, 4, "catalog|idx|0|0|idx1|4", false, 0);
    ...
    {
        SharedPtr<MetaIndexCache> index_cache = MakeShared<MetaIndexCache>(0, 0, "idx1", 0, 6, "catalog|idx|0|0|idx1|6", true, 0);
        cache.PutOrErase({index_cache}, nullptr);
        index_cache = cache.GetIndex(0, 0, "idx1", 3);
        EXPECT_EQ(index_cache.get(), nullptr);
        index_cache = cache.GetIndex(0, 0, "idx1", 5);
    ...
    {
        SharedPtr<MetaIndexCache> index_cache = MakeShared<MetaIndexCache>(0, 0, "idx1", 0, 8, "catalog|idx|0|0|idx1|8", false, 0);
        cache.PutOrErase({index_cache}, nullptr);
        index_cache = cache.GetIndex(0, 0, "idx1", 3);
        EXPECT_EQ(index_cache.get(), nullptr);
        index_cache = cache.GetIndex(0, 0, "idx1", 5);
    ...
    }
    {
        ...
        index_cache = MakeShared<MetaIndexCache>(0, 0, "idx2", 1, 10, "catalog|idx|0|0|idx2|10", false, 0);
        cache.PutOrErase({index_cache}, nullptr);
