...
protected:
    ...
    void CheckDataAfterSuccesfulCompact() {
        ...
        SizeT table_row_cnt;
        std::tie(table_row_cnt, status) = table_meta->GetTableRowCount(txn->kv_instance(), txn->BeginTS(), txn->CommitTS());
    ...
    void CheckIndexAfterSuccessfulCompact(const String &index_name) {
        ...
        {
            auto [segment_ids_ptr, status] = table_index_meta->GetSegmentIndexIDs1(txn->kv_instance(), txn->BeginTS(), txn->CommitTS());
            EXPECT_TRUE(status.ok());
            EXPECT_EQ(*segment_ids_ptr, Vector<SegmentID>({2}));
