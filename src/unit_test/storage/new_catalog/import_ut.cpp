...
TEST_P(TestTxnImport, test_import_with_index) {
    ...
    auto check_index = [&](const String &index_name) {
        auto *txn = new_txn_mgr->BeginTxn(std::make_unique<String>("check index1"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_with_index_rollback) {
    ...
    {
        auto *txn_import = new_txn_mgr->BeginTxn(std::make_unique<String>("import"), TransactionType::kNormal);
        Vector<std::shared_ptr<DataBlock>> input_blocks = {make_input_block(), make_input_block()};
...

    {
        ...
        auto *txn_add_column = new_txn_mgr->BeginTxn(std::make_unique<String>("add column"), TransactionType::kNormal);
        // Add two columns
        auto column_def3 =
...
TEST_P(TestTxnImport, test_insert_and_import) {
    ...
    {
        auto *txn = new_txn_mgr->BeginTxn(std::make_unique<String>("import2"), TransactionType::kNormal);
        Vector<std::shared_ptr<DataBlock>> input_blocks = {
    ...
    {
        auto *txn = new_txn_mgr->BeginTxn(std::make_unique<String>("append2"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_drop_db) {
    ...
    //                           t2                  drop db   commit (success)
    {
        ...
        // Import two segments, each segments contains two blocks
        for (size_t i = 0; i < 2; ++i) {
            auto *txn3 = new_txn_mgr->BeginTxn(std::make_unique<String>("import"), TransactionType::kNormal);
            Vector<std::shared_ptr<DataBlock>> input_blocks = {make_input_block(), make_input_block()};
        ...
        // Scan and check
        auto *txn5 = new_txn_mgr->BeginTxn(std::make_unique<String>("scan"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_drop_table) {
    ...
    //                           t2                  drop table   commit (success)
    {
        ...
        auto *txn6 = new_txn_mgr->BeginTxn(std::make_unique<String>("drop table"), TransactionType::kNormal);
        ...
        auto *txn8 = new_txn_mgr->BeginTxn(std::make_unique<String>("drop db"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_add_columns) {
    ...
    //         t2                  add columns    commit (success)
    {
        ...
        auto *txn6 = new_txn_mgr->BeginTxn(std::make_unique<String>("add columns"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_drop_columns) {
    ...
    //         t2                  drop columns        commit (success)
    {
        ...
        auto *txn4 = new_txn_mgr->BeginTxn(std::make_unique<String>("drop columns"), TransactionType::kNormal);
    ...
    //         t2                  drop columns   commit (success)
    {
        ...
        auto *txn4 = new_txn_mgr->BeginTxn(std::make_unique<String>("add columns"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_append_table) {
    ...
    //                           t2                  append   commit (success)
    {
        ...
        auto *txn6 = new_txn_mgr->BeginTxn(std::make_unique<String>("concurrent append 1"), TransactionType::kNormal);
    ...
    //                           t2                  append    commit (success)
    {
        ...
        auto *txn6 = new_txn_mgr->BeginTxn(std::make_unique<String>("concurrent append"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_import_table) {
    ...
    //                           t2                  import   commit (success)
    {
        ...
        // Import two segments, each segments contains two blocks
        auto *txn3 = new_txn_mgr->BeginTxn(std::make_unique<String>("import"), TransactionType::kNormal);
        Vector<std::shared_ptr<DataBlock>> input_blocks1 = {make_input_block(), make_input_block()};
        status = txn3->Import(*db_name, *table_name, input_blocks1);
        EXPECT_TRUE(status.ok());
        status = new_txn_mgr->CommitTxn(txn3);
        EXPECT_TRUE(status.ok());

        Vector<std::shared_ptr<DataBlock>> input_blocks2 = {make_input_block(), make_input_block()};
    ...
    //                           t2                  import           commit (success)
    {
        ...
        auto *txn6 = new_txn_mgr->BeginTxn(std::make_unique<String>("concurrent import"), TransactionType::kNormal);
        Vector<std::shared_ptr<DataBlock>> input_blocks2 = {make_input_block(), make_input_block()};
        status = txn6->Import(*db_name, *table_name, input_blocks2);
        EXPECT_TRUE(status.ok());
        status = new_txn_mgr->CommitTxn(txn6);
...
TEST_P(TestTxnImport, test_import_rename) {
    ...
    //                           t2                  rename    commit (success)
    {
        ...
        // rename table
        auto *txn4 = new_txn_mgr->BeginTxn(std::make_unique<String>("rename table"), TransactionType::kNormal);
...
TEST_P(TestTxnImport, test_import_and_compact) {
    ...
    //                           t2                  compact  commit (success)
    {
        ...
        // Import 1 segment, each segments contains two blocks
        auto *txn3 = new_txn_mgr->BeginTxn(std::make_unique<String>("import"), TransactionType::kNormal);
        Vector<std::shared_ptr<DataBlock>> input_blocks1 = {make_input_block(Value::MakeInt(1), Value::MakeVarchar("abcdefghijklmnopqrstuvwxyz")),
...
TEST_P(TestTxnImport, test_import_and_optimize_index) {
    ...
    //                           t2                  optimize index  commit (success)
    {
        ...
        auto *txn1 = new_txn_mgr->BeginTxn(std::make_unique<String>(fmt::format("import")), TransactionType::kNormal);
        Vector<std::shared_ptr<DataBlock>> input_blocks = {make_input_block(Value::MakeInt(1), Value::MakeVarchar("abcdefghijklmnopqrstuvwxyz"))};
