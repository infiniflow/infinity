...
TEST_P(TestTxnReplayTest, test_replay_create_db2) {
    ...
    {
        auto *txn1 = new_txn_mgr->BeginTxn(std::make_unique<String>("create db"), TransactionType::kNormal);
        Status status = txn1->CreateDatabase("db2", ConflictType::kError, std::make_shared<String>());
        EXPECT_TRUE(status.ok());
        status = new_txn_mgr->CommitTxn(txn1);
        EXPECT_TRUE(status.ok());
    }
...
TEST_P(TestTxnReplayTest, test_replay_flush_gap_create) {
    ...
    }
    {
        ...
        std::shared_ptr<String> index_name_ptr = std::make_shared<String>("index_name");
    ...
    {
        auto *txn = new_txn_mgr->BeginTxn(std::make_unique<String>("cleanup"), TransactionType::kCleanup);
...
TEST_P(TestTxnReplayTest, test_replay_flush_gap_append_rename) {
    ...
    {
        auto *txn = new_txn_mgr->BeginTxn(std::make_unique<String>("rename table"), TransactionType::kNormal);
...
TEST_P(TestTxnReplayTest, test_replay_flush_gap_append_dump_optimize_index) {
    ...
    {
        auto *txn = new_txn_mgr->BeginTxn(std::make_unique<String>("optimize indexes"), TransactionType::kNormal);
