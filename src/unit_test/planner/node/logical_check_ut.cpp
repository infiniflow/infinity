...
module infinity_core:ut.logical_check;
...
class LogicalCheckTest : public NewRequestTest {
public:
...
INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, LogicalCheckTest, ::testing::Values(BaseTestParamStr::NEW_CONFIG_PATH));

TEST_P(LogicalCheckTest, test1) {
    ...
    {
        std::string sql = "check table tb";
        std::unique_ptr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(sql);

        auto nodes = query_context->logical_planner()->LogicalPlans();
        for (const auto &node : nodes) {
            if (node->operator_type() == LogicalNodeType::kCheck) {
                i64 space = 4;
