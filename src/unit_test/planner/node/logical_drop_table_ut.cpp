...
module infinity_core:ut.logical_filter;
...
class LogicalDropTableTest : public NewRequestTest {
public:
...
INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, LogicalDropTableTest, ::testing::Values(BaseTestParamStr::NEW_CONFIG_PATH));

TEST_P(LogicalDropTableTest, test1) {
    ...
    {
        std::string sql = "drop table tb";
        std::unique_ptr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(sql);

        auto nodes = query_context->logical_planner()->LogicalPlans();
        for (const auto &node : nodes) {
            CheckLogicalNode(node, LogicalNodeType::kDropTable);
        }
