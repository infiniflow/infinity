...
module infinity_core:ut.logical_flush;
...
class LogicalFlushTest : public NewRequestTest {
public:
...
INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, LogicalFlushTest, ::testing::Values(BaseTestParamStr::NEW_CONFIG_PATH));

TEST_P(LogicalFlushTest, test1) {
    ...
    {
        std::string sql = "flush data";
        std::unique_ptr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(sql);

        auto nodes = query_context->logical_planner()->LogicalPlans();
        for (const auto &node : nodes) {
            CheckLogicalNode(node, LogicalNodeType::kFlush);
        }
...
#if 0
    {
        std::string sql = "flush log";
        std::unique_ptr<QueryContext> query_context = MakeQueryContext();
        QueryResult query_result = query_context->Query(sql);

        auto nodes = query_context->logical_planner()->LogicalPlans();
        for (const auto &node : nodes) {
            if (node->operator_type() == LogicalNodeType::kFlush) {
                i64 space = 4;
                LOG_INFO(fmt::format("ToString: {}", node->ToString(space)));
