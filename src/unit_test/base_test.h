...
protected:
    ...
    const char *GetPersistenceDir() { return "/var/infinity/persistence"; }
    ...
    void RemoveDbDirs() {
        const char *infinity_db_dirs[] = {GetFullDataDir(), GetFullWalDir(), GetFullLogDir(), GetFullTmpDir(), GetPersistenceDir()};
        for (auto &dir : infinity_db_dirs) {
            RemoveDirectory(dir);
        }
    }
