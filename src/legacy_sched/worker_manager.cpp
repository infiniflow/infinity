...
void WorkerManager::CreateWorkers(std::uint16_t max_cores) {
...
Worker *WorkerManager::GetLeastLoadedWorker() const { return nullptr; }

Worker *WorkerManager::GetMostLoadedWorker() const { return nullptr; }

Worker *WorkerManager::GetWorker(std::uint16_t worker_id) const {
    ExecutorAssert(worker_id < max_cores_, "worker_id overflow") return workers_[worker_id];
}
