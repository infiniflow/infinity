...
public:
    ...
    RingBufferIterator&
    operator=(const RingBufferIterator& other) {
    ...
    equal(RingBufferIterator const& other) const {
        return &data_ == &other.data_ && position_ == other.position_;
    }

    inline void
    increment() { position_ = (position_ + 1) % PG_MSG_BUFFER_SIZE; }

    [[nodiscard]] inline reference
    dereference() const { return data_[position_]; }
