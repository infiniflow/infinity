...
void ExplainLogicalPlan::Explain(const LogicalCreateTable *create_node, SharedPtr<Vector<SharedPtr<String>>> &result, i64 intent_size) {
    ...
    // Table name
    {
        String table_name_str = String(intent_size, ' ') + " - table name: " + *create_node->table_definitions()->table_name();
...
void ExplainLogicalPlan::Explain(const LogicalKnnScan *knn_scan_node, SharedPtr<Vector<SharedPtr<String>>> &result, i64 intent_size) {
    ...
    // Table index
    String table_index = String(intent_size, ' ') + " - table index: #" + std::to_string(knn_scan_node->TableIndex());
    result->emplace_back(MakeShared<String>(table_index));
    ...
    if (knn_scan_node->left_node() != nullptr) {
        intent_size += 2;
...
void ExplainLogicalPlan::Explain(const LogicalAggregate *aggregate_node, SharedPtr<Vector<SharedPtr<String>>> &result, i64 intent_size) {
    ...
    {
        String aggregate_table_index = String(intent_size, ' ') + " - aggregate table index: #" + std::to_string(aggregate_node->aggregate_index_);
    ...
    if (groups_count != 0) {
        ...
        String group_table_index = String(intent_size, ' ') + " - group by table index: #" + std::to_string(aggregate_node->groupby_index_);
    ...
    switch (show_node->scan_type()) {
        case ShowType::kShowTables: {
            ...
            String output_columns_str =
                String(intent_size, ' ') +
                " - output columns: [database, table, type, column_count, row_count, segment_count, block_count, segment_capacity]";
            result->emplace_back(MakeShared<String>(output_columns_str));
            break;
        }
...
void ExplainLogicalPlan::Explain(const BaseExpression *base_expression, String &expr_str) {
    switch (base_expression->type()) {
        case ExpressionType::kAggregate: {
            ...
            PlannerAssert(aggregate_expression->arguments().size() == 1, "More than one argument in aggregate function") expr_str +=
                aggregate_expression->aggregate_function_.name() + "(";
        ...
        case ExpressionType::kCast: {
            ...
            PlannerAssert(cast_expression->arguments().size() == 1, "More than one argument in cast function") expr_str += "CAST(";
        ...
        case ExpressionType::kColumn: {
            ...
            expr_str += column_expression->Name() + " (#" + std::to_string(column_expression->binding().table_idx) + "." +
        ...
        case ExpressionType::kReference: {
            ...
            expr_str += reference_expression->Name() + " (#" + std::to_string(reference_expression->column_index()) + ")";
            break;
        }
