...
void BindContext::AddBindContext(const SharedPtr<BindContext> &other_ptr) {
    ...
    for (const auto &table_name2index_pair : other_ptr->table_name2table_index_) {
        ...
        PlannerAssert(!table_name2table_index_.contains(table_name), fmt::format("{} was bound before", table_name));
    ...
    for (const auto &table_index2name_pair : other_ptr->table_table_index2table_name_) {
        ...
        PlannerAssert(!table_table_index2table_name_.contains(table_index), fmt::format("Table index: {} is bound before", table_index));
    ...
    for (auto &name_binding_pair : other_ptr->binding_by_name_) {
        ...
        PlannerAssert(!binding_by_name_.contains(binding_name), fmt::format("Table: {} was bound before", binding_name));
