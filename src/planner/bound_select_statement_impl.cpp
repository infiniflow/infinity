...
std::shared_ptr<LogicalNode> BoundSelectStatement::BuildPlan(QueryContext *query_context) {
    ...
    if (!group_by_expressions_.empty() || !aggregate_expressions_.empty()) {
        ...
        if (has_distinct_agg) {
            ...
            bool distinct_columns_all_same = true;

            // Track distinct columns for each aggregate expression separately
            std::vector<std::set<std::string>> aggregate_distinct_column_sets;
            std::set<std::string> all_distinct_column_names;
            std::vector<std::shared_ptr<BaseExpression>> distinct_agg_args;
            std::vector<std::shared_ptr<BaseExpression>> non_distinct_agg_args;
            std::set<std::string> non_distinct_args_set;

            for (const auto &agg_expr : aggregate_expressions_) {
                auto *ae = static_cast<AggregateExpression *>(agg_expr.get());
                std::set<std::string> current_agg_distinct_cols;
                for (const auto &arg : ae->arguments()) {
                    std::string arg_name = arg->Name();
                    if (ae->distinct()) {
                        current_agg_distinct_cols.insert(arg_name);
                        if (all_distinct_column_names.find(arg_name) == all_distinct_column_names.end()) {
                            all_distinct_column_names.insert(arg_name);
                            distinct_agg_args.push_back(arg);
                        }
                    } else {
                        has_non_distinct_agg = true;
                        if (non_distinct_args_set.find(arg_name) == non_distinct_args_set.end()) {
                            non_distinct_args_set.insert(arg_name);
                            non_distinct_agg_args.push_back(arg);
                        }
                    }
                }

                // Track this aggregate's distinct columns
                if (ae->distinct() && !current_agg_distinct_cols.empty()) {
                    if (!aggregate_distinct_column_sets.empty()) {
                        if (aggregate_distinct_column_sets.back() != current_agg_distinct_cols) {
                            distinct_columns_all_same = false;
                        }
                    }
                    aggregate_distinct_column_sets.push_back(current_agg_distinct_cols);
                }
            }

            // Helper functions for aggregate plan construction
            // Update projection expressions to reference aggregate output
            auto update_projection_for_aggregate = [&](std::vector<std::shared_ptr<BaseExpression>> &proj_exprs, u64 groupby_index, u64 agg_index) {
                std::string groupby_table = fmt::format("groupby_{}", groupby_index);
                std::string agg_table = "__agg";
                std::set<std::string> group_by_names_set;
                for (const auto &grp : group_by_expressions_) {
                    group_by_names_set.insert(grp->Name());
                }

                std::function<void(std::shared_ptr<BaseExpression> &)> update_agg_column_exprs = [&](std::shared_ptr<BaseExpression> &expr) {
                    if (!expr)
                        return;
                    if (auto *col = dynamic_cast<ColumnExpression *>(expr.get())) {
                        if (group_by_names_set.count(col->column_name()) > 0) {
                            size_t output_col_idx = 0;
                            for (size_t j = 0; j < group_by_expressions_.size(); ++j) {
                                if (group_by_expressions_[j]->Name() == col->column_name()) {
                                    output_col_idx = j;
                                    break;
                                }
                            }
                            auto new_col =
                                ColumnExpression::Make(col->Type(), groupby_table, groupby_index, col->column_name(), output_col_idx, col->depth());
                            new_col->alias_ = col->alias_;
                            new_col->source_position_ = col->source_position_;
                            expr = new_col;
                        } else {
                            std::string col_name = col->column_name();
                            size_t output_col_idx = col->binding().column_idx;
                            auto new_col = ColumnExpression::Make(col->Type(), agg_table, agg_index, col_name, output_col_idx, col->depth());
                            new_col->alias_ = col->alias_;
                            new_col->source_position_ = col->source_position_;
                            expr = new_col;
                        }
                        ...
                        for (auto &arg : expr->arguments()) {
                            update_agg_column_exprs(arg);
                        }
                    }
                };

                for (size_t i = 0; i < proj_exprs.size(); ++i) {
                    update_agg_column_exprs(proj_exprs[i]);
                }
            };

            // Add ORDER BY sort node
            auto add_order_by_node = [&](u64 table_index) {
                std::string table_name = fmt::format("groupby_{}", table_index);
                if (!order_by_expressions_.empty()) {
                    std::vector<std::shared_ptr<BaseExpression>> sort_exprs;
                    for (size_t i = 0; i < order_by_expressions_.size(); ++i) {
                        const auto &order_expr = order_by_expressions_[i];
                        size_t col_idx = 0;
                        for (size_t j = 0; j < names_ptr_->size(); ++j) {
                            if ((*names_ptr_)[j] == order_expr->Name()) {
                                col_idx = j;
                                break;
                            }
                        }
                        auto new_col = ColumnExpression::Make(*types_ptr_->at(col_idx), table_name, table_index, order_expr->Name(), col_idx, 0);
                        sort_exprs.push_back(new_col);
                    }
                    auto sort = std::make_shared<LogicalSort>(bind_context->GetNewLogicalNodeId(), sort_exprs, order_by_types_);
                    sort->set_left_node(root);
                    root = sort;
                }
            };

            // Add final Project node for output formatting
            auto add_final_project_node = [&](std::vector<std::shared_ptr<BaseExpression>> &proj_exprs) {
                u64 final_project_index = bind_context->GenerateTableIndex();
                auto final_project = std::make_shared<LogicalProject>(bind_context->GetNewLogicalNodeId(),
                                                                      proj_exprs,
                                                                      final_project_index,
                                                                      std::map<size_t, std::shared_ptr<HighlightInfo>>());
                final_project->set_left_node(root);
                root = final_project;
            };

            // Use HashAggregate + MergeHashAggregate for:
            // - Mixed aggregates (some distinct, some non-distinct)
            // - Different distinct columns across aggregates
            //
            // Use 2-level aggregates for:
            // - Single distinct column
            // - Same distinct columns across aggregates
            if (has_non_distinct_agg || !distinct_columns_all_same) {
                // LEVEL 1: HashAggregate (parallel)
                // For mixed aggregates, HashAggregate does not deduplicate distinct columns, MergeHashAggregate will handle deduplication.
                // For different distinct columns, do partial deduplication.
                u64 level1_output_index = bind_context->GenerateTableIndex();
                std::string level1_table_name = fmt::format("__level1_hash_agg_{}", level1_output_index);
                auto level1_hash_agg = std::make_shared<LogicalHashAggregate>(bind_context->GetNewLogicalNodeId(),
                                                                              group_by_expressions_,
                                                                              distinct_agg_args,
                                                                              non_distinct_agg_args,
                                                                              level1_output_index,
                                                                              level1_output_index);

                level1_hash_agg->set_left_node(root);
                root = level1_hash_agg;

                // Level 1 output order: hash_agg_group_by + distinct_agg_args + non_distinct_agg_args
                std::map<std::string, size_t> level1_column_index;
                size_t idx = 0;
                for (size_t i = 0; i < group_by_expressions_.size(); ++i, ++idx) {
                    level1_column_index[group_by_expressions_[i]->Name()] = idx;
                }
                for (size_t i = 0; i < distinct_agg_args.size(); ++i, ++idx) {
                    level1_column_index[distinct_agg_args[i]->Name()] = idx;
                }
                for (size_t i = 0; i < non_distinct_agg_args.size(); ++i, ++idx) {
                    level1_column_index[non_distinct_agg_args[i]->Name()] = idx;
                }

                // LEVEL 2: MergeHashAggregate (serial) - apply aggregates with global deduplication
                std::vector<std::shared_ptr<BaseExpression>> level2_groups;
                std::vector<std::shared_ptr<BaseExpression>> level2_aggregates;

                // If there was an original GROUP BY, add those columns to level2 groups
                for (size_t i = 0; i < group_by_expressions_.size(); ++i) {
                    const auto &group_expr = group_by_expressions_[i];
                    std::string group_name = group_expr->Name();
                    auto it = level1_column_index.find(group_name);
                    if (it == level1_column_index.end()) {
                        UnrecoverableError(fmt::format("Cannot find group by column {} in level 1 output", group_name));
                    }
                    auto new_col = ColumnExpression::Make(group_expr->Type(), level1_table_name, level1_output_index, group_name, it->second, 0);
                    level2_groups.push_back(new_col);
                }

                for (const auto &agg_expr : aggregate_expressions_) {
                    auto *ae = static_cast<AggregateExpression *>(agg_expr.get());
                    std::vector<std::shared_ptr<BaseExpression>> new_args;
                    for (const auto &arg : ae->arguments()) {
                        std::string arg_name = arg->Name();
                        auto it = level1_column_index.find(arg_name);
                        if (it == level1_column_index.end()) {
                            UnrecoverableError(fmt::format("Cannot find argument {} in level 1 output", arg_name));
                        }
                        auto new_col = ColumnExpression::Make(arg->Type(), level1_table_name, level1_output_index, arg_name, it->second, 0);
                        new_args.push_back(new_col);
                    }

                    auto new_agg = std::make_shared<AggregateExpression>(ae->aggregate_function_, new_args);
                    new_agg->SetDistinct(ae->distinct());
                    new_agg->SetCountStar(ae->IsCountStar());
                    level2_aggregates.push_back(new_agg);
                }

                u64 level2_groupby_index = bind_context->GenerateTableIndex();
                u64 level2_aggregate_index = bind_context->GenerateTableIndex();
                auto level2_merge_agg = std::make_shared<LogicalMergeHashAggregate>(bind_context->GetNewLogicalNodeId(),
                                                                                    level2_groups,
                                                                                    level2_aggregates,
                                                                                    level2_groupby_index,
                                                                                    level2_aggregate_index);

                level2_merge_agg->set_left_node(root);
                root = level2_merge_agg;

                auto out_types = level2_merge_agg->GetOutputTypes();
                auto out_names = level2_merge_agg->GetOutputNames();
                auto agg_types = std::make_shared<std::vector<std::shared_ptr<DataType>>>();
                auto agg_names_ptr = std::make_shared<std::vector<std::string>>();
                for (size_t i = group_by_expressions_.size(); i < out_types->size(); ++i) {
                    agg_types->push_back(out_types->at(i));
                    agg_names_ptr->push_back(out_names->at(i));
                }

                // Update projection expressions to reference level 2 output
                update_projection_for_aggregate(projection_expressions_, level2_groupby_index, level2_aggregate_index);

                // Add ORDER BY if present
                add_order_by_node(level2_groupby_index);

                // Add final Project node for output formatting
                add_final_project_node(projection_expressions_);

                return root;

            } else {
                // Use 2-level aggregation
                // LEVEL 1: DISTINCT is converted to GROUP BY
                std::vector<std::shared_ptr<BaseExpression>> level1_groups;
                for (const auto &group_by : group_by_expressions_) {
                    level1_groups.push_back(group_by);
                }
                for (const auto &distinct_col : distinct_agg_args) {
                    level1_groups.push_back(distinct_col);
                }

                std::vector<std::shared_ptr<BaseExpression>> level1_aggregates; // Empty
                u64 level1_output_index = bind_context->GenerateTableIndex();
                std::string level1_table_name = fmt::format("__level1_{}", level1_output_index);
                auto level1_aggregate = std::make_shared<LogicalAggregate>(bind_context->GetNewLogicalNodeId(),
                                                                           base_table_ref,
                                                                           level1_groups,
                                                                           level1_output_index,
                                                                           level1_aggregates,
                                                                           level1_output_index);
                level1_aggregate->set_left_node(root);
                root = level1_aggregate;
                ...
                // LEVEL 2: Aggregate
                std::vector<std::shared_ptr<BaseExpression>> level2_groupby_exprs;
                for (size_t i = 0; i < group_by_expressions_.size(); ++i) {
                    const auto &group_expr = group_by_expressions_[i];
                    std::string group_name = group_expr->Name();
                    auto it = level1_column_index.find(group_name);
                    ...
                    size_t level1_col_idx = it->second;
                    auto new_col = ColumnExpression::Make(group_expr->Type(), level1_table_name, level1_output_index, group_name, level1_col_idx, 0);
                    level2_groupby_exprs.push_back(new_col);
                }

                std::vector<std::shared_ptr<BaseExpression>> level2_aggregates;
                for (const auto &agg_expr : aggregate_expressions_) {
                    auto *ae = static_cast<AggregateExpression *>(agg_expr.get());
                    bool is_not_count = ae->aggregate_function_.name() != "COUNT";
                    if (is_not_count || ae->arguments().size() <= 1) {
                        // Normal aggregate
                        std::vector<std::shared_ptr<BaseExpression>> new_args;
                        for (const auto &arg : ae->arguments()) {
                            std::string arg_name = arg->Name();
                            auto it = level1_column_index.find(arg_name);
                            if (it == level1_column_index.end()) {
                                UnrecoverableError(fmt::format("Cannot find column {} in level 1 output", arg_name));
                            }
                            size_t level1_col_idx = it->second;
                            auto new_col = ColumnExpression::Make(arg->Type(), level1_table_name, level1_output_index, arg_name, level1_col_idx, 0);
                            new_args.push_back(new_col);
                        }
                        auto new_agg = std::make_shared<AggregateExpression>(ae->aggregate_function_, new_args);
                        new_agg->SetDistinct(false);
                        level2_aggregates.push_back(new_agg);
                    } else {
                        // COUNT(DISTINCT c1, c2, ...) - count rows
                        std::vector<std::shared_ptr<BaseExpression>> count_args;
                        if (!level1_groups.empty()) {
                            std::string first_col_name = level1_groups[0]->Name();
                            auto count_star_col =
                                ColumnExpression::Make(level1_groups[0]->Type(), level1_table_name, level1_output_index, first_col_name, 0, 0);
                            count_args.push_back(count_star_col);
                        }
                        auto new_agg = std::make_shared<AggregateExpression>(ae->aggregate_function_, count_args);
                        new_agg->SetDistinct(false);
                        new_agg->SetCountStar(true);
                        level2_aggregates.push_back(new_agg);
                    }
                }

                u64 level2_output_index = bind_context->GenerateTableIndex();
                u64 level2_groupby_index = bind_context->GenerateTableIndex();
                auto level2_aggregate = std::make_shared<LogicalAggregate>(bind_context->GetNewLogicalNodeId(),
                                                                           base_table_ref,
                                                                           level2_groupby_exprs,
                                                                           level2_groupby_index,
                                                                           level2_aggregates,
                                                                           level2_output_index);
                level2_aggregate->set_left_node(root);
                root = level2_aggregate;

                // Update projection expressions to reference level 2 output
                update_projection_for_aggregate(projection_expressions_, level2_groupby_index, level2_output_index);
