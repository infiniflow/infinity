...
LogicalKnnScan::GetColumnBindings() const {
...
LogicalKnnScan::GetOutputTypes() const {
...
LogicalKnnScan::table_collection_ptr() const {
    return base_table_ref_->table_entry_ptr_;
}

String
LogicalKnnScan::TableAlias() const {
    return base_table_ref_->alias_;
}

u64
LogicalKnnScan::TableIndex() const {
    return base_table_ref_->table_index_;
}


String
LogicalKnnScan::ToString(i64& space) {
    std::stringstream ss;
    String arrow_str;
    if(space > 3) {
        space -= 4;
