...
LogicalFusion::LogicalFusion(const u64 node_id, std::shared_ptr<BaseTableRef> base_table_ref, std::shared_ptr<FusionExpression> fusion_expr)
    : LogicalNode(node_id, LogicalNodeType::kFusion), base_table_ref_(std::move(base_table_ref)), fusion_expr_(std::move(fusion_expr)) {}

std::string LogicalFusion::ToString(i64 &space) const {
    std::stringstream ss;
