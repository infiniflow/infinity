...
public:
    ...
    static inline void Embedding2Json(const EmbeddingType &embedding, EmbeddingDataType type, size_t dimension, nlohmann::json& embedding_json) {
        switch (type) {
            case kElemBit: {
...
private:
    template <typename T>
    static inline void Embedding2JsonInternal(const EmbeddingType &embedding, size_t dimension, nlohmann::json& embedding_json) {
        for (size_t i = 0; i < dimension; ++i) {
            embedding_json.push_back(((T *)(embedding.ptr))[i]);
        }
    }

    static inline void BitmapEmbedding2JsonInternal(const EmbeddingType &embedding, size_t dimension, nlohmann::json& embedding_json) {
