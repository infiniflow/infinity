...
class PhysicalCreateIndex : public PhysicalOperator {
  public:
    ...
    void Execute(QueryContext *query_context, InputState *input_state,
  ...
  public:
    static inline SharedPtr<PhysicalCreateIndex>
    Make(SharedPtr<String> schema_name, SharedPtr<String> table_name,
         SharedPtr<IndexDef> index_definition, ConflictType conflict_type,
         SharedPtr<Vector<String>> output_names,
         SharedPtr<Vector<SharedPtr<DataType>>> output_types, u64 id) {
        return MakeShared<PhysicalCreateIndex>(schema_name, table_name,
                                               index_definition, conflict_type,
                                               output_names, output_types, id);
    }
    ...
    PhysicalCreateIndex(SharedPtr<String> schema_name,
                        ...
                        u64 id)
        : PhysicalOperator(PhysicalOperatorType::kCreateIndex, nullptr, nullptr,
                           id),
          schema_name_(schema_name), table_name_(table_name),
          index_def_ptr_(index_definition), conflict_type_(conflict_type),
