        ...
        auto choose_middle = [&](const VectorBlockRawIndex &middle, const VectorBlockRawIndex &input) -> bool {
            if (!middle) {
                ...
                const float *middle_val_ptr =
                    reinterpret_cast<const float *>(middle_data_block_array[middle.block_id_]->column_vectors_[score_column_idx]->data());
                const float *input_val_ptr = reinterpret_cast<const float *>(
                    input_data_block_array[input.block_id_ - middle_block_cnt]->column_vectors_[score_column_idx]->data());
