...
void PhysicalInsert::Execute(QueryContext *query_context,
                             InputState *input_state,
                             OutputState *output_state) {
    SizeT column_count = value_list_.size();
    SizeT table_collection_column_count =
        table_collection_entry_->columns_.size();
    if (column_count != table_collection_column_count) {
        ExecutorError(fmt::format(
            "Insert values count{} isn't matched with table column count{}.",
            column_count, table_collection_column_count));
    }
    ...
    for (SizeT expr_idx = 0; expr_idx < column_count; ++expr_idx) {
        ...
        evaluator.Execute(value_list_[expr_idx], expr_states[expr_idx],
                          output_block->column_vectors[expr_idx]);
        if (blocks_column[0].get() !=
            output_block->column_vectors[expr_idx].get()) {
    ...
    const String &db_name =
        *TableCollectionEntry::GetDBEntry(table_collection_entry_)->db_name_;
    const String &table_name = *table_collection_entry_->table_collection_name_;
    txn->Append(db_name, table_name, output_block);

    UniquePtr<String> result_msg = MakeUnique<String>(
        fmt::format("INSERTED {} Rows", output_block->row_count()));
    if (output_state == nullptr) {
        // Generate the result table
        Vector<SharedPtr<ColumnDef>> column_defs;
        SharedPtr<TableDef> result_table_def_ptr =
            MakeShared<TableDef>(MakeShared<String>("default"),
                                 MakeShared<String>("Tables"), column_defs);
        output_ =
            MakeShared<Table>(result_table_def_ptr, TableType::kDataTable);
        output_->SetResultMsg(std::move(result_msg));
    } else {
        InsertOutputState *insert_output_state =
            static_cast<InsertOutputState *>(output_state);
...
void PhysicalInsert::Execute(QueryContext *query_context) {
    Execute(query_context, nullptr, nullptr);
}
