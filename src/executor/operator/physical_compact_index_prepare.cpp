...
bool PhysicalCompactIndexPrepare::Execute(QueryContext *query_context, OperatorState *operator_state) {
    ...
    if (compact_index_prepare_operator_state->create_index_shared_data_.get() != nullptr) {
        (*compact_index_prepare_operator_state->create_index_shared_data_)[create_index_idx]->Init(new_table_ref->block_index_.get());
    }

    Txn *txn = query_context->GetTxn();
    auto status = txn->CreateIndexPrepare(table_index_entry, new_table_ref, prepare_, false);
    compact_state_data->AddNewIndex(table_index_entry, txn);
    if (!status.ok()) {
